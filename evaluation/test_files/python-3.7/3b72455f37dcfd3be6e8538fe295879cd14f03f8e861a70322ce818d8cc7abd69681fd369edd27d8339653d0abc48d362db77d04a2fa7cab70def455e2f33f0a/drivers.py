...
class RandomPath(VehicleDriverBase):
    ...
    def __init__(self, workspace, speed=1, dthresh=0.05, seed=0, headinggain=0.3, goalmarkerstyle=None):
        ...
        if goalmarkerstyle is None:
            self._goal_marker_style = {'marker': 'D', 'markersize': 6, 'color': 'r', 'linestyle': 'None'}
        else:
        ...
        self.verbose = True
        self._goal = None
        self._dthresh = dthresh * max(self._workspace[1] - self._workspace[0], self._workspace[3] - self._workspace[2])
        self._veh = None
    ...
    def demand(self):
        ...
        if d < self._dthresh or abs(d - self._d_prev) < 0.001:
            self._new_goal()
        self._d_prev = d
        speed = self._speed
        goal_heading = atan2(self._goal[1] - self._veh._x[1], self._goal[0] - self._veh._x[0])
