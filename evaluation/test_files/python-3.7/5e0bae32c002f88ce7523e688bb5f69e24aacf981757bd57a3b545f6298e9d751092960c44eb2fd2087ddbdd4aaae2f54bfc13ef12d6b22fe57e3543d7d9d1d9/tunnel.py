...
def select_random_ports(n):
    ...
    for sock in sockets:
        sock.close()
    return ports
_password_pat = re.compile('pass(word|phrase):'.encode('utf8'), re.IGNORECASE)
...
def _paramiko_tunnel(lport, rport, server, remoteip, keyfile=None, password=None):
    ...
    client.set_missing_host_key_policy(paramiko.WarningPolicy())
    try:
        client.connect(server, port, username=username, key_filename=keyfile, look_for_keys=True, password=password)
