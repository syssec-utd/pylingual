...
class TranslationModel(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--moe-freq', type=int, metavar='D', default=0, help='Frequency at which we insert MoE Transformer layers')
        parser.add_argument('--encoder-moe-freq', type=int, metavar='D', default=0, help='Frequency at which we insert MoE Transformer encoder layers')
        parser.add_argument('--decoder-moe-freq', type=int, metavar='D', default=0, help='Frequency at which we insert MoE Transformer decoder layers')
        parser.add_argument('--moe-expert-count', type=int, metavar='D', default=0, help='Number of experts in each MoE Layer')
        parser.add_argument('--moe-gating-use-fp32', default=False, action='store_true', help='Use FP32 computations in MoE top2 gating function')
        parser.add_argument('--moe-second-expert-policy', type=str, default='sampling', help='policy for second expert, options: all/sampling/random')
        parser.add_argument('--moe-normalize-gate-prob-before-dropping', default=False, action='store_true', help='whether to normalize gate probs before or after dropping experts for capacity and randomization')
        parser.add_argument('--moe-expert-ffn-dim', type=int, default=0, help='MoE Expert FFN dimension')
        parser.add_argument('--moe-top1-expert', default=False, action='store_true', help='Use top1 gate instead of top2')
        parser.add_argument('--moe-eval-capacity-token-fraction', type=float, default=0.25, help='Fraction of tokens as capacity during validationif set to negative, use same as training. range: (0.0, 1.0].')
        parser.add_argument('--moe-normalize-expert-grad', type=str, default='world_size', help="Divide expert gradients by (1) 'world_size' (2) 'sqrt_world_size'")
        parser.add_argument('--use-moe-pad-mask', default=False, action='store_true', help="Don't route padding tokens to any expert")
        parser.add_argument('--use-xmoe', default=False, action='store_true', help='Enable X-Moe')
        parser.add_argument('--freeze-moe', default=False, action='store_true', help='Freeze MoE Params')
        parser.add_argument('--deepnorm', default=False, action='store_true', help='Enable DeepNorm')
        parser.add_argument('--subln', default=False, action='store_true', help='Enable SubLN')
        parser.add_argument('--pretrained-dense-mt-model-path', type=str, default='')
        parser.add_argument('--alternate-ffn-embed-dim', type=int, default=0, help='FFN embed dim of alternate pseudo-MoE blocks')
        parser.add_argument('--rel-pos-buckets', type=int, default=0, help='')
        parser.add_argument('--max-rel-pos', type=int, default=0, help='')
    ...
    def build_model(cls, args, task):
        ...
        encoder_embed_positions = PositionalEmbedding(args.max_source_positions, args.encoder_embed_dim, src_dict.pad(), learned=args.encoder_learned_pos) if not args.no_token_positional_embeddings else None
        decoder_embed_positions = PositionalEmbedding(args.max_target_positions, args.decoder_embed_dim, tgt_dict.pad(), learned=args.decoder_learned_pos) if not args.no_token_positional_embeddings else None
        if args.share_decoder_input_output_embed:
            output_projection = torch.nn.Linear(decoder_embed_tokens.weight.shape[1], decoder_embed_tokens.weight.shape[0], bias=False)
            output_projection.weight = decoder_embed_tokens.weight
        else:
            output_projection = torch.nn.Linear(args.decoder_embed_dim, len(tgt_dict), bias=False)
            torch.nn.init.normal_(output_projection.weight, mean=0, std=args.decoder_embed_dim ** (-0.5))
        encoder = cls.build_encoder(args, encoder_embed_tokens, encoder_embed_positions, src_dict)
        decoder = cls.build_decoder(args, decoder_embed_tokens, decoder_embed_positions, output_projection, tgt_dict)
    ...
    def build_encoder(cls, args, embed_tokens, embed_positions, dictionary):
        ...
        return MTEncoder(config, embed_tokens, embed_positions, is_encoder_decoder=True, dictionary=dictionary)

    @classmethod
    def build_decoder(cls, args, embed_tokens, embed_positions, output_projection, dictionary):
        config = DecoderConfig()
        config.override(args)
        return MTDecoder(config, embed_tokens, embed_positions, output_projection, is_encoder_decoder=True, dictionary=dictionary)

    def forward(self, src_tokens, src_lengths, prev_output_tokens, return_all_hiddens: bool=False, features_only: bool=False, **kwargs):
        encoder_out = self.encoder(src_tokens, return_all_hiddens=return_all_hiddens)
        decoder_out = self.decoder(prev_output_tokens, encoder_out=encoder_out, features_only=features_only, return_all_hiddens=return_all_hiddens)
        return decoder_out
...
class MTEncoder(Encoder, FairseqEncoder):
    ...
    def reorder_encoder_out(self, encoder_out, new_order):
        ...
        return {'encoder_out': new_encoder_out, 'encoder_padding_mask': new_encoder_padding_mask, 'encoder_embedding': new_encoder_embedding, 'encoder_states': encoder_states}
