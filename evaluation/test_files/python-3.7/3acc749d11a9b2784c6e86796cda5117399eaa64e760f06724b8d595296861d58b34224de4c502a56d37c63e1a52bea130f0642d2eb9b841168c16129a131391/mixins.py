...
class DiffParserTestingMixin(object):
    ...
    def assert_parsed_diff(self, parsed_diff, num_changes=0, **attrs):
        ...
        """
        self._assert_parsed_diff_obj(parsed_diff, attrs=attrs, default_attrs={'uses_commit_ids_as_revisions': False})
        self.assertEqual(len(parsed_diff.changes), num_changes)

    def assert_parsed_diff_change(self, parsed_diff_change, num_files=0, **attrs):
        ...
        Raises:
            AssertionError:
                One or more attributes do not match.
        """
        self._assert_parsed_diff_obj(parsed_diff_change, attrs=attrs, default_attrs={'commit_id': None, 'parent_commit_id': None})
    ...
    def assert_parsed_diff_file(self, parsed_diff_file, **attrs):
        ...
        self._assert_parsed_diff_obj(parsed_diff_file, attrs=attrs, default_attrs={'binary': False, 'copied': False, 'delete_count': 0, 'deleted': False, 'index_header_value': None, 'insert_count': 0, 'is_symlink': False, 'moved': False, 'new_symlink_target': None, 'new_unix_mode': None, 'old_symlink_target': None, 'old_unix_mode': None})
        self.assertEqual(parsed_diff_file.data.splitlines(), data.splitlines())
