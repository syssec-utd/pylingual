...
from phidata.infra.docker.resource.group import DockerResourceGroup, DockerBuildContext
from phidata.infra.docker.resource.utils import filter_and_flatten_docker_resource_groups, get_docker_resources_from_group
from phidata.infra.docker.exceptions import DockerArgsException, DockerApiClientException, DockerResourceCreationFailedException
from phidata.utils.common import is_empty
from phidata.utils.cli_console import print_info, print_info, print_heading, print_subheading, print_error, print_warning, confirm_yes_no
...
class DockerWorker:
    ...
    def build_docker_resource_groups(self, app_filter: Optional[str]=None) -> Optional[Dict[str, DockerResourceGroup]]:
        """
        ...
        if apps_to_build is not None and isinstance(apps_to_build, list):
            for app in apps_to_build:
                ...
                app.workspace_config_file_path = self.docker_args.workspace_config_file_path
                ...
                if app.aws_config_file is None and self.docker_args.aws_config_file is not None:
                    app.aws_config_file = self.docker_args.aws_config_file
                if app.aws_shared_credentials_file is None and self.docker_args.aws_shared_credentials_file is not None:
                    app.aws_shared_credentials_file = self.docker_args.aws_shared_credentials_file
                app_rgs: Optional[Dict[str, DockerResourceGroup]] = app.get_docker_resource_groups(docker_build_context=DockerBuildContext(network=self.docker_args.network))
        ...
        if self.docker_args.default_resources_available():
            ...
            num_rgs_to_build += 1
            default_docker_rg = DockerResourceGroup(images=self.docker_args.images, containers=self.docker_args.containers, volumes=self.docker_args.volumes)
            if self.docker_args.resources is None:
                self.docker_args.resources = []
            self.docker_args.resources.append(default_docker_rg)
        if self.docker_args.resources is not None and isinstance(self.docker_args.resources, list):
            for resource in self.docker_args.resources:
                ...
                if isinstance(resource, DockerResourceGroup):
                    ...
                    num_rgs_built += 1
        logger.debug(f'# DockerResourceGroups built: {num_rgs_built}/{num_rgs_to_build}')
        return docker_resource_groups

    def create_resources(self, name_filter: Optional[str]=None, type_filter: Optional[str]=None, app_filter: Optional[str]=None, auto_confirm: Optional[bool]=False) -> bool:
        logger.debug('-*- Creating DockerResources')
        docker_resource_groups: Optional[Dict[str, DockerResourceGroup]] = self.build_docker_resource_groups(app_filter=app_filter)
        if docker_resource_groups is None:
            print_info('No resources available')
            return True
        docker_resources_to_create: List[DockerResource] = filter_and_flatten_docker_resource_groups(docker_resource_groups=docker_resource_groups, name_filter=name_filter, type_filter=type_filter, app_filter=app_filter)
        if not auto_confirm:
        ...
        for resource in docker_resources_to_create:
            ...
            try:
                ...
                if _resource_created:
                    ...
                    logger.error(f'Resource {resource.get_resource_type()}: {resource.get_resource_name()} could not be created.')
        ...
        if num_resources_to_create == num_resources_created:
            return True
        logger.error(f'Resources created: {num_resources_created} do not match resources required: {num_resources_to_create}')
        return False

    def create_resources_dry_run(self, name_filter: Optional[str]=None, type_filter: Optional[str]=None, app_filter: Optional[str]=None, auto_confirm: Optional[bool]=False) -> None:
        logger.debug('-*- Creating DockerResources')
        docker_resource_groups: Optional[Dict[str, DockerResourceGroup]] = self.build_docker_resource_groups(app_filter=app_filter)
        if docker_resource_groups is None:
            print_info('No resources available')
    ...
    def get_resources(self, name_filter: Optional[str]=None, type_filter: Optional[str]=None, app_filter: Optional[str]=None) -> Optional[List[DockerResource]]:
        logger.debug('-*- Getting DockerResources')
        docker_resource_groups: Optional[Dict[str, DockerResourceGroup]] = self.build_docker_resource_groups(app_filter=app_filter)
        if docker_resource_groups is None:
            print_info('No resources available')
            return None
        docker_resources: List[DockerResource] = filter_and_flatten_docker_resource_groups(docker_resource_groups=docker_resource_groups, name_filter=name_filter, type_filter=type_filter, app_filter=app_filter)
        return docker_resources

    def delete_resources(self, name_filter: Optional[str]=None, type_filter: Optional[str]=None, app_filter: Optional[str]=None, auto_confirm: Optional[bool]=False) -> bool:
        logger.debug('-*- Deleting DockerResources')
        docker_resource_groups: Optional[Dict[str, DockerResourceGroup]] = self.build_docker_resource_groups(app_filter=app_filter)
        if docker_resource_groups is None:
            print_info('No resources available')
            return True
        docker_resources_to_delete: List[DockerResource] = filter_and_flatten_docker_resource_groups(docker_resource_groups=docker_resource_groups, name_filter=name_filter, type_filter=type_filter, app_filter=app_filter, sort_order='delete')
        if not auto_confirm:
            print_heading('--**-- Confirm resources:')
        ...
        for resource in docker_resources_to_delete:
            ...
            try:
                ...
                if _resource_deleted:
                    ...
                    logger.error(f'Resource {resource.get_resource_type()}: {resource.get_resource_name()} could not be deleted.')
        ...
        if num_resources_to_delete == num_resources_deleted:
            return True
        logger.error(f'Resources deleted: {num_resources_deleted} do not match resources required: {num_resources_to_delete}')
        return False

    def delete_resources_dry_run(self, name_filter: Optional[str]=None, type_filter: Optional[str]=None, app_filter: Optional[str]=None, auto_confirm: Optional[bool]=False) -> None:
        logger.debug('-*- Deleting DockerResources')
        docker_resource_groups: Optional[Dict[str, DockerResourceGroup]] = self.build_docker_resource_groups(app_filter=app_filter)
        if docker_resource_groups is None:
            print_info('No resources available')
    ...
    def patch_resources(self, name_filter: Optional[str]=None, type_filter: Optional[str]=None, app_filter: Optional[str]=None, auto_confirm: Optional[bool]=False) -> bool:
        logger.debug('-*- Patching DockerResources')
        docker_resource_groups: Optional[Dict[str, DockerResourceGroup]] = self.build_docker_resource_groups(app_filter=app_filter)
        if docker_resource_groups is None:
            print_info('No resources available')
            return True
        docker_resources_to_patch: List[DockerResource] = filter_and_flatten_docker_resource_groups(docker_resource_groups=docker_resource_groups, name_filter=name_filter, type_filter=type_filter, app_filter=app_filter)
        if not auto_confirm:
            print_heading('--**-- Confirm resources:')
        ...
        for resource in docker_resources_to_patch:
            ...
            try:
                ...
                if _resource_patched:
                    ...
                    logger.error(f'Resource {resource.get_resource_type()}: {resource.get_resource_name()} could not be patched.')
        ...
        if num_resources_to_patch == num_resources_patched:
            return True
        logger.error(f'Resources patched: {num_resources_patched} do not match resources required: {num_resources_to_patch}')
        return False

    def patch_resources_dry_run(self, name_filter: Optional[str]=None, type_filter: Optional[str]=None, app_filter: Optional[str]=None, auto_confirm: Optional[bool]=False) -> None:
        logger.debug('-*- Patching DockerResources')
        docker_resource_groups: Optional[Dict[str, DockerResourceGroup]] = self.build_docker_resource_groups(app_filter=app_filter)
        if docker_resource_groups is None:
            print_info('No resources available')
