...
def validate_img(notebook_frontend, cell_index, image_fmt, retina):
    ...
    commands = [f'b64data = {b64data}', 'data = base64.decodebytes(b64data)', f'display(Image(data, retina={retina}))']
    ...
    computed_width_js = "(element) => { return window.getComputedStyle(element).getPropertyValue('width') }"
    computed_height_js = "(element) => { return window.getComputedStyle(element).getPropertyValue('height') }"
    computed_width_raw = img_element.evaluate(computed_width_js)
    computed_height_raw = img_element.evaluate(computed_height_js)
    computed_width = re.search('[0-9]+', computed_width_raw)
    computed_height = re.search('[0-9]+', computed_height_raw)
