...
class Executor(object):
    ...
    def _convert_args(self, expr, args, kwargs):
        ...
        for (i, name) in enumerate(param_names):
            if i < num_of_args:
                if kwargs.get(name):
                    raise Exception('duplicate argument supplied in both positional args (at position: {0}), and keyword argument (with name: {1})'.format(i, name))
            else:
                cargs.append(kwargs[name])
        if len(cargs) != len(params):
            raise Exception('insufficient arguments, expected {0}, provided {1}'.format(len(cargs), len(params)))
        return tuple(cargs)
