...
class Independencies(object):
    ...
    def __eq__(self, other):
        if not isinstance(other, Independencies):
            return False
        return all((independency in other.get_assertions() for independency in self.get_assertions())) and all((independency in self.get_assertions() for independency in other.get_assertions()))
    ...
    def contains(self, assertion):
        ...
        if not isinstance(assertion, IndependenceAssertion):
            raise TypeError(f"' in <Independencies()>' requires IndependenceAssertion as left operand, not {type(assertion)}")
    ...
    def closure(self):
        ...
        def apply_left_and_right(func):
            ...
            def symmetric_func(*args):
                ...
                if len(args) == 2:
                    return func(*args) + func(args[0], sg0(args[1])) + func(sg0(args[0]), args[1]) + func(sg0(args[0]), sg0(args[1]))
        ...
        def sg2(ind):
            ...
            if single_var(ind.event2):
                ...
                return [IndependenceAssertion(ind.event1, ind.event2 - {elem}, {elem} | ind.event3) for elem in ind.event2]
        ...
        while new_inds:
            new_pairs = set(itertools.permutations(new_inds, 2)) | set(itertools.product(new_inds, all_independencies)) | set(itertools.product(all_independencies, new_inds))
            all_independencies |= new_inds
            new_inds = set(sum([sg1(ind) for ind in new_inds] + [sg2(ind) for ind in new_inds] + [sg3(*inds) for inds in new_pairs], []))
...
class IndependenceAssertion(object):
    ...
    def __str__(self):
        if self.event3:
            return '({event1} ⟂ {event2} | {event3})'.format(event1=', '.join([str(e) for e in self.event1]), event2=', '.join([str(e) for e in self.event2]), event3=', '.join([str(e) for e in self.event3]))
        else:
            return '({event1} ⟂ {event2})'.format(event1=', '.join([str(e) for e in self.event1]), event2=', '.join([str(e) for e in self.event2]))
    __repr__ = __str__

    def __eq__(self, other):
        if not isinstance(other, IndependenceAssertion):
            return False
        return (self.event1, self.event2, self.event3) == other.get_assertion() or (self.event2, self.event1, self.event3) == other.get_assertion()
    ...
    def latex_string(self):
        if len(self.event3) == 0:
            return '{event1} \\perp {event2}'.format(event1=', '.join([str(e) for e in self.event1]), event2=', '.join([str(e) for e in self.event2]))
        else:
            return '{event1} \\perp {event2} \\mid {event3}'.format(event1=', '.join([str(e) for e in self.event1]), event2=', '.join([str(e) for e in self.event2]), event3=', '.join([str(e) for e in self.event3]))