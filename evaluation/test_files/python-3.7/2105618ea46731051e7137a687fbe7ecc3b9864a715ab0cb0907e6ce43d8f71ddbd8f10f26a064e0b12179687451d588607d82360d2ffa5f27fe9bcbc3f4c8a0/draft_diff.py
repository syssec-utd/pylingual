...
from djblets.webapi.fields import BooleanFieldType, FileFieldType, StringFieldType
...
class DraftDiffResource(DiffResource):
    ...
    item_child_resources = [resources.draft_diffcommit, resources.draft_filediff]
    ...
    @webapi_response_errors(DOES_NOT_EXIST, INVALID_ATTRIBUTE, INVALID_FORM_DATA, NOT_LOGGED_IN, PERMISSION_DENIED)
    @webapi_request_fields(optional={'cumulative_diff': {'type': FileFieldType, 'description': 'The cumulative diff of the entire commit series this resource represents.'}, 'finalize_commit_series': {'type': BooleanFieldType, 'description': 'Whether or not this is a request to finalize the commit series represented by this resource.\n\nIf this is set to ``true``, then both the ``cumulative_diff`` and ``validation_info`` fields are required.'}, 'parent_diff': {'type': FileFieldType, 'description': 'The parent diff of the cumulative diff of the entire commit series this resource represents.'}, 'validation_info': {'type': StringFieldType, 'description': 'Validation information returned when validating the last commit in the series with the :ref:`DiffCommit validation resource <webapi2.0-validate-diff-commit-resource>`.'}}, allow_unknown=True)
    def update(self, request, finalize_commit_series=False, validation_info=None, extra_fields={}, *args, **kwargs):
        """Update a diff.
        ...
        if finalize_commit_series:
            if review_request.created_with_history:
                ...
                error_rsp = self._finalize_commit_series(request, diffset, cumulative_diff, parent_diff, validation_info)
                ...
                return (INVALID_ATTRIBUTE, {'reason': 'This review request was not created with commit history support.'})
            else:
                finalize_commit_series = False
        if extra_fields or finalize_commit_series:
            diffset.save(update_fields=('extra_data',))
        return (200, {self.item_result_key: diffset})

    def _finalize_commit_series(self, request, diffset, cumulative_diff, parent_diff, validation_info):
        ...
        if cumulative_diff is None:
            field_errors['cumulative_diff'] = ['This field is required when finalize_commit_series is set.']
        if validation_info is None or not validation_info.strip():
            field_errors['validation_info'] = ['This field is required when finalize_commit_series is set.']
        else:
            try:
                validation_info = deserialize_validation_info(validation_info)
            except (TypeError, ValueError) as e:
                field_errors['validation_info'] = ['Could not parse field: %s' % e]
        if diffset.is_commit_series_finalized:
            return (INVALID_ATTRIBUTE, {'reason': 'This diff is already finalized.'})
        if field_errors:
            return (INVALID_FORM_DATA, {'fields': field_errors})
        if parent_diff:
        ...
        try:
            diffset.finalize_commit_series(cumulative_diff=diff_file_contents, validation_info=validation_info, parent_diff=parent_diff_file_contents, request=request, save=False)
        except ValidationError as e:
            if e.code == 'invalid':
                return (INVALID_ATTRIBUTE, {'reason': e.message})
            elif e.code == 'validation_info':
                return (INVALID_FORM_DATA, {'fields': {'validation_info': [e.message]}})
