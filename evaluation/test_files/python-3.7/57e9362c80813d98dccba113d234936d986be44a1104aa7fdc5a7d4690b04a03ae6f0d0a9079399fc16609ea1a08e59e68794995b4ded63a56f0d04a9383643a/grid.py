...
class OccupancyGrid(Grid):
    ...
    def _update(self, step: int, occ_eval_fn: Callable, occ_thre: float=0.01, ema_decay: float=0.95, warmup_steps: int=256) -> None:
        ...
        self._binary = (self.occs > torch.clamp(self.occs.mean(), max=occ_thre)).reshape(self._binary.shape)
    ...
    def query_occ(self, samples: torch.Tensor) -> torch.Tensor:
        ...
        """
        return query_grid(samples, self._roi_aabb, self.occs.reshape(self.resolution.tolist()), self.contraction_type)

def _meshgrid3d(res: torch.Tensor, device: Union[torch.device, str]='cpu') -> torch.Tensor:
    """Create 3D grid coordinates."""
    assert len(res) == 3
    res = res.tolist()
    return torch.stack(torch.meshgrid([torch.arange(res[0]), torch.arange(res[1]), torch.arange(res[2])], indexing='ij'), dim=-1).long().to(device)