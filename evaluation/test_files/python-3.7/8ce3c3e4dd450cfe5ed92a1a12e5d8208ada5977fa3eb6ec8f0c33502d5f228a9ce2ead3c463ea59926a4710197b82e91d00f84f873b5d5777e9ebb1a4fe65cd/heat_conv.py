...
class HEATConv(MessagePassing):
    ...
    def __init__(self, in_channels: int, out_channels: int, num_node_types: int, num_edge_types: int, edge_type_emb_dim: int, edge_dim: int, edge_attr_emb_dim: int, heads: int=1, concat: bool=True, negative_slope: float=0.2, dropout: float=0.0, root_weight: bool=True, bias: bool=True, **kwargs):
        ...
        self.att = Linear(2 * out_channels + edge_type_emb_dim + edge_attr_emb_dim, self.heads, bias=False)
    ...
    def forward(self, x: Tensor, edge_index: Adj, node_type: Tensor, edge_type: Tensor, edge_attr: OptTensor=None) -> Tensor:
        """"""
        x = self.hetero_lin(x, node_type)
        edge_type_emb = F.leaky_relu(self.edge_type_emb(edge_type), self.negative_slope)
        out = self.propagate(edge_index, x=x, edge_type_emb=edge_type_emb, edge_attr=edge_attr, size=None)
        if self.concat:
            if self.root_weight:
    ...
    def message(self, x_i: Tensor, x_j: Tensor, edge_type_emb: Tensor, edge_attr: Tensor, index: Tensor, ptr: OptTensor, size_i: Optional[int]) -> Tensor:
