...
@pytest.mark.parametrize('method', [operator.methodcaller('sum'), lambda x: x.agg('sum'), lambda x: x.agg('mean'), lambda x: x.agg('median')])
...
@pytest.mark.parametrize('obj', [pd.Series([0, 0]), pd.DataFrame({'A': [0, 1], 'B': [1, 2]})])
@pytest.mark.parametrize('method', [lambda x: x.agg(['sum', 'count']), lambda x: x.transform(lambda y: y), lambda x: x.apply(lambda y: y), lambda x: x.agg('std'), lambda x: x.agg('var'), lambda x: x.agg('sem'), lambda x: x.agg('size'), lambda x: x.agg('ohlc'), lambda x: x.agg('describe')])
@not_implemented_mark
