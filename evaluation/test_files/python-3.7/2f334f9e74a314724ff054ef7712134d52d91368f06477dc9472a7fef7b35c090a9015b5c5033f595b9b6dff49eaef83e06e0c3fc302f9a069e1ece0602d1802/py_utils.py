  ...
  """
    ...
    for (k, v) in six.iteritems(dict2):
        if isinstance(v, dict):
            ...
            if k in merged_dict:
                raise ValueError('Dictionaries cannot contain the same key, unless the corresponding values are dictionaries.')
            merged_dict[k] = v
    return merged_dict
...
def flatten_with_joined_string_paths(structure, separator='/'):
    ...
    return [(separator.join(map(str, path)), item) for (path, item) in tree.flatten_with_path(structure)]
