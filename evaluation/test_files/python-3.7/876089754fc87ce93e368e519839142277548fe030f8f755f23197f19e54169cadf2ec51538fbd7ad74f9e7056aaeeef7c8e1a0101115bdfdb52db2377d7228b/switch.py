...
SWITCH_TYPES = [SwitchEntityDescription(key='allow_or_block', name='Allowed on network', icon='mdi:block-helper', entity_category=EntityCategory.CONFIG)]
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry, async_add_entities: AddEntitiesCallback) -> None:
    ...
    def new_device_callback() -> None:
        ...
        for (mac, device) in router.devices.items():
            if mac in tracked:
                continue
            new_entities.extend([NetgearAllowBlock(coordinator, router, device, entity_description) for entity_description in SWITCH_TYPES])
            tracked.add(mac)
...
class NetgearAllowBlock(NetgearDeviceEntity, SwitchEntity):
    ...
    def __init__(self, coordinator: DataUpdateCoordinator, router: NetgearRouter, device: dict, entity_description: SwitchEntityDescription) -> None:
...
class NetgearRouterSwitchEntity(NetgearRouterEntity, SwitchEntity):
    ...
    def __init__(self, router: NetgearRouter, entity_description: NetgearSwitchEntityDescription) -> None:
        """Initialize a Netgear device."""
