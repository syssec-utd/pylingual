...
from .models import Book, Car, CustomManager, CustomQuerySet, DeconstructibleCustomManager, FastCarAsBase, FastCarAsDefault, FunPerson, OneToOneRestrictedModel, Person, PersonFromAbstract, PersonManager, PublishedBookManager, RelatedModel, RestrictedModel
...
class CustomManagerTests(TestCase):
    ...
    def test_fk_related_manager(self):
        ...
        self.assertQuerysetEqual(self.b1.favorite_books.order_by('first_name').all(), ['Bugs', 'Droopy'], lambda c: c.first_name, ordered=False)
        self.assertQuerysetEqual(self.b1.fun_people_favorite_books.all(), ['Bugs'], lambda c: c.first_name, ordered=False)
        self.assertQuerysetEqual(self.b1.favorite_books(manager='boring_people').all(), ['Droopy'], lambda c: c.first_name, ordered=False)
        self.assertQuerysetEqual(self.b1.favorite_books(manager='fun_people').all(), ['Bugs'], lambda c: c.first_name, ordered=False)
    ...
    def test_gfk_related_manager(self):
        ...
        self.assertQuerysetEqual(self.b1.favorite_things.all(), ['Bugs', 'Droopy'], lambda c: c.first_name, ordered=False)
        self.assertQuerysetEqual(self.b1.fun_people_favorite_things.all(), ['Bugs'], lambda c: c.first_name, ordered=False)
        self.assertQuerysetEqual(self.b1.favorite_things(manager='boring_people').all(), ['Droopy'], lambda c: c.first_name, ordered=False)
        self.assertQuerysetEqual(self.b1.favorite_things(manager='fun_people').all(), ['Bugs'], lambda c: c.first_name, ordered=False)
    ...
    def test_m2m_related_manager(self):
        ...
        self.assertQuerysetEqual(self.b1.authors.order_by('first_name').all(), ['Bugs', 'Droopy'], lambda c: c.first_name, ordered=False)
        self.assertQuerysetEqual(self.b1.fun_authors.order_by('first_name').all(), ['Bugs'], lambda c: c.first_name, ordered=False)
        self.assertQuerysetEqual(self.b1.authors(manager='boring_people').all(), ['Droopy'], lambda c: c.first_name, ordered=False)
        self.assertQuerysetEqual(self.b1.authors(manager='fun_people').all(), ['Bugs'], lambda c: c.first_name, ordered=False)
    ...
    def test_removal_through_default_fk_related_manager(self, bulk=True):
        ...
        self.assertQuerysetEqual(FunPerson._base_manager.filter(favorite_book=self.b1), ['Bugs', 'Droopy'], lambda c: c.first_name, ordered=False)
        self.b1.fun_people_favorite_books.remove(bugs, bulk=bulk)
        self.assertQuerysetEqual(FunPerson._base_manager.filter(favorite_book=self.b1), ['Droopy'], lambda c: c.first_name, ordered=False)
    ...
    def test_removal_through_specified_fk_related_manager(self, bulk=True):
        ...
        self.assertQuerysetEqual(self.b1.favorite_books(manager='boring_people').all(), [], lambda c: c.first_name, ordered=False)
        droopy.favorite_book = self.b1
        droopy.save()
        self.b1.favorite_books(manager='fun_people').clear(bulk=bulk)
        self.assertQuerysetEqual(self.b1.favorite_books(manager='boring_people').all(), ['Droopy'], lambda c: c.first_name, ordered=False)
        self.assertQuerysetEqual(self.b1.favorite_books(manager='fun_people').all(), [], lambda c: c.first_name, ordered=False)
    ...
    def test_removal_through_default_gfk_related_manager(self, bulk=True):
        ...
        self.assertQuerysetEqual(FunPerson._base_manager.order_by('first_name').filter(favorite_thing_id=self.b1.pk), ['Bugs', 'Droopy'], lambda c: c.first_name, ordered=False)
        self.b1.fun_people_favorite_things.remove(bugs, bulk=bulk)
        self.assertQuerysetEqual(FunPerson._base_manager.order_by('first_name').filter(favorite_thing_id=self.b1.pk), ['Droopy'], lambda c: c.first_name, ordered=False)
        bugs.favorite_book = self.b1
        bugs.save()
    ...
    def test_removal_through_specified_gfk_related_manager(self, bulk=True):
        ...
        self.assertQuerysetEqual(self.b1.favorite_things(manager='boring_people').all(), [], lambda c: c.first_name, ordered=False)
        droopy.favorite_thing = self.b1
        droopy.save()
        self.b1.favorite_things(manager='fun_people').clear(bulk=bulk)
        self.assertQuerysetEqual(self.b1.favorite_things(manager='boring_people').all(), ['Droopy'], lambda c: c.first_name, ordered=False)
        self.assertQuerysetEqual(self.b1.favorite_things(manager='fun_people').all(), [], lambda c: c.first_name, ordered=False)
    ...
    def test_removal_through_default_m2m_related_manager(self):
        ...
        self.assertQuerysetEqual(self.b1.fun_authors.through._default_manager.all(), ['Bugs', 'Droopy'], lambda c: c.funperson.first_name, ordered=False)
        self.b1.fun_authors.remove(bugs)
        self.assertQuerysetEqual(self.b1.fun_authors.through._default_manager.all(), ['Droopy'], lambda c: c.funperson.first_name, ordered=False)
        self.b1.fun_authors.add(bugs)
    ...
    def test_removal_through_specified_m2m_related_manager(self):
        ...
        self.assertQuerysetEqual(self.b1.authors(manager='boring_people').all(), [], lambda c: c.first_name, ordered=False)
        self.b1.authors.add(droopy)
        self.b1.authors(manager='fun_people').clear()
        self.assertQuerysetEqual(self.b1.authors(manager='boring_people').all(), ['Droopy'], lambda c: c.first_name, ordered=False)
        self.assertQuerysetEqual(self.b1.authors(manager='fun_people').all(), [], lambda c: c.first_name, ordered=False)
...
class CustomManagersRegressTestCase(TestCase):
    ...
    def test_save_clears_annotations_from_base_manager(self):
        ...
        self.assertEqual(book.favorite_avg, 1)
