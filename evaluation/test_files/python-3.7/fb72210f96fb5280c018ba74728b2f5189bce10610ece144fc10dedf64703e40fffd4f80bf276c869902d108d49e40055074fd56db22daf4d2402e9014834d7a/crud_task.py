...
def list_by(db: Session, owner_id: int, page: int=0, size: int=100) -> List[Task]:
    return db.query(Task).filter(Task.created_by == owner_id, Task.deleted_at == None).order_by(Task.id.desc()).offset(offset=page * size).limit(limit=size).all()
...
def count(db: Session, owner_id: int) -> int:
    return db.query(Task).filter(Task.created_by == owner_id, Task.deleted_at == None).count()