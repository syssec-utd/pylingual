...
control_flow_ops = LazyLoader('control_flow_ops', globals(), 'tensorflow.python.ops.control_flow_ops')
...
def tensor_list_set_item(input_handle, index, item, resize_if_index_out_of_bounds=False, name=None):
    ...
    if resize_if_index_out_of_bounds:
        ...
        input_handle = control_flow_ops.cond(index >= input_list_size, lambda : gen_list_ops.tensor_list_resize(input_handle, index + 1), lambda : input_handle)
    output_handle = gen_list_ops.tensor_list_set_item(input_handle=input_handle, index=index, item=item, name=name)
    handle_data_util.copy_handle_data(input_handle, output_handle)
    return output_handle
