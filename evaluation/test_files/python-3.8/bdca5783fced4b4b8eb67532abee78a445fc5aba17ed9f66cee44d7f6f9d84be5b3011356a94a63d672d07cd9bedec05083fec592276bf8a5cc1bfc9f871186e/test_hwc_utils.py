...
class HwcUtilsTestCase(unittest.TestCase):
    ...
    def test_navigate_value(self):
        value = {'foo': {'quiet': {'tree': 'test', 'trees': [0, 1]}}}
        ...
        self.assertRaisesRegexp(HwcModuleException, '.* key\\(q\\) is not exist in dict', navigate_value, value, ['foo', 'q', 'tree'])
        self.assertRaisesRegexp(HwcModuleException, '.* the index is out of list', navigate_value, value, ['foo', 'quiet', 'trees'], {'foo.quiet.trees': 2})