...
class PptxParser(BaseParser):
    ...
    def _init_parser(self) -> Dict:
        ...
        try:
            ...
            raise ValueError('The package `python-pptx` is required to read Powerpoint files.')
        try:
            import torch
        except ImportError:
        ...
        try:
            ...
            raise ValueError('The package `transformers` is required to caption images.')
        try:
            from PIL import Image
        except ImportError:
            raise ValueError('PIL is required to read image files.Please run `pip install Pillow`')
        model = VisionEncoderDecoderModel.from_pretrained('nlpconnect/vit-gpt2-image-captioning')
        feature_extractor = ViTFeatureExtractor.from_pretrained('nlpconnect/vit-gpt2-image-captioning')
        tokenizer = AutoTokenizer.from_pretrained('nlpconnect/vit-gpt2-image-captioning')
        return {'feature_extractor': feature_extractor, 'model': model, 'tokenizer': tokenizer}
