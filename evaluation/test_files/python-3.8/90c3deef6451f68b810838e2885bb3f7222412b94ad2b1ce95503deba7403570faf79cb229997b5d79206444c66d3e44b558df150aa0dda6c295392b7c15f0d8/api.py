    ...
    def create_exception(self, source_error):
        ...
        if issubclass(preferred_type, errors_impl.OpError):
            ...
            if init_args == ('self', 'node_def', 'op', 'message', 'error_code'):
                return preferred_type(node_def=source_error.node_def, op=source_error.op, message=message, error_code=self.error_code)
            elif init_args == ('self', 'node_def', 'op', 'message'):
                if 'error_code' in init_argspec.kwonlyargs:
                    return preferred_type(node_def=source_error.node_def, op=source_error.op, message=message, errro_code=self.error_code)
                else:
                    return preferred_type(node_def=source_error.node_def, op=source_error.op, message=message)
        elif preferred_type in (AutoGraphError, ConversionError, StagingError, errors_impl.InaccessibleTensorError, errors_impl.OperatorNotAllowedInGraphError):
...
def _attach_metadata(e, f, converted):
    ...
    if metadata is None:
        logging.log(1, 'Caught error in %s (converted=%s)', f, converted, exc_info=True)
...
def converted_call(f, options, args, kwargs, caller_fn_scope=None):
  ...
  """
    ...
    if _is_known_loaded_type(f, 'wrapt', 'FunctionWrapper') or _is_known_loaded_type(f, 'wrapt', 'BoundFunctionWrapper'):
        logging.warn('Entity {} appears to be decorated by wrapt, which is not yet supported by AutoGraph. The function will be called without transformation. You may however apply AutoGraph before the decorator.'.format(f))
    ...
    try:
        ...
        if not tf_inspect.isclass(target_entity):
            if not hasattr(target_entity, '__code__'):
                ...
                logging.log(2, 'Permanently whitelisted: %s: dynamic code (exec?)', target_entity)
                return _call_unconverted(f, args, kwargs, options)
        program_ctx = converter.ProgramContext(options=options, autograph_module=tf_inspect.getmodule(converted_call))
        converted_f = conversion.convert(target_entity, program_ctx)
        if logging.has_verbosity(2):
            logging.log(2, 'Defaults of %s : %s', converted_f, converted_f.__defaults__)
            if six.PY3:
                logging.log(2, 'KW defaults of %s : %s', converted_f, converted_f.__kwdefaults__)
            if kwargs is not None:
        ...
        if is_autograph_strict_conversion_mode():
            raise
        logging.warn('Entity %s could not be transformed and will be executed as-is. Please report this to the AutoGraph team. When filing the bug, set the verbosity to 10 (on Linux, `export AUTOGRAPH_VERBOSITY=10`) and attach the full output. Cause: %s', target_entity, e)
        return _call_unconverted(f, args, kwargs, options)
...
def to_graph(entity, recursive=True, experimental_optional_features=None):
  ...
  """
    try:
        program_ctx = converter.ProgramContext(options=converter.ConversionOptions(recursive=recursive, user_requested=True, optional_features=experimental_optional_features), autograph_module=tf_inspect.getmodule(to_graph))
