...
CFN_RESOURCE_TYPE_IDENTIFIER = re.compile('^[a-zA-Z0-9]+::[a-zA-Z0-9]+::[a-zA-Z0-9]+$')
...
class CustomPoliciesIntegration(BaseIntegrationFeature):
    ...
    def is_valid(self) -> bool:
        return self.bc_integration.is_integration_configured() and (not self.bc_integration.skip_download) and (not self.integration_feature_failures)
    ...
    def _convert_raw_check(policy: dict[str, Any]) -> dict[str, Any]:
        metadata = {'id': policy['id'], 'name': policy['title'], 'category': policy['category'], 'frameworks': policy.get('frameworks', [])}
        check = {'metadata': metadata, 'definition': json.loads(policy['code'])}
        return check
