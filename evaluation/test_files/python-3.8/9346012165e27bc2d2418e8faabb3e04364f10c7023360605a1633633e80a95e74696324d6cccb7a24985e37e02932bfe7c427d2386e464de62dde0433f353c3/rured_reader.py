    ...
    def process_rured_file(self, data: List[Dict], num_neg_samples: str) -> Tuple[List, Dict]:
        ...
        for sample in data:
            ...
            if sample['obj_type'] in self.ner_stat:
                ...
                self.ner_stat[sample['obj_type']] = 1
            processed_samples.append(([sample['token'], [[(sample['subj_start'], sample['subj_end'])], [(sample['obj_start'], sample['obj_end'])]], [sample['subj_type'], sample['obj_type']]], self.label_to_one_hot(self.rel2id[sample['relation']])))
        if num_neg_samples == 'equal':
            neg_to_eliminate = random.sample(neg_samples, len(neg_samples) - pos_samples)
            processed_samples = [sample for (sample_idx, sample) in enumerate(processed_samples) if sample_idx not in neg_to_eliminate]
            ...
            processed_samples = [sample for (sample_idx, sample) in enumerate(processed_samples) if sample_idx not in neg_samples]
        else:
