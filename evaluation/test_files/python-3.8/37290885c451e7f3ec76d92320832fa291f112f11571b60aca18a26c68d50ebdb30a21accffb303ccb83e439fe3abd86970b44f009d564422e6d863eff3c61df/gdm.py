...
class GDM:
    ...
    def find_by_data(self, values):
        ...
        self.scan()
        return [entry for entry in self.entries if all((item in entry['data'].items() for item in values.items()))]
            ...
            while True:
                try:
                    ...
                    if '200 OK' in data.splitlines()[0]:
                        ddata = {k: v.strip() for (k, v) in (line.split(':') for line in data.splitlines() if ':' in line)}
