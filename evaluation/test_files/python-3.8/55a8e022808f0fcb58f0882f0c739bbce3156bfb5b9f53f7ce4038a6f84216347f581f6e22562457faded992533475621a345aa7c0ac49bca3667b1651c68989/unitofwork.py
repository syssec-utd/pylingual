...
def _warn_for_cascade_backrefs(state, prop):
    util.warn_deprecated_20('"%s" object is being merged into a Session along the backref cascade path for relationship "%s"; in SQLAlchemy 2.0, this reverse cascade will not take place.  Set cascade_backrefs to False in either the relationship() or backref() function for the 2.0 behavior; or to set globally for the whole Session, set the future=True flag' % (state.class_.__name__, prop), code='s9r1')
...
def track_cascade_events(descriptor, prop):
    ...
    def append(state, item, initiator):
        ...
        if sess:
            ...
            if prop._cascade.save_update and (prop.cascade_backrefs and (not sess.future) or key == initiator.key) and (not sess._contains_state(item_state)):
    ...
    def set_(state, newvalue, oldvalue, initiator):
        ...
        if sess:
            ...
            if newvalue is not None:
                ...
                if prop._cascade.save_update and (prop.cascade_backrefs and (not sess.future) or key == initiator.key) and (not sess._contains_state(newvalue_state)):
                    if key != initiator.key:
...
class UOWTransaction(object):
    ...
    def _generate_actions(self):
        ...
        if cycles:
            convert = dict(((rec, set(rec.per_state_flush_actions(self))) for rec in cycles))
