...
from sqlalchemy.sql import compiler, elements, COLLECT_CARTESIAN_PRODUCTS, WARN_LINTING, crud
...
class ClickHouseSQLCompiler(compiler.SQLCompiler):
    ...
    def visit_is_not_distinct_from_binary(self, binary, operator, **kw):
        ...
        """
        return 'hasAny([%s], [%s])' % (self.process(binary.left, **kw), self.process(binary.right, **kw))

    def visit_is_distinct_from_binary(self, binary, operator, **kw):
        return 'NOT %s' % self.visit_is_not_distinct_from_binary(binary, operator, **kw)

    def visit_empty_set_expr(self, element_types):
        return 'SELECT %s WHERE 1!=1' % (', '.join(('CAST(NULL AS %s)' % self.dialect.type_compiler.process(t if isinstance(t, types.Nullable) else types.Nullable(t)) for t in element_types or [types.Int8()])),)
    ...
    def visit_case(self, clause, **kwargs):
        ...
        for (cond, result) in clause.whens:
            text += 'WHEN ' + cond._compiler_dispatch(self, **kwargs) + ' THEN ' + result._compiler_dispatch(self, **kwargs) + ' '
        if clause.else_ is None:
            raise exc.CompileError('ELSE clause is required in CASE')
        text += 'ELSE ' + clause.else_._compiler_dispatch(self, **kwargs) + ' END'
        return text

    def visit_if__func(self, func, **kw):
        return '(%s) ? (%s) : (%s)' % (self.process(func.clauses.clauses[0], **kw), self.process(func.clauses.clauses[1], **kw), self.process(func.clauses.clauses[2], **kw))
    ...
    def visit_join(self, join, asfrom=False, **kwargs):
        ...
        if isinstance(onclause, elements.Tuple):
            text += ' USING ' + onclause._compiler_dispatch(self, include_table=False, **kwargs)
        else:
    ...
    def visit_array_join(self, array_join, **kwargs):
        ...
        return ' \nARRAY JOIN {columns}'.format(columns=', '.join((col._compiler_dispatch(self, within_label_clause=False, **kwargs) for col in array_join.clauses)))

    def visit_left_array_join(self, array_join, **kwargs):
        kwargs['within_columns_clause'] = True
        return ' \nLEFT ARRAY JOIN {columns}'.format(columns=', '.join((col._compiler_dispatch(self, within_label_clause=False, **kwargs) for col in array_join.clauses)))

    def visit_label(self, label, from_labeled_label=False, **kw):
        if from_labeled_label:
            return super(ClickHouseSQLCompiler, self).visit_label(label, render_label_as_label=label)
        else:
    ...
    def visit_delete(self, delete_stmt, **kw):
        if not self.dialect.supports_delete:
            raise exc.CompileError('ALTER DELETE is not supported by this server version')
        ...
        if delete_stmt._where_criteria:
            t = self._generate_delimited_and_list(delete_stmt._where_criteria, include_table=False, **kw)
            if t:
                text += ' WHERE ' + t
        else:
    ...
    def visit_update(self, update_stmt, **kw):
        if not self.dialect.supports_update:
            raise exc.CompileError('ALTER UPDATE is not supported by this server version')
        ...
        text = 'ALTER TABLE '
        table_text = self.update_tables_clause(update_stmt, update_stmt.table, render_extra_froms, **kw)
        ...
        if update_stmt._where_criteria:
            t = self._generate_delimited_and_list(update_stmt._where_criteria, include_table=False, **kw)
            if t:
                text += ' WHERE ' + t
        else:
            raise exc.CompileError('WHERE clause is required')
