...
class ExportModule(export_base.ExportModule):
    ...
    def __init__(self, params, model: tf.keras.Model, input_signature: Union[tf.TensorSpec, Dict[str, tf.TensorSpec]], preprocessor: Optional[Callable[..., Any]]=None, inference_step: Optional[Callable[..., Any]]=None, postprocessor: Optional[Callable[..., Any]]=None):
    ...
    """
        super().__init__(params, model=model, preprocessor=preprocessor, inference_step=inference_step, postprocessor=postprocessor)
    ...
    """
        ...
        for (_, def_name) in function_keys.items():
            signatures[def_name] = self.serve.get_concrete_function(self.input_signature)
        return signatures