...
class DistributedValues(ds_types.DistributedValues):
    ...
    def _get_cross_replica(self):
        raise NotImplementedError('DistributedValues._get_cross_replica should be implemented by sub-classes which support cross-replica accesses.')
...
tensor_conversion_registry.register_tensor_conversion_function(PerReplica, _per_replica_to_tensor)
...
nested_structure_coder.register_codec(nested_structure_coder.BuiltInTypeSpecCodec(PerReplicaSpec, struct_pb2.TypeSpecProto.PER_REPLICA_SPEC))
...
class DistributedVariable(DistributedDelegate, variables_lib.Variable, core.Tensor):
    ...
    def __init__(self, strategy, values, aggregation, var_policy=None):
        if aggregation == variables_lib.VariableAggregation.MEAN and (not values[0].dtype.is_floating):
            raise ValueError('creating distributed tf.Variable with aggregation=MEAN and a non-floating dtype is not supported, please use a different aggregation or dtype')
        ...
        for v in values:
            if isinstance(v, composite_tensor.CompositeTensor) and hasattr(v, 'handle'):
                ...
                v._distributed_container = weakref.ref(self)
        if ops.executing_eagerly_outside_functions() and getattr(strategy, '_enable_packed_variable_in_eager_mode', False):
            name = '%s/packed/' % self._common_name
            if hasattr(values[0], '_vars'):
                packed_var = packed.PackedDistributedVariable(sum((value._vars for value in values), []), name=name)
                for value in values:
    ...
    def _is_mirrored(self):
        return self._policy is not None and self._policy._is_mirrored()
    ...
    @property
    def _unique_id(self):
        return self._primary._unique_id
    ...
    def handle(self):
        ...
        replica_id = values_util.get_current_replica_id_as_int()
        if replica_id is None:
            raise ValueError('DistributedVariable.handle is not available outside the replica context or a `tf.distribute.Strategy.update()` call.')
        else:
            if self._use_packed_variable():
    ...
    def _get_save_slice_info(self):
        return self._primary._get_save_slice_info()

    def _set_save_slice_info(self, save_slice_info):
        for v in self._values:
            v._set_save_slice_info(save_slice_info)
    ...
    def op(self):
        ...
        if distribute_lib.in_cross_replica_context():
            return DistributedVarOp(self._primary.op.name, self._primary.op.graph, self._primary.op.traceback, self._primary.op.type)
        return self._get().op

    @property
    def _in_graph_mode(self):
        return self._primary._in_graph_mode
    ...
    def numpy(self):
        if context.executing_eagerly():
            ...
            raise NotImplementedError('DistributedVariable.numpy() is only available when eager execution is enabled.')
    ...
    def assign_sub(self, value, use_locking=False, name=None, read_value=True):
        ...
        return values_util.on_write_assign_sub(self, value, use_locking=use_locking, name=name, read_value=read_value)
    ...
    def assign_add(self, value, use_locking=False, name=None, read_value=True):
        ...
        return values_util.on_write_assign_add(self, value, use_locking=use_locking, name=name, read_value=read_value)
    ...
    def assign(self, value, use_locking=False, name=None, read_value=True):
        ...
        return values_util.on_write_assign(self, value, use_locking=use_locking, name=name, read_value=read_value)
    ...
    def scatter_sub(self, sparse_delta, use_locking=False, name=None):
        ...
        return values_util.scatter_sub(self, sparse_delta, use_locking=use_locking, name=name)
    ...
    def scatter_add(self, sparse_delta, use_locking=False, name=None):
        ...
        return values_util.scatter_add(self, sparse_delta, use_locking=use_locking, name=name)
    ...
    def scatter_mul(self, sparse_delta, use_locking=False, name=None):
        ...
        return values_util.scatter_mul(self, sparse_delta, use_locking=use_locking, name=name)
    ...
    def scatter_div(self, sparse_delta, use_locking=False, name=None):
        ...
        return values_util.scatter_div(self, sparse_delta, use_locking=use_locking, name=name)
    ...
    def scatter_min(self, sparse_delta, use_locking=False, name=None):
        ...
        return values_util.scatter_min(self, sparse_delta, use_locking=use_locking, name=name)
    ...
    def scatter_max(self, sparse_delta, use_locking=False, name=None):
        ...
        return values_util.scatter_max(self, sparse_delta, use_locking=use_locking, name=name)
    ...
    def scatter_update(self, sparse_delta, use_locking=False, name=None):
        ...
        return values_util.scatter_update(self, sparse_delta, use_locking=use_locking, name=name)
    ...
    def _as_graph_element(self):
        ...
        if self._policy:
            return self._policy._as_graph_element(self)
        raise NotImplementedError('DistributedVariable._as_graph_element requires a valid VariablePolicy. Please set the policy via the `var_policy` argument in the constructor, or override this method in sub-classes which support cross-replica accesses.')
    ...
    def _get_cross_replica(self):
        ...
        raise NotImplementedError('DistributedVariable._get_cross_replica requires a valid VariablePolicy. Please set the policy via the `var_policy` argument in the constructor, or override this method in sub-classes which support cross-replica accesses.')
    ...
    """
        ...
        raise NotImplementedError('DistributedVariable._update_replica requires a valid VariablePolicy. Please set the policy via the `var_policy` argument in the constructor, or override this method in sub-classes which support cross-replica accesses.')
    ...
    def __tf_tensor__(self, dtype: Optional[dtypes.DType]=None, name: Optional[str]=None) -> ops.Tensor:
    ...
    def _export_to_saved_model_graph(self, object_map=None, tensor_map=None, options=None, **kwargs):
        resource_list = self._primary._export_to_saved_model_graph(object_map=object_map, tensor_map=tensor_map, options=options, **kwargs)
        for v in [v for v in self._values if v != self._primary]:
            if options.experimental_variable_policy._expand_distributed_variables():
                resource_list.extend(v._export_to_saved_model_graph(object_map=object_map, tensor_map=tensor_map, options=options, **kwargs))
            else:
        ...
        if self._packed_var is not None:
            tensor_map[self._packed_var.packed_handle] = tensor_map[self._primary.handle]
    ...
    def __tf_experimental_restore_capture__(self, concrete_function, internal_capture):
        ...
        record.record_operation('captured_value', [internal_capture], [self], backward_function=lambda x: [x], forward_function=lambda x: [x])
        return self
...
tensor_conversion_registry.register_tensor_conversion_function(DistributedVariable, _tensor_conversion_distributed_var)

def _tensor_conversion_mirrored(var, dtype=None, name=None, as_ref=False):
    return var._dense_var_to_tensor(dtype=dtype, name=name, as_ref=as_ref)
tensor_conversion_registry.register_tensor_conversion_function(MirroredVariable, _tensor_conversion_mirrored)
...
tensor_conversion_registry.register_tensor_conversion_function(Mirrored, _tensor_conversion_mirrored_val)

def _tensor_conversion_sync_on_read(var, dtype=None, name=None, as_ref=False):
    return var._dense_var_to_tensor(dtype=dtype, name=name, as_ref=as_ref)
tensor_conversion_registry.register_tensor_conversion_function(SyncOnReadVariable, _tensor_conversion_sync_on_read)
...
class VariablePolicy(object):
  ...
  """
    ...
    def value(self):
        raise NotImplementedError('VariablePolicy.value should be overriden by sub-classes.')

    def _is_mirrored(self):
        raise NotImplementedError('VariablePolicy._is_mirrored should be overriden by sub-classes.')

    def _as_graph_element(self, _):
        raise NotImplementedError('VariablePolicy._as_graph_element should be overriden by sub-classes.')

    def _get_cross_replica(self, var):
        raise NotImplementedError('VariablePolicy._get_cross_replica should be overriden by sub-classes.')

    def _update_replica(self, var, update_fn, value, **kwargs):
        raise NotImplementedError('VariablePolicy._update_replica should be overriden by sub-classes.')
