...
class ADT(Object):
    ...
    def __init__(self, tag, fields):
        for f in fields:
            assert isinstance(f, ObjectTypes), 'Expect object or tvm NDArray type, but received : {0}'.format(type(f))
...
class ShapeTuple(Object):
    ...
    def __init__(self, shape):
        assert isinstance(shape, (list, tuple)), 'Expect list of tuple, but received : {0}'.format(type(shape))
        for x in shape:
