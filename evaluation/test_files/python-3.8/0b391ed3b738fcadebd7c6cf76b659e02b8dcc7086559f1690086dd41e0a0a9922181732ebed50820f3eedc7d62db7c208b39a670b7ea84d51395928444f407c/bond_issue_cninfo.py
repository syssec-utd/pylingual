...
def bond_treasure_issue_cninfo(start_date: str='20210910', end_date: str='20211109') -> pd.DataFrame:
    """
    ...
    headers = {'Accept': '*/*', 'Accept-Encoding': 'gzip, deflate', 'Accept-Language': 'zh-CN,zh;q=0.9,en;q=0.8', 'Cache-Control': 'no-cache', 'Content-Length': '0', 'Host': 'webapi.cninfo.com.cn', 'mcode': mcode, 'Origin': 'http://webapi.cninfo.com.cn', 'Pragma': 'no-cache', 'Proxy-Connection': 'keep-alive', 'Referer': 'http://webapi.cninfo.com.cn/', 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/93.0.4577.63 Safari/537.36', 'X-Requested-With': 'XMLHttpRequest'}
    params = {'sdate': '-'.join([start_date[:4], start_date[4:6], start_date[6:]]), 'edate': '-'.join([end_date[:4], end_date[4:6], end_date[6:]])}
    r = requests.post(url, headers=headers, params=params)
    data_json = r.json()
    temp_df = pd.DataFrame(data_json['records'])
    temp_df.rename(columns={'F009D': '缴款日', 'SECNAME': '债券简称', 'DECLAREDATE': '公告日期', 'F004D': '发行起始日', 'F003D': '发行终止日', 'F008N': '单位面值', 'SECCODE': '债券代码', 'F007N': '发行价格', 'F006N': '计划发行总量', 'F005N': '实际发行总量', 'F028N': '增发次数', 'BONDNAME': '债券名称', 'F014V': '发行对象', 'F002V': '交易市场', 'F013V': '发行方式'}, inplace=True)
    temp_df = temp_df[['债券代码', '债券简称', '发行起始日', '发行终止日', '计划发行总量', '实际发行总量', '发行价格', '单位面值', '缴款日', '增发次数', '交易市场', '发行方式', '发行对象', '公告日期', '债券名称']]
...
def bond_local_government_issue_cninfo(start_date: str='20210911', end_date: str='20211110') -> pd.DataFrame:
    """
...
def bond_corporate_issue_cninfo(start_date: str='20210911', end_date: str='20211110') -> pd.DataFrame:
    """
    ...
    data_json = r.json()
    temp_df = pd.DataFrame(data_json['records'])
    temp_df.rename(columns={'SECNAME': '债券简称', 'DECLAREDATE': '公告日期', 'F004D': '交易所网上发行终止日', 'F003D': '交易所网上发行起始日', 'F008N': '发行面值', 'SECCODE': '债券代码', 'F007N': '发行价格', 'F006N': '实际发行总量', 'F005N': '计划发行总量', 'F022N': '最小认购单位', 'F017V': '承销方式', 'F052N': '最低认购额', 'F015V': '发行范围', 'BONDNAME': '债券名称', 'F014V': '发行对象', 'F013V': '发行方式', 'F023V': '募资用途说明'}, inplace=True)
    temp_df = temp_df[['债券代码', '债券简称', '公告日期', '交易所网上发行起始日', '交易所网上发行终止日', '计划发行总量', '实际发行总量', '发行面值', '发行价格', '发行方式', '发行对象', '发行范围', '承销方式', '最小认购单位', '募资用途说明', '最低认购额', '债券名称']]
    temp_df['公告日期'] = pd.to_datetime(temp_df['公告日期']).dt.date
...
def bond_cov_issue_cninfo(start_date: str='20210913', end_date: str='20211112') -> pd.DataFrame:
    """
...
def bond_cov_stock_issue_cninfo() -> pd.DataFrame:
    ...
    data_json = r.json()
    temp_df = pd.DataFrame(data_json['records'])
    temp_df.rename(columns={'F003N': '转股价格', 'SECNAME': '债券简称', 'DECLAREDATE': '公告日期', 'F005D': '自愿转换期终止日', 'F004D': '自愿转换期起始日', 'F017V': '标的股票', 'BONDNAME': '债券名称', 'F002V': '转股简称', 'F001V': '转股代码', 'SECCODE': '债券代码'}, inplace=True)
    temp_df = temp_df[['债券代码', '债券简称', '公告日期', '转股代码', '转股简称', '转股价格', '自愿转换期起始日', '自愿转换期终止日', '标的股票', '债券名称']]
    temp_df['公告日期'] = pd.to_datetime(temp_df['公告日期']).dt.date
...
if __name__ == '__main__':
    bond_treasure_issue_cninfo_df = bond_treasure_issue_cninfo(start_date='20210910', end_date='20211109')
