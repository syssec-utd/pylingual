...
class RunnerRegistry:
    ...
    def __init__(self, banner: str, runner_filter: RunnerFilter, *runners: _BaseRunner) -> None:
        ...
        self.tool = tool_name
        self._check_type_to_report_map: dict[str, Report] = {}
        for runner in runners:
            if isinstance(runner, image_runner):
                runner.image_referencers = self.image_referencing_runners

    def run(self, root_folder: Optional[str]=None, external_checks_dir: Optional[List[str]]=None, files: Optional[List[str]]=None, collect_skip_comments: bool=True, repo_root_for_plan_enrichment: list[str | Path] | None=None) -> List[Report]:
        integration_feature_registry.run_pre_runner()
        if len(self.runners) == 1:
            reports: Iterable[Report] = [self.runners[0].run(root_folder, external_checks_dir=external_checks_dir, files=files, runner_filter=self.runner_filter, collect_skip_comments=collect_skip_comments)]
        else:

            def _parallel_run(runner: _BaseRunner) -> Report:
                return runner.run(root_folder=root_folder, external_checks_dir=external_checks_dir, files=files, runner_filter=self.runner_filter, collect_skip_comments=collect_skip_comments)
    ...
    def get_fail_thresholds(config: argparse.Namespace, report_type: str) -> _ExitCodeThresholds:
        ...
        return {'soft_fail': soft_fail, 'soft_fail_checks': soft_fail_on_checks, 'soft_fail_threshold': soft_fail_threshold, 'hard_fail_checks': hard_fail_on_checks, 'hard_fail_threshold': hard_fail_threshold}

    def print_reports(self, scan_reports: List[Report], config: argparse.Namespace, url: Optional[str]=None, created_baseline_path: Optional[str]=None, baseline: Optional[Baseline]=None) -> Literal[0, 1]:
        ...
        if 'cli' in config.output:
            ...
            for report in cli_reports:
                cli_output += report.print_console(is_quiet=config.quiet, is_compact=config.compact, created_baseline_path=created_baseline_path, baseline=baseline, use_bc_ids=config.output_bc_ids)
            print(cli_output)
            ansi_escape = re.compile('(?:\\x1B[@-_]|[\\x80-\\x9F])[0-?]*[ -/]*[@-~]')
        ...
        if 'sarif' in config.output:
            ...
            for report in sarif_reports:
                print(report.print_console(is_quiet=config.quiet, is_compact=config.compact, created_baseline_path=created_baseline_path, baseline=baseline, use_bc_ids=config.output_bc_ids))
        ...
        if 'junitxml' in config.output:
            ...
            if junit_reports:
                test_suites = [report.get_test_suite(properties=properties, use_bc_ids=config.output_bc_ids) for report in junit_reports]
            else:
        ...
        file_names = {'cli': 'results_cli.txt', 'github_failed_only': 'results_github_failed_only.txt', 'sarif': 'results_sarif.sarif', 'json': 'results_json.json', 'junitxml': 'results_junitxml.xml', 'cyclonedx': 'results_cyclonedx.xml'}
        if config.output_file_path:
            for output in config.output:
                if output in file_names:
                    self.save_output_to_file(file_name=f'{config.output_file_path}/{file_names[output]}', data=data_outputs[output], data_format=output)
        exit_code = 1 if 1 in exit_codes else 0
        return cast(Literal[0, 1], exit_code)

    def print_iac_bom_reports(self, output_path: str, scan_reports: list[Report], output_types: list[str], account_id: str) -> dict[str, str]:
        ...
        if 'cyclonedx' in output_types:
            ...
            cyclonedx = CycloneDX(reports=scan_reports, repo_id=metadata_integration.bc_integration.repo_id, export_iac_only=True)
            cyclonedx_output = cyclonedx.get_xml_output()
            self.save_output_to_file(file_name=os.path.join(output_path, cyclonedx_output_path), data=cyclonedx_output, data_format='cyclonedx')
    ...
    def get_enriched_resources(repo_roots: list[str | Path]) -> dict[str, dict[str, Any]]:
        ...
        for repo_root in repo_roots:
            ...
            Parser().parse_directory(directory=repo_root, out_definitions=tf_definitions, out_parsing_errors=parsing_errors)
                ...
                for (block_type, block_value) in definition.items():
                    if block_type in CHECK_BLOCK_TYPES:
                        for entity in block_value:
                            ...
                            entity_context = data_structures_utils.get_inner_dict(definitions_context[full_file_path], entity_context_path)
                            entity_lines_range = [entity_context.get('start_line'), entity_context.get('end_line')]
                            entity_code_lines = entity_context.get('code_lines')
                            skipped_checks = entity_context.get('skipped_checks')
                            enriched_resources[entity_id] = {'entity_code_lines': entity_code_lines, 'entity_lines_range': entity_lines_range, 'scanned_file': scanned_file, 'skipped_checks': skipped_checks}
