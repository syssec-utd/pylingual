    ...
    def get(self, requester_path: str, secret: str) -> str:
        ...
        while True:
            ...
            if secret in secrets:
                _LOGGER.debug('Secret %s retrieved from secrets.yaml in folder %s', secret, secret_dir)
        ...
        try:
            ...
            if 'logger' in secrets:
                ...
                if logger == 'debug':
                    ...
                    _LOGGER.error("Error in secrets.yaml: 'logger: debug' expected, but 'logger: %s' found", logger)
                del secrets['logger']
...
def _parse_yaml_pure_python(content: str | TextIO | StringIO, secrets: Secrets | None=None) -> JSON_TYPE:
...
def _parse_yaml(loader: type[SafeLoader] | type[SafeLineLoader], content: str | TextIO, secrets: Secrets | None=None) -> JSON_TYPE:
    """Load a YAML file."""
    return yaml.load(content, Loader=lambda stream: loader(stream, secrets)) or OrderedDict()
...
def _add_reference(obj, loader: LoaderType, node: yaml.nodes.Node):
...
def _include_yaml(loader: LoaderType, node: yaml.nodes.Node) -> JSON_TYPE:
    ...
    try:
        ...
        raise HomeAssistantError(f'{node.start_mark}: Unable to read file {fname}.') from exc
...
def _include_dir_list_yaml(loader: LoaderType, node: yaml.nodes.Node) -> list[JSON_TYPE]:
    ...
    loc = os.path.join(os.path.dirname(loader.get_name()), node.value)
    return [load_yaml(f, loader.secrets) for f in _find_files(loc, '*.yaml') if os.path.basename(f) != SECRET_YAML]
...
def _ordered_dict(loader: LoaderType, node: yaml.nodes.MappingNode) -> OrderedDict:
    ...
    for ((key, _), (child_node, _)) in zip(nodes, node.value):
        ...
        try:
            ...
            raise yaml.MarkedYAMLError(context=f'invalid key: "{key}"', context_mark=yaml.Mark(fname, 0, line, -1, None, None)) from exc
        if key in seen:
            fname = loader.get_stream_name()
            _LOGGER.warning('YAML file %s contains duplicate key "%s". Check lines %d and %d', fname, key, seen[key], line)
        seen[key] = line
