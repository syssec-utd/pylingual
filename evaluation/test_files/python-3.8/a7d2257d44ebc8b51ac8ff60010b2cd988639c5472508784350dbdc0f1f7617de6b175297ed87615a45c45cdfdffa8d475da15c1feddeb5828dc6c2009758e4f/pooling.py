...
def _extract_ethosu_pooling_params(attrs, args):
    ...
    return (ifm, lut, pooling_type, ifm_scale, ifm_zero_point, ofm_scale, ofm_zero_point, pool_shape, ofm_channels, ofm_dtype, strides, padding, activation, clip_min, clip_max, rounding_mode, upscale, ifm_layout, ofm_layout)
...
def pooling_strategy_ethosu(attrs, inputs, out_type, target):
    strategy = OpStrategy()
    strategy.add_implementation(create_ethosu_pooling_compute, _strategy.wrap_topi_schedule(schedule_injective), name='ethosu_pooling')
    return strategy

def ethosu_pooling(ifm: tvm.relay.Expr, lut: tvm.relay.Expr, pooling_type: str, ifm_scale: float, ifm_zero_point: int, ofm_scale: float, ofm_zero_point: int, pool_shape: Tuple[int, int], ofm_channels: int, ofm_dtype: str, strides: Tuple[int, int]=(1, 1), padding: Tuple[int, int, int, int]=(0, 0, 0, 0), activation: str='NONE', clip_min: int=0, clip_max: int=0, rounding_mode: str='TFL', upscale: str='NONE', ifm_layout: str='NHWC', ofm_layout: str='NHWC') -> tvm.relay.Call:
    ...
    """
    return _make.ethosu_pooling(ifm, lut, pooling_type, ifm_scale, ifm_zero_point, ofm_scale, ofm_zero_point, pool_shape, ofm_channels, ofm_dtype, strides, padding, activation, clip_min, clip_max, rounding_mode, upscale, ifm_layout, ofm_layout)