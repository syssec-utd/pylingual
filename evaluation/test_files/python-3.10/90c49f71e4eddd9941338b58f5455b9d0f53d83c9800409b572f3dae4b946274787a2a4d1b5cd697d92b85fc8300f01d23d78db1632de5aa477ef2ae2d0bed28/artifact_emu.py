...
class ArtifactEmulator:
    ...
    def create(self, variables):
        ...
        art_data = {'id': art_id, 'digest': 'abc123', 'state': state, 'labels': [], 'aliases': aliases, 'artifactSequence': art_seq, 'versionIndex': 0, 'currentManifest': dict(file=dict(directUrl=direct_url))}
    ...
    def create_files(self, variables):
        base_url = self._base_url
        response = {'data': {'createArtifactFiles': {'files': {'edges': [{'node': {'id': idx, 'name': af['name'], 'displayName': af['name'], 'uploadUrl': f"{base_url}/storage?file={af['name']}&id={af['artifactID']}", 'uploadHeaders': [], 'artifact': {'id': af['artifactID']}}} for (idx, af) in enumerate(variables['artifactFiles'])]}}}}
        return response
    ...
    def query(self, variables, query=None):
        ...
        is_public_api_query = query and (query.startswith(public_api_query_str) or query.startswith(public_api_query_str2) or query.startswith(public_api_query_str3))
        if art_name:
