...
class PassBase(ABC):
    ...
    def _check_conflict_including_common_rules(self, other_pass):
        return self._check_conflict(other_pass) and all([r(other_pass, self) for r in PassBase._COMMON_RULES])
    ...
    def apply(self, main_programs, startup_programs, context=None):
        ...
        if not self._check_self():
            return context
        if not all([self._check_conflict_including_common_rules(p) for p in context.passes]):
            return context
    ...
    def _apply_impl(self, main_programs, startup_programs, context):
        for (main_program, startup_program) in zip(main_programs, startup_programs):
...
class CPPPassWrapper(PassBase):
    ...
    def _apply_single_impl(self, main_program, startup_program, context):
        _apply_cpp_pass(main_program, startup_program, self.cpp_name, self._attrs, self.cpp_attr_types)

def _fusion_opt_last_rule(pass_before, pass_after):
    if pass_before._type() == PassType.FUSION_OPT and pass_after._type() != PassType.FUSION_OPT:
        return False
    else:
        return True

def _make_rule_from_white_lists_dict(before_white_lists_dict, after_white_lists_dict):
    ...
    return rule
PassBase._BEFORE_WHITE_LISTS_DICT = {'fuse_gradient_merge': ['fuse_all_reduce']}
PassBase._AFTER_WHITE_LISTS_DICT = {}
PassBase._COMMON_RULES = [_fusion_opt_last_rule, lambda pass_before, pass_after: type(pass_before) != type(pass_after), _make_rule_from_white_lists_dict(PassBase._BEFORE_WHITE_LISTS_DICT, PassBase._AFTER_WHITE_LISTS_DICT)]
...
def _solve_pass_conflict(passes, context):
    ...
    passes = []
    for p in old_passes:
        if all([p._check_conflict_including_common_rules(applied_p) for applied_p in context.passes]):
            passes.append(p)
    if not passes:
        return []
    n = len(passes)
    ...
    for i in range(n):
        for j in range(n):
            adjacent_matrix[i][j] = passes[j]._check_conflict_including_common_rules(passes[i])
