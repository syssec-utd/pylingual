...
def stock_board_concept_name_ths() -> pd.DataFrame:
    ...
    headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/89.0.4389.90 Safari/537.36', 'Cookie': f'v={v_code}'}
    ...
    r = requests.get(url, headers=headers)
    soup = BeautifulSoup(r.text, 'lxml')
    need_list = [item.find_all('a') for item in soup.find_all(attrs={'class': 'cate_group'})]
    temp_list = []
    for item in need_list:
        temp_list.extend(item)
    temp_df = pd.DataFrame([[item.text for item in temp_list], [item['href'] for item in temp_list]]).T
...
def stock_board_concept_cons_ths(symbol: str='阿里巴巴概念') -> pd.DataFrame:
    ...
    for page in tqdm(range(1, page_num + 1), leave=False):
        ...
        temp_df = pd.read_html(r.text)[0]
        big_df = pd.concat([big_df, temp_df], ignore_index=True)
    big_df.rename({'涨跌幅(%)': '涨跌幅', '涨速(%)': '涨速', '换手(%)': '换手', '振幅(%)': '振幅'}, inplace=True, axis=1)
    del big_df['加自选']
...
def stock_board_concept_hist_ths(start_year: str='2000', symbol: str='安防') -> pd.DataFrame:
    ...
    for year in tqdm(range(int(start_year), current_year + 1), leave=False):
        ...
        headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/89.0.4389.90 Safari/537.36', 'Referer': 'http://q.10jqka.com.cn', 'Host': 'd.10jqka.com.cn'}
        r = requests.get(url, headers=headers)
        data_text = r.text
        try:
    ...
    if big_df.columns.shape[0] == 12:
        big_df.columns = ['日期', '开盘价', '最高价', '最低价', '收盘价', '成交量', '成交额', '_', '_', '_', '_', '_']
    else:
        big_df.columns = ['日期', '开盘价', '最高价', '最低价', '收盘价', '成交量', '成交额', '_', '_', '_', '_']
    big_df = big_df[['日期', '开盘价', '最高价', '最低价', '收盘价', '成交量', '成交额']]
    big_df['日期'] = pd.to_datetime(big_df['日期']).dt.date
...
if __name__ == '__main__':
    ...
    stock_board_concept_hist_ths_df = stock_board_concept_hist_ths(start_year='2022', symbol='阿里巴巴概念')
