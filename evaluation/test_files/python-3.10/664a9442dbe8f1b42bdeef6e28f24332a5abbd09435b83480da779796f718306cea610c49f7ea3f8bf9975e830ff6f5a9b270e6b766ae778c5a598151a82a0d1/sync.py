...
from ..utils import flat_map, get_pip_version_for_python_executable, get_required_pip_specification, get_sys_path_for_python_executable
DEFAULT_REQUIREMENTS_FILE = 'requirements.txt'

@click.command(context_settings={'help_option_names': ('-h', '--help')})
@click.version_option(package_name='pip-tools')
@click.option('-a', '--ask', is_flag=True, help='Show what would happen, then ask whether to continue')
...
@click.option('-f', '--find-links', multiple=True, help='Look for archives in this directory or on this HTML page; may be used more than once')
@click.option('-i', '--index-url', help='Change index URL (defaults to PyPI)')
@click.option('--extra-index-url', multiple=True, help='Add another index URL to search; may be used more than once')
@click.option('--trusted-host', multiple=True, help='Mark this host as trusted, even though it does not have valid or any HTTPS; may be used more than once')
@click.option('--no-index', is_flag=True, help='Ignore package index (only looking at --find-links URLs instead)')
@click.option('--python-executable', help='Custom python executable path if targeting an environment other than current.')
...
def cli(ask: bool, dry_run: bool, force: bool, find_links: tuple[str, ...], index_url: str | None, extra_index_url: tuple[str, ...], trusted_host: tuple[str, ...], no_index: bool, python_executable: str | None, verbose: int, quiet: int, user_only: bool, cert: str | None, client_cert: str | None, src_files: tuple[str, ...], pip_args: str | None) -> None:
    ...
    installed_dists = _get_installed_distributions(user_only=user_only, local_only=python_executable is None, paths=paths)
    (to_install, to_uninstall) = sync.diff(merged_requirements, installed_dists)
    install_flags = _compose_install_flags(finder, no_index=no_index, index_url=index_url, extra_index_url=extra_index_url, trusted_host=trusted_host, find_links=find_links, user_only=user_only, cert=cert, client_cert=client_cert) + shlex.split(pip_args or '')
    sys.exit(sync.sync(to_install, to_uninstall, dry_run=dry_run, install_flags=install_flags, ask=ask, python_executable=python_executable))
...
def _compose_install_flags(finder: PackageFinder, no_index: bool, index_url: str | None, extra_index_url: tuple[str, ...], trusted_host: tuple[str, ...], find_links: tuple[str, ...], user_only: bool, cert: str | None, client_cert: str | None) -> list[str]:
    """
...
def _get_installed_distributions(local_only: bool=True, user_only: bool=False, paths: list[str] | None=None) -> list[Distribution]:
    """Return a list of installed Distribution objects."""
    env = get_environment(paths)
    dists = env.iter_installed_distributions(local_only=local_only, user_only=user_only, skip=[])
