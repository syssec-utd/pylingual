...
class Exchange(object):
    ...
    httpExceptions = {'422': ExchangeError, '418': DDoSProtection, '429': RateLimitExceeded, '404': ExchangeNotAvailable, '409': ExchangeNotAvailable, '410': ExchangeNotAvailable, '451': ExchangeNotAvailable, '500': ExchangeNotAvailable, '501': ExchangeNotAvailable, '502': ExchangeNotAvailable, '520': ExchangeNotAvailable, '521': ExchangeNotAvailable, '522': ExchangeNotAvailable, '525': ExchangeNotAvailable, '526': ExchangeNotAvailable, '400': ExchangeNotAvailable, '403': ExchangeNotAvailable, '405': ExchangeNotAvailable, '503': ExchangeNotAvailable, '530': ExchangeNotAvailable, '408': RequestTimeout, '504': RequestTimeout, '401': AuthenticationError, '407': AuthenticationError, '511': AuthenticationError}
    headers = None
    balance = None
    orderbooks = None
    orders = None
    ...
    def __init__(self, config={}):
        ...
        self.options = self.get_default_options() if self.options is None else self.options
        ...
        self.tokenBucket = self.extend({'refillRate': 1.0 / self.rateLimit if self.rateLimit > 0 else float('inf'), 'delay': 0.001, 'capacity': 1.0, 'defaultCost': 1.0}, getattr(self, 'tokenBucket', {}))
    ...
    def define_rest_api(self, api, method_name, paths=[]):
        for (key, value) in api.items():
            ...
            elif re.search('^(?:get|post|put|delete|head|patch)$', key, re.IGNORECASE) is not None:
    ...
    def fetch(self, url, method='GET', headers=None, body=None):
        ...
        json_response = None
        try:
            response = self.session.request(method, url, data=body, headers=request_headers, timeout=int(self.timeout / 1000), proxies=self.proxies, verify=self.verify and self.validateServerSsl)
    ...
    def urlencode_with_array_repeat(params={}):
        return re.sub('%5B\\d*%5D', '', Exchange.urlencode(params, True))
    ...
    def rsa(request, secret, alg='RS256'):
        algorithms = {'RS256': hashes.SHA256(), 'RS384': hashes.SHA384(), 'RS512': hashes.SHA512()}
    ...
    def ecdsa(request, secret, algorithm='p256', hash=None, fixed_length=False):
        algorithms = {'p192': [ecdsa.NIST192p, 'sha256'], 'p224': [ecdsa.NIST224p, 'sha256'], 'p256': [ecdsa.NIST256p, 'sha256'], 'p384': [ecdsa.NIST384p, 'sha384'], 'p521': [ecdsa.NIST521p, 'sha512'], 'secp256k1': [ecdsa.SECP256k1, 'sha256']}
        ...
        while fixed_length and (r_int > half_order or r_int <= minimum_size or s_int <= minimum_size):
            (r_binary, s_binary, v) = key.sign_digest_deterministic(digest, hashfunc=hash_function, sigencode=ecdsa.util.sigencode_strings_canonize, extra_entropy=Exchange.number_to_le(counter, 32))
            (r_int, s_int) = ecdsa.util.sigdecode_strings((r_binary, s_binary), key.privkey.order)
            counter += 1
    ...
    def precision_from_string(self, str):
        if 'e' in str:
            numStr = re.sub('\\de', '', str)
            return int(numStr) * -1
        parts = re.sub('0+$', '', str).split('.')
    ...
    def fetch_fees(self):
        ...
        return {'trading': trading, 'funding': funding}
    ...
    def build_ohlcvc(self, trades, timeframe='1m', since=None, limit=None):
        ...
        for i in range(0, oldest):
            ...
            if j == 0 or (opening_time and opening_time >= ohlcvs[candle][timestamp] + ms):
                ohlcvs.append([opening_time, trade['price'], trade['price'], trade['price'], trade['price'], trade['amount'], 1])
            else:
    ...
    def totp(key):
        ...
        otp = str(hex_to_dec(hmac_res[offset:offset + 8]) & 2147483647)
        return otp[-6:]
    ...
    def crc32(string, signed=False):
        ...
        if signed and unsigned >= 2147483648:
            return unsigned - 4294967296
        else:
            return unsigned

    def get_default_options(self):
        return {'defaultNetworkCodeReplacements': {'ETH': {'ERC20': 'ETH'}, 'TRX': {'TRC20': 'TRX'}, 'CRO': {'CRC20': 'CRONOS'}}}
    ...
    def safe_ledger_entry(self, entry, currency=None):
        ...
        return {'id': self.safe_string(entry, 'id'), 'timestamp': timestamp, 'datetime': self.iso8601(timestamp), 'direction': direction, 'account': self.safe_string(entry, 'account'), 'referenceId': self.safe_string(entry, 'referenceId'), 'referenceAccount': self.safe_string(entry, 'referenceAccount'), 'type': self.safe_string(entry, 'type'), 'currency': currency['code'], 'amount': self.parse_number(amount), 'before': self.parse_number(before), 'after': self.parse_number(after), 'status': self.safe_string(entry, 'status'), 'fee': fee, 'info': entry}
    ...
    def set_markets(self, markets, currencies=None):
        ...
        for i in range(0, len(marketValues)):
            ...
            market = self.deep_extend(self.safe_market(), {'precision': self.precision, 'limits': self.limits}, self.fees['trading'], value)
        ...
        if currencies is not None:
            ...
            for i in range(0, len(values)):
                ...
                if 'base' in market:
                    ...
                    currency = {'id': self.safe_string_2(market, 'baseId', 'base'), 'numericId': self.safe_string(market, 'baseNumericId'), 'code': self.safe_string(market, 'base'), 'precision': currencyPrecision}
                    baseCurrencies.append(currency)
                if 'quote' in market:
                    currencyPrecision = self.safe_value_2(marketPrecision, 'quote', 'price', defaultCurrencyPrecision)
                    currency = {'id': self.safe_string_2(market, 'quoteId', 'quote'), 'numericId': self.safe_string(market, 'quoteNumericId'), 'code': self.safe_string(market, 'quote'), 'precision': currencyPrecision}
    ...
    def safe_order(self, order, market=None):
        ...
        return self.extend(order, {'symbol': symbol, 'side': side, 'lastTradeTimestamp': lastTradeTimeTimestamp, 'price': self.parse_number(price), 'amount': self.parse_number(amount), 'cost': self.parse_number(cost), 'average': self.parse_number(average), 'filled': self.parse_number(filled), 'remaining': self.parse_number(remaining), 'timeInForce': timeInForce, 'postOnly': postOnly, 'trades': trades})
    ...
    def calculate_fee(self, symbol, type, side, amount, price, takerOrMaker='taker', params={}):
        ...
        return {'type': takerOrMaker, 'currency': market[key], 'rate': self.parse_number(rate), 'cost': self.parse_number(cost)}
        ...
        for i in range(0, len(fees)):
            ...
            if feeCurrencyCode is not None:
                ...
                if rateKey in reduced[feeCurrencyCode]:
                    ...
                    reduced[feeCurrencyCode][rateKey] = {'currency': feeCurrencyCode, 'cost': cost}
                    if rate is not None:
    ...
    def safe_ticker(self, ticker, market=None):
        ...
        return self.extend(ticker, {'bid': self.safe_number(ticker, 'bid'), 'bidVolume': self.safe_number(ticker, 'bidVolume'), 'ask': self.safe_number(ticker, 'ask'), 'askVolume': self.safe_number(ticker, 'askVolume'), 'high': self.safe_number(ticker, 'high'), 'low': self.safe_number(ticker, 'low'), 'open': self.parse_number(open), 'close': self.parse_number(close), 'last': self.parse_number(last), 'change': self.parse_number(change), 'percentage': self.parse_number(percentage), 'average': self.parse_number(average), 'vwap': self.parse_number(vwap), 'baseVolume': self.parse_number(baseVolume), 'quoteVolume': self.parse_number(quoteVolume), 'previousClose': self.safe_number(ticker, 'previousClose')})
    ...
    def fetch_ohlcv(self, symbol, timeframe='1m', since=None, limit=None, params={}):
        ...
        for i in range(0, len(ohlcvc)):
            result.append([self.safe_integer(ohlcvc[i], 0), self.safe_number(ohlcvc[i], 1), self.safe_number(ohlcvc[i], 2), self.safe_number(ohlcvc[i], 3), self.safe_number(ohlcvc[i], 4), self.safe_number(ohlcvc[i], 5)])
        return result
    ...
    def convert_trading_view_to_ohlcv(self, ohlcvs, timestamp='t', open='o', high='h', low='l', close='c', volume='v', ms=False):
        ...
        for i in range(0, len(timestamps)):
            result.append([self.safe_integer(timestamps, i) if ms else self.safe_timestamp(timestamps, i), self.safe_value(opens, i), self.safe_value(highs, i), self.safe_value(lows, i), self.safe_value(closes, i), self.safe_value(volumes, i)])
        return result
    ...
    def fetch_l2_order_book(self, symbol, limit=None, params={}):
        ...
        return self.extend(orderbook, {'asks': self.sort_by(self.aggregate(orderbook['asks']), 0), 'bids': self.sort_by(self.aggregate(orderbook['bids']), 0, True)})
    ...
    def parse_ohlcv(self, ohlcv, market=None):
        if isinstance(ohlcv, list):
            return [self.safe_integer(ohlcv, 0), self.safe_number(ohlcv, 1), self.safe_number(ohlcv, 2), self.safe_number(ohlcv, 3), self.safe_number(ohlcv, 4), self.safe_number(ohlcv, 5)]
        return ohlcv

    def get_network(self, network, code):
        network = network.upper()
        aliases = {'ETHEREUM': 'ETH', 'ETHER': 'ETH', 'ERC20': 'ETH', 'ETH': 'ETH', 'TRC20': 'TRX', 'TRON': 'TRX', 'TRX': 'TRX', 'BEP20': 'BSC', 'BSC': 'BSC', 'HRC20': 'HT', 'HECO': 'HT', 'SPL': 'SOL', 'SOL': 'SOL', 'TERRA': 'LUNA', 'LUNA': 'LUNA', 'POLYGON': 'MATIC', 'MATIC': 'MATIC', 'EOS': 'EOS', 'WAVES': 'WAVES', 'AVALANCHE': 'AVAX', 'AVAX': 'AVAX', 'QTUM': 'QTUM', 'CHZ': 'CHZ', 'NEO': 'NEO', 'ONT': 'ONT', 'RON': 'RON'}
        if network == code:
            return network
        ...
        if networkId is None:
            if currencyCode is None:
                ...
                if currencyCode in defaultNetworkCodeReplacements:
                    replacementObject = defaultNetworkCodeReplacements[currencyCode]
    ...
    def parse_order_book(self, orderbook, symbol, timestamp=None, bidsKey='bids', asksKey='asks', priceKey=0, amountKey=1):
        ...
        return {'symbol': symbol, 'bids': self.sort_by(bids, 0, True), 'asks': self.sort_by(asks, 0), 'timestamp': timestamp, 'datetime': self.iso8601(timestamp), 'nonce': None}
    ...
    def resolve_path(self, path, params):
        return [self.implode_params(path, params), self.omit(params, self.extract_params(path))]
    ...
    def fetch_status(self, params={}):
        if self.has['fetchTime']:
            ...
            self.status = self.extend(self.status, {'updated': time})
        return self.status
    ...
    def handle_option_and_params(self, params, methodName, optionName, defaultValue=None):
        defaultOptionName = 'default' + self.capitalize(optionName)
        ...
        if subTypeInParams is not None:
            ...
            if market is not None:
                if market['linear']:
                    ...
                    subType = 'inverse'
            if subType is None:
                values = self.handleOptionAndParams(None, methodName, 'subType', defaultValue)
                subType = values[0]
    ...
    def deposit_withdraw_fee(self, info):
        return {'info': info, 'withdraw': {'fee': None, 'percentage': None}, 'deposit': {'fee': None, 'percentage': None}, 'networks': {}}
