...
def launch_es(sleep=15, delete_existing=False):
    ...
    status = subprocess.run([f'docker run -d -p 9200:9200 -e "discovery.type=single-node" --name {ELASTICSEARCH_CONTAINER_NAME} elasticsearch:7.9.2'], shell=True)
...
def launch_opensearch(sleep=15, delete_existing=False):
    ...
    status = subprocess.run([f'docker run -d -p 9201:9200 -p 9600:9600 -e "discovery.type=single-node" --name {OPENSEARCH_CONTAINER_NAME} opensearchproject/opensearch:1.2.4'], shell=True)
    if status.returncode:
...
def launch_weaviate(sleep=15):
    ...
    status = subprocess.run(["docker run -d -p 8080:8080 --env AUTHENTICATION_ANONYMOUS_ACCESS_ENABLED='true' --env PERSISTENCE_DATA_PATH='/var/lib/weaviate' --name {WEAVIATE_CONTAINER_NAME} semitechnologies/weaviate:1.7.2"], shell=True)
    if status.returncode:
...
def stop_container(container_name, delete_container=False):
    ...
    if status.returncode:
        logger.warning(f'Tried to stop {container_name} but this failed. It is likely that there was no Docker container with the name {container_name}')
    if delete_container:
...
def launch_milvus(sleep=15, delete_existing=False):
    ...
    if status.returncode:
        logger.warning('Tried to start Milvus through Docker but this failed. It is likely that there is already an existing Milvus instance running. ')
    else:
        time.sleep(sleep)

def launch_milvus1(sleep=15):
    logger.debug('Starting Milvus ...')
    logger.warning('Automatic Milvus config creation not yet implemented. If you are starting Milvus using launch_milvus(), make sure you have a properly populated milvus/conf folder. See (https://milvus.io/docs/v1.0.0/milvus_docker-cpu.md) for more details.')
    status = subprocess.run([f'docker run -d --name {MILVUS1_CONTAINER_NAME}           -p 19530:19530           -p 19121:19121           milvusdb/milvus:1.1.0-cpu-d050721-5e559c'], shell=True)
    if status.returncode:
        logger.warning('Tried to start Milvus through Docker but this failed. It is likely that there is already an existing Milvus instance running. ')
    else:
        time.sleep(sleep)