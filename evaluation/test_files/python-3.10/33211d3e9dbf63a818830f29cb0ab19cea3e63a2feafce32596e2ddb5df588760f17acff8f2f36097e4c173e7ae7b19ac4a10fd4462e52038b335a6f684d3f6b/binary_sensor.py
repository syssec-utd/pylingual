...
async def async_setup_entry(hass: HomeAssistant, config_entry: ConfigEntry, async_add_entities: AddEntitiesCallback) -> None:
    ...
    if is_local(config_entry):
        ...
        async_add_entities((entity for (zone_id, zone) in local_data.system.zones.items() for entity in (RiscoLocalBinarySensor(local_data.system.id, zone_id, zone), RiscoLocalAlarmedBinarySensor(local_data.system.id, zone_id, zone), RiscoLocalArmedBinarySensor(local_data.system.id, zone_id, zone))))
    else:
        coordinator: RiscoDataUpdateCoordinator = hass.data[DOMAIN][config_entry.entry_id][DATA_COORDINATOR]
        async_add_entities((RiscoCloudBinarySensor(coordinator, zone_id, zone) for (zone_id, zone) in coordinator.data.zones.items()))
...
class RiscoCloudBinarySensor(RiscoCloudZoneEntity, BinarySensorEntity):
    ...
    def __init__(self, coordinator: RiscoDataUpdateCoordinator, zone_id: int, zone: Zone) -> None:
        """Init the zone."""
        super().__init__(coordinator=coordinator, name=None, suffix='', zone_id=zone_id, zone=zone)
...
class RiscoLocalBinarySensor(RiscoLocalZoneEntity, BinarySensorEntity):
    ...
    def __init__(self, system_id: str, zone_id: int, zone: Zone) -> None:
        """Init the zone."""
        super().__init__(system_id=system_id, name=None, suffix='', zone_id=zone_id, zone=zone)

    @property
    def extra_state_attributes(self) -> Mapping[str, Any] | None:
        """Return the state attributes."""
        return {**(super().extra_state_attributes or {}), 'groups': self._zone.groups}
...
class RiscoLocalAlarmedBinarySensor(RiscoLocalZoneEntity, BinarySensorEntity):
    ...
    def __init__(self, system_id: str, zone_id: int, zone: Zone) -> None:
        """Init the zone."""
        super().__init__(system_id=system_id, name='Alarmed', suffix='_alarmed', zone_id=zone_id, zone=zone)
...
class RiscoLocalArmedBinarySensor(RiscoLocalZoneEntity, BinarySensorEntity):
    ...
    def __init__(self, system_id: str, zone_id: int, zone: Zone) -> None:
        """Init the zone."""
        super().__init__(system_id=system_id, name='Armed', suffix='_armed', zone_id=zone_id, zone=zone)
