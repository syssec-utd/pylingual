    ...
    if _non_static_mode():
        if return_eids:
            if sorted_eids is None:
                raise ValueError(f'`sorted_eid` should not be None if return_eids is True.')
            (edge_src, edge_dst, sample_index, reindex_nodes, edge_eids) = _C_ops.graph_khop_sampler(row, sorted_eids, colptr, input_nodes, 'sample_sizes', sample_sizes, 'return_eids', True)
            return (edge_src, edge_dst, sample_index, reindex_nodes, edge_eids)
        else:
            (edge_src, edge_dst, sample_index, reindex_nodes, _) = _C_ops.graph_khop_sampler(row, None, colptr, input_nodes, 'sample_sizes', sample_sizes, 'return_eids', False)
