...
class MaxNorm(Constraint):
    ...
    def __call__(self, w):
        norms = backend.sqrt(tf.reduce_sum(tf.square(w), axis=self.axis, keepdims=True))
...
class UnitNorm(Constraint):
    ...
    @doc_controls.do_not_generate_docs
    def __call__(self, w):
        return w / (backend.epsilon() + backend.sqrt(tf.reduce_sum(tf.square(w), axis=self.axis, keepdims=True)))
...
@keras_export('keras.constraints.RadialConstraint', 'keras.constraints.radial_constraint')
class RadialConstraint(Constraint):
    ...
    def __call__(self, w):
        ...
        if w_shape.rank is None or w_shape.rank != 4:
            raise ValueError(f'The weight tensor must have rank 4. Received weight tensor with shape: {w_shape}')
        (height, width, channels, kernels) = w_shape
        w = backend.reshape(w, (height, width, channels * kernels))
        w = backend.map_fn(self._kernel_constraint, backend.stack(tf.unstack(w, axis=-1), axis=0))
        return backend.reshape(backend.stack(tf.unstack(w, axis=0), axis=-1), (height, width, channels, kernels))
    ...
    def _kernel_constraint(self, kernel):
        ...
        kernel_new = backend.switch(backend.cast(tf.math.floormod(kernel_shape, 2), 'bool'), lambda : kernel[start - 1:start, start - 1:start], lambda : kernel[start - 1:start, start - 1:start] + backend.zeros((2, 2), dtype=kernel.dtype))
        index = backend.switch(backend.cast(tf.math.floormod(kernel_shape, 2), 'bool'), lambda : backend.constant(0, dtype='int32'), lambda : backend.constant(1, dtype='int32'))
        while_condition = lambda index, *args: backend.less(index, start)

        def body_fn(i, array):
            return (i + 1, tf.pad(array, padding, constant_values=kernel[start + i, start + i]))
        (_, kernel_new) = tf.compat.v1.while_loop(while_condition, body_fn, [index, kernel_new], shape_invariants=[index.get_shape(), tf.TensorShape([None, None])])
        return kernel_new
max_norm = MaxNorm
non_neg = NonNeg
unit_norm = UnitNorm
...
def serialize(constraint, use_legacy_format=False):
    if not isinstance(constraint, Constraint):
        warnings.warn(f'The `keras.constraints.serialize()` API should only be used for objects of type `keras.constraints.Constraint`. Found an instance of type {type(constraint)}, which may lead to improper serialization.')
...
def deserialize(config, custom_objects=None, use_legacy_format=False):
    if use_legacy_format:
        return legacy_serialization.deserialize_keras_object(config, module_objects=globals(), custom_objects=custom_objects, printable_module_name='constraint')
    return deserialize_keras_object(config, module_objects=globals(), custom_objects=custom_objects, printable_module_name='constraint')
...
def get(identifier):
    ...
    if isinstance(identifier, dict):
        ...
        raise ValueError(f'Could not interpret constraint function identifier: {identifier}')