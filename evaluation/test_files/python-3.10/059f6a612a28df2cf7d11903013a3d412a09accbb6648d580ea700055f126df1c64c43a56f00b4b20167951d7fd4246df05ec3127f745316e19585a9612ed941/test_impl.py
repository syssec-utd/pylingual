...
test_data_categorical = {'ordered': pd.Categorical(list('testdata') * 30, ordered=True), 'unordered': pd.Categorical(list('testdata') * 30, ordered=False)}
(NCOLS, NROWS) = (100, 200)
...
datetime_data = _make_data(lambda : datetime(year=random.randint(1900, 2100), month=random.randint(1, 12), day=random.randint(1, 20)))
string_data = {'separator data': ['abC|DeF,Hik', '234,3245.67', 'gSaf,qWer|Gre', 'asd3,4sad|', np.NaN]}
...
def test_missing_from_masked():
    df = pd.DataFrame({'x': np.array([1, 2, 3, 4, 0]), 'y': np.array([1.5, 2.5, 3.5, 4.5, 0]), 'z': np.array([True, False, True, True, True])})
    df2 = df.__dataframe__()
