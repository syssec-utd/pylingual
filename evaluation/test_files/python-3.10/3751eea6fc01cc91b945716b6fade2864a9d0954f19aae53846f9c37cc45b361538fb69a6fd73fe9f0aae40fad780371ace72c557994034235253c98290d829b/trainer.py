...
class TrainerForAuto(Seq2SeqTrainer):
    ...
    def predict(self, test_dataset, ignore_keys=None, metric_key_prefix=None, max_length=None, num_beams=None):
        if getattr(self, '_is_seq2seq', None):
            return super().predict(test_dataset, ignore_keys, metric_key_prefix=metric_key_prefix, max_length=max_length, num_beams=num_beams)
        else:
    ...
    def evaluate(self, eval_dataset=None, ignore_keys=None, metric_key_prefix='eval'):
        ...
        if getattr(self, '_is_seq2seq', None):
            metrics = eval_dataset and super().evaluate(eval_dataset, ignore_keys, metric_key_prefix, max_length=self.args.generation_max_length, num_beams=self.args.generation_num_beams)
        else:
            metrics = eval_dataset and super(Seq2SeqTrainer, self).evaluate(eval_dataset, ignore_keys, metric_key_prefix)
