...
from localstack.aws.api.iam import ActionNameListType, ActionNameType, AttachedPermissionsBoundary, ContextEntryListType, CreateServiceLinkedRoleResponse, CreateUserResponse, DeleteServiceLinkedRoleResponse, DeletionTaskIdType, DeletionTaskStatusType, EvaluationResult, GetServiceLinkedRoleDeletionStatusResponse, GetUserResponse, IamApi, ListInstanceProfileTagsResponse, ListRolesResponse, NoSuchEntityException, PolicyEvaluationDecisionType, ResourceHandlingOptionType, ResourceNameListType, ResourceNameType, Role, SimulatePolicyResponse, SimulationPolicyListType, Tag, User, arnType, customSuffixType, existingUserNameType, groupNameType, instanceProfileNameType, markerType, maxItemsType, pathPrefixType, pathType, policyDocumentType, roleDescriptionType, roleNameType, tagKeyListType, tagListType, userNameType
...
ADDITIONAL_MANAGED_POLICIES = {'AWSLambdaExecute': {'Arn': 'arn:aws:iam::aws:policy/AWSLambdaExecute', 'Path': '/', 'CreateDate': '2017-10-20T17:23:10+00:00', 'DefaultVersionId': 'v4', 'Document': {'Version': '2012-10-17', 'Statement': [{'Effect': 'Allow', 'Action': ['logs:*'], 'Resource': 'arn:aws:logs:*:*:*'}, {'Effect': 'Allow', 'Action': ['s3:GetObject', 's3:PutObject'], 'Resource': 'arn:aws:s3:::*'}]}, 'UpdateDate': '2019-05-20T18:22:18+00:00'}}
...
class IamProvider(IamApi):
    ...
    @staticmethod
    def build_evaluation_result(action_name: ActionNameType, resource_name: ResourceNameType, policy_statements: List[Dict]) -> EvaluationResult:
        ...
        for statement in policy_statements:
            if action_name in statement['Action'] and resource_name in statement['Resource'] and (statement['Effect'] == 'Allow'):
                eval_res['EvalDecision'] = PolicyEvaluationDecisionType.allowed
                eval_res['MatchedStatements'] = []
        return eval_res
    ...
    def simulate_principal_policy(self, context: RequestContext, policy_source_arn: arnType, action_names: ActionNameListType, policy_input_list: SimulationPolicyListType=None, permissions_boundary_policy_input_list: SimulationPolicyListType=None, resource_arns: ResourceNameListType=None, resource_policy: policyDocumentType=None, resource_owner: ResourceNameType=None, caller_arn: ResourceNameType=None, context_entries: ContextEntryListType=None, resource_handling_option: ResourceHandlingOptionType=None, max_items: maxItemsType=None, marker: markerType=None) -> SimulatePolicyResponse:
        ...
        policy_version = moto_iam_backend.get_policy_version(policy_source_arn, policy.default_version_id)
        try:
            policy_statements = json.loads(policy_version.document).get('Statement', [])
        except Exception:
            raise NoSuchEntityException('Policy not found')
        evaluations = [self.build_evaluation_result(action_name, resource_arn, policy_statements) for action_name in action_names for resource_arn in resource_arns]
    ...
    def detach_role_policy(self, context: RequestContext, role_name: roleNameType, policy_arn: arnType) -> None:
        try:
    ...
    def list_roles(self, context: RequestContext, path_prefix: pathPrefixType=None, marker: markerType=None, max_items: maxItemsType=None) -> ListRolesResponse:
        ...
        for moto_role in moto_roles:
            ...
            if path_prefix:
                response_role['AssumeRolePolicyDocument'] = quote(json.dumps(moto_role.assume_role_policy_document or {}))
        return ListRolesResponse(Roles=response_roles, IsTruncated=False)

    def update_group(self, context: RequestContext, group_name: groupNameType, new_path: pathType=None, new_group_name: groupNameType=None) -> None:
    ...
    def list_instance_profile_tags(self, context: RequestContext, instance_profile_name: instanceProfileNameType, marker: markerType=None, max_items: maxItemsType=None) -> ListInstanceProfileTagsResponse:
    ...
    def tag_instance_profile(self, context: RequestContext, instance_profile_name: instanceProfileNameType, tags: tagListType) -> None:
        profile = moto_iam_backend.get_instance_profile(instance_profile_name)
        value_by_key = {tag['Key']: tag['Value'] for tag in tags}
        profile.tags.update(value_by_key)

    def untag_instance_profile(self, context: RequestContext, instance_profile_name: instanceProfileNameType, tag_keys: tagKeyListType) -> None:
        profile = moto_iam_backend.get_instance_profile(instance_profile_name)
        for tag in tag_keys:
            profile.tags.pop(tag, None)

    def create_service_linked_role(self, context: RequestContext, aws_service_name: groupNameType, description: roleDescriptionType=None, custom_suffix: customSuffixType=None) -> CreateServiceLinkedRoleResponse:
        policy_doc = json.dumps({'Version': '2012-10-17', 'Statement': [{'Effect': 'Allow', 'Principal': {'Service': aws_service_name}, 'Action': 'sts:AssumeRole'}]})
        path = f'{SERVICE_LINKED_ROLE_PATH_PREFIX}/{aws_service_name}'
        role_name = f'r-{short_uid()}'
        role = moto_iam_backend.create_role(role_name=role_name, assume_role_policy_document=policy_doc, path=path, permissions_boundary='', description=description, tags={}, max_session_duration=3600)
        role.service_linked_role_arn = 'arn:aws:iam::{0}:role/aws-service-role/{1}/{2}'.format(get_aws_account_id(), aws_service_name, role.name)
        res_role = self.moto_role_to_role_type(role)
        return CreateServiceLinkedRoleResponse(Role=res_role)

    def delete_service_linked_role(self, context: RequestContext, role_name: roleNameType) -> DeleteServiceLinkedRoleResponse:
        moto_iam_backend.delete_role(role_name)
        return DeleteServiceLinkedRoleResponse(DeletionTaskId=short_uid())

    def get_service_linked_role_deletion_status(self, context: RequestContext, deletion_task_id: DeletionTaskIdType) -> GetServiceLinkedRoleDeletionStatusResponse:
        return GetServiceLinkedRoleDeletionStatusResponse(Status=DeletionTaskStatusType.SUCCEEDED)

    def put_user_permissions_boundary(self, context: RequestContext, user_name: userNameType, permissions_boundary: arnType) -> None:
        if (user := moto_iam_backend.users.get(user_name)):
            user.permissions_boundary = permissions_boundary
        else:
            raise NoSuchEntityException()

    def delete_user_permissions_boundary(self, context: RequestContext, user_name: userNameType) -> None:
        if (user := moto_iam_backend.users.get(user_name)):
    ...
    def create_user(self, context: RequestContext, user_name: userNameType, path: pathType=None, permissions_boundary: arnType=None, tags: tagListType=None) -> CreateUserResponse:
        response = call_moto(context=context)
        user = moto_iam_backend.get_user(user_name)
        if permissions_boundary:
            user.permissions_boundary = permissions_boundary
            response['User']['PermissionsBoundary'] = AttachedPermissionsBoundary(PermissionsBoundaryArn=permissions_boundary, PermissionsBoundaryType='Policy')
        return response

    def get_user(self, context: RequestContext, user_name: existingUserNameType=None) -> GetUserResponse:
        response = call_moto(context=context)
        ...
        if not moto_user and (not user_name):
            ...
            sts_client = aws_stack.connect_to_service('sts', aws_access_key_id=access_key_id, aws_secret_access_key=INTERNAL_AWS_SECRET_ACCESS_KEY)
            caller_identity = sts_client.get_caller_identity()
            caller_arn = caller_identity['Arn']
            if caller_arn.endswith(':root'):
                return GetUserResponse(User=User(UserId=context.account_id, Arn=caller_arn, CreateDate=datetime.now(), PasswordLastUsed=datetime.now()))
            else:
                raise CommonServiceException('ValidationError', 'Must specify userName when calling with non-User credentials')
        if hasattr(moto_user, 'permissions_boundary') and moto_user.permissions_boundary:
            response['User']['PermissionsBoundary'] = AttachedPermissionsBoundary(PermissionsBoundaryArn=moto_user.permissions_boundary, PermissionsBoundaryType='Policy')
        return response
...
def apply_patches():
    ...
    def iam_response_get_user(fn, self):
        ...
        if tags and '<Tags>' not in result:
            tags_str = ''.join(['<member><Key>%s</Key><Value>%s</Value></member>' % (t['Key'], t['Value']) for t in tags['Tags']])
            result = result.replace('</Arn>', '</Arn><Tags>%s</Tags>' % tags_str)
        return result

    @patch(Policy.__init__)
    def policy__init__(fn, self, name, default_version_id=None, description=None, document=None, **kwargs):
