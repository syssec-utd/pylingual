...
class RangedData(metaclass=ABCMeta):
    ...
    def __check_expire(self):
        if isinstance(self.__expire, (int, float)):
            if self.__expire <= 0:
                raise ValueError('Expire should be greater than 0, but {actual} found.'.format(actual=repr(self.__expire)))
        else:
            raise TypeError('Expire should be int or float, but {actual} found.'.format(actual=type(self.__expire).__name__))
    ...
    def __check_time(self, time_: float):
        if self.__queue:
            ...
            if time_ < _time:
                raise ValueError('Time {time} invalid for descending from last time {last_time}'.format(time=repr(time_), last_time=repr(_time)))
