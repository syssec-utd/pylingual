...
def get_fastapi_app(args: 'argparse.Namespace', topology_graph: 'TopologyGraph', connection_pool: 'GrpcConnectionPool', logger: 'JinaLogger', metrics_registry: Optional['CollectorRegistry']=None):
    """
    ...
    with ImportExtensions(required=True):
        ...
        from jina.serve.runtimes.gateway.http.models import JinaEndpointRequestModel, JinaRequestModel, JinaResponseModel
    app = FastAPI(title=args.title or 'My Jina Service', description=args.description or 'This is my awesome service. You can set `title` and `description` in your `Flow` or `Gateway` to customize the title and description.', version=__version__)
    if args.cors:
    ...
    streamer = RequestStreamer(args=args, request_handler=request_handler.handle_request(graph=topology_graph, connection_pool=connection_pool), result_handler=request_handler.handle_result())
    ...
    if not args.no_debug_endpoints:
        openapi_tags.append({'name': 'Debug', 'description': 'Debugging interface. In production, you should hide them by setting `--no-debug-endpoints` in `Flow`/`Gateway`.'})
        from jina.serve.runtimes.gateway.http.models import JinaHealthModel

        @app.get(path='/', summary='Get the health of Jina Gateway service', response_model=JinaHealthModel)
        ...
        from jina.serve.runtimes.gateway.http.models import PROTO_TO_PYDANTIC_MODELS, JinaInfoModel
        from jina.types.request.status import StatusMessage

        @app.get(path='/dry_run', summary='Get the readiness of Jina Flow service, sends an empty DocumentArray to the complete Flow to validate connectivity', response_model=PROTO_TO_PYDANTIC_MODELS.StatusProto)
        async def _flow_health():
            ...
            da = DocumentArray()
            try:
                _ = await _get_singleton_result(request_generator(exec_endpoint=__dry_run_endpoint__, data=da, data_type=DataInputType.DOCUMENT))
        ...
        @app.get(path='/status', summary='Get the status of Jina service', response_model=JinaInfoModel, tags=['Debug'])
        async def _status():
            """
        ...
        @app.post(path='/post', summary='Post a data request to some endpoint', response_model=JinaResponseModel, tags=['Debug'])
        async def post(body: JinaEndpointRequestModel, response: Response):
            ...
            try:
                ...
                result = bd
                result['header'] = _generate_exception_header(err)
    ...
    def _generate_exception_header(error: InternalNetworkError):
        ...
        exception_dict = {'name': str(error.__class__), 'stacks': [str(x) for x in traceback.extract_tb(error.og_exception.__traceback__)], 'executor': ''}
        status_dict = {'code': DataRequest().status.ERROR, 'description': error.details() if error.details() else '', 'exception': exception_dict}
    ...
    def expose_executor_endpoint(exec_endpoint, http_path=None, **kwargs):
        ...
        kwargs['response_model'] = kwargs.get('response_model', JinaResponseModel)
    ...
    if not args.no_crud_endpoints:
        openapi_tags.append({'name': 'CRUD', 'description': 'CRUD interface. If your service does not implement those interfaces, you can should hide them by setting `--no-crud-endpoints` in `Flow`/`Gateway`.'})
        crud = {'/index': {'methods': ['POST']}, '/search': {'methods': ['POST']}, '/delete': {'methods': ['DELETE']}, '/update': {'methods': ['PUT']}}
    ...
    if args.expose_endpoints:
        endpoints = json.loads(args.expose_endpoints)
    ...
    if args.expose_graphql_endpoint:
        with ImportExtensions(required=True):
            ...
            async def get_docs_from_endpoint(data, target_executor, parameters, exec_endpoint):
                req_generator_input = {'data': [asdict(d) for d in data], 'target_executor': target_executor, 'parameters': parameters, 'exec_endpoint': exec_endpoint, 'data_type': DataInputType.DICT}
            ...
            class Mutation:
                ...
                @strawberry.mutation
                async def docs(self, data: Optional[List[StrawberryDocumentInput]]=None, target_executor: Optional[str]=None, parameters: Optional[JSONScalar]=None, exec_endpoint: str='/search') -> List[StrawberryDocument]:
