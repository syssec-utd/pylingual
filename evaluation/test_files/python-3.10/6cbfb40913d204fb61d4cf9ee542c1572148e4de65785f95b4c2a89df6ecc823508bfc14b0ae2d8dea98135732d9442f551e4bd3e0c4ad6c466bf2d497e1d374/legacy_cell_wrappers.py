...
class DropoutWrapper(_RNNCellWrapperV1):
    ...
    def __init__(self, cell, input_keep_prob=1.0, output_keep_prob=1.0, state_keep_prob=1.0, variational_recurrent=False, input_size=None, dtype=None, seed=None, dropout_state_filter_visitor=None, **kwargs):
        ...
        if dropout_state_filter_visitor is not None and (not callable(dropout_state_filter_visitor)):
            raise TypeError(f'dropout_state_filter_visitor must be callable. Received: {dropout_state_filter_visitor}')
            ...
            for (prob, attr) in [(input_keep_prob, 'input_keep_prob'), (state_keep_prob, 'state_keep_prob'), (output_keep_prob, 'output_keep_prob')]:
                ...
                if const_prob is not None:
                    if const_prob < 0 or const_prob > 1:
                        raise ValueError(f'Parameter {attr} must be between 0 and 1. Received {const_prob}')
        ...
        if not self._variational_recurrent:
            ...
            def dropout(i, do_dropout, v):
                if not isinstance(do_dropout, bool) or do_dropout:
                    return tf.nn.dropout(v, rate=1.0 - keep_prob, seed=self._gen_seed(salt_prefix, i))
                else:
                    return v
    ...
    def _call_wrapped_cell(self, inputs, state, cell_call_fn, **kwargs):
        ...
        if _should_dropout(self._state_keep_prob):
            shallow_filtered_substructure = tf.__internal__.nest.get_traverse_shallow_structure(self._dropout_state_filter, new_state)
    ...
    def from_config(cls, config, custom_objects=None):
        ...
        return super(DropoutWrapper, cls).from_config(config, custom_objects=custom_objects)
...
class ResidualWrapper(_RNNCellWrapperV1):
    ...
    def from_config(cls, config, custom_objects=None):
        ...
        return super(ResidualWrapper, cls).from_config(config, custom_objects=custom_objects)
