    ...
    def __call__(self, *args, **kwargs):
        ...
        for input_parameter in self.__op_proto__.inputs:
            ...
            if not input_parameter.duplicable and len(input_arguments) > 1:
                raise ValueError('Input %s expects only one input, but %d are given.' % (input_parameter.name, len(input_arguments)))
        ...
        for output_parameter in self.__op_proto__.outputs:
            ...
            if not output_parameter.duplicable and len(output_arguments) > 1:
                raise ValueError('Output %s expects only one output, but %d are given.' % (output_parameter.name, len(output_arguments)))
        ...
        for attr in self.__op_proto__.attrs:
            ...
            if user_defined_attr is not None:
                ...
                if attr.type == framework_pb2.INT:
                    ...
                    raise NotImplementedError('A not supported attribute type: %s.' % str(attr.type))
        return op_desc
...
def create_op_creation_method(op_proto):
    ...
    return OpInfo(method=__impl__, name=op_proto.type, inputs=[(var.name, var.duplicable) for var in op_proto.inputs], outputs=[(var.name, var.duplicable) for var in op_proto.outputs], attrs=[attr.name for attr in op_proto.attrs])
...
class OperatorFactory(object):
    ...
    def __call__(self, *args, **kwargs):
        if 'type' in kwargs:
            if len(args) != 0:
                raise ValueError('Except the argument "type",all of the other arguments should be keyword arguments.')
            t = kwargs.pop('type')
        else:
            if len(args) != 1:
                raise ValueError('Except the argument "type",all of the other arguments should be keyword arguments.')
