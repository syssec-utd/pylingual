...
class TileEncoder(DeepLakeMemoryObject):
    ...
    def get_tile_layout_shape(self, global_sample_index: int) -> Tuple[int, ...]:
        ...
        if len(tile_shape) != len(sample_shape):
            raise ValueError('Tile shape and sample shape must have the same number of dimensions.')
        layout = [np.ceil(sample_shape_dim / tile_shape_dim) for (tile_shape_dim, sample_shape_dim) in zip(tile_shape, sample_shape)]
