...
ALIAS_CASES = [{'service': 'ec2', 'operation': 'describe_flow_logs', 'original_name': 'Filter', 'new_name': 'Filters', 'parameter_value': [{'Name': 'traffic-type', 'Values': ['ACCEPT']}]}, {'service': 'cloudsearchdomain', 'operation': 'search', 'original_name': 'return', 'new_name': 'returnFields', 'parameter_value': '_all_fields', 'extra_args': {'query': 'foo'}}, {'service': 'logs', 'operation': 'create_export_task', 'original_name': 'from', 'new_name': 'fromTime', 'parameter_value': 0, 'extra_args': {'logGroupName': 'name', 'to': 10, 'destination': 'mybucket'}}]
...
def _can_use_parameter_in_client_call(session, case, use_alias=True):
    client = session.create_client(case['service'], region_name='us-east-1', aws_access_key_id='foo', aws_secret_access_key='bar')
    ...
    try:
        ...
        raise AssertionError('Expecting %s to be valid parameter for %s.%s but received %s.' % (case['new_name'], case['service'], case['operation'], e))