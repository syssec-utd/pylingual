...
class HydroNet(InMemoryDataset):
    ...
    def __init__(self, root: str, name: Optional[str]=None, transform: Optional[Callable]=None, pre_transform: Optional[Callable]=None, num_workers: int=8, clusters: Optional[Union[int, List[int]]]=None, use_processed: bool=True):
        self.name = name
    ...
    def process(self):
        ...
        self._partitions = process_map(self._create_partitions, self.raw_paths, max_workers=self.num_workers, position=0, leave=True)
    ...
    def select_clusters(self, clusters: Optional[Union[int, List[int]]]) -> 'Optional[List[Partition]]':
        ...
        if not all([is_valid_cluster(x) for x in clusters]):
            raise ValueError('Selected clusters must be an integer in the range [3, 30]')
    ...
    def _validate_name(self, clusters) -> List[int]:
        ...
        if self.name not in ['small', 'medium']:
            raise ValueError(f"Invalid subset name '{self.name}'. Must be either 'small' or 'medium'")
        return range(3, 26)
...
def read_energy(file: str, chunk_size: int) -> np.ndarray:
    ...
    if chunk_size - 2 == 11 * 3:
        ...
        df = pd.read_table(file, sep='\\s+', names=['label', 'energy'], dtype=np.float32, skiprows=skipatoms, usecols=['energy'], memory_map=True)
...
def read_atoms(file: str, chunk_size: int) -> Tuple[np.ndarray, np.ndarray]:
    ...
    df = pd.read_table(file, sep='\\s+', names=list(dtypes.keys()), dtype=dtypes, skiprows=skipheaders, memory_map=True)
...
class RemoteFile:
    ...
    def raw_dataset() -> 'RemoteFile':
        return RemoteFile(url='https://figshare.com/ndownloader/files/38063847', name='W3-W30_all_geoms_TTM2.1-F.zip')

    @staticmethod
    def processed_dataset() -> 'RemoteFile':
        return RemoteFile(url='https://figshare.com/ndownloader/files/38075781', name='W3-W30_pyg_processed.zip')

    @staticmethod
    def hydronet_splits() -> 'RemoteFile':
        return RemoteFile(url='https://figshare.com/ndownloader/files/38075904', name='hydronet_splits.zip')
...
class Partition(InMemoryDataset):
    ...
    def get(self, idx: int) -> Data:
        ...
        data = Data(z=torch.from_numpy(self.z[idx, :]), pos=torch.from_numpy(self.pos[idx, :, :]), y=torch.tensor(self.y[idx]))
