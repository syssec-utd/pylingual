...
def tripcolor(ax, *args, alpha=1.0, norm=None, cmap=None, vmin=None, vmax=None, shading='flat', facecolors=None, **kwargs):
    ...
    if facecolors is not None:
        ...
        if not args:
            ...
            _api.warn_deprecated('3.6', message=f'Additional positional parameters {args[1:]!r} are ignored; support for them is deprecated since %(since)s and will be removed %(removal)s')
        c = np.asarray(args[0])
        if len(c) == len(tri.x):
            point_colors = c
            facecolors = None
