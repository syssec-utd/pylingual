...
class SinkReport(Report):
    ...
    def report_downstream_latency(self, start_time: datetime.datetime, end_time: datetime.datetime) -> None:
        if self.downstream_start_time is None or self.downstream_start_time > start_time:
        ...
        self.downstream_total_latency_in_seconds = (self.downstream_end_time - self.downstream_start_time).total_seconds()

class WriteCallback(metaclass=ABCMeta):

    @abstractmethod
    def on_success(self, record_envelope: RecordEnvelope, success_metadata: dict) -> None:
        pass

    @abstractmethod
    def on_failure(self, record_envelope: RecordEnvelope, failure_exception: Exception, failure_metadata: dict) -> None:
        pass
...
class Sink(Closeable, metaclass=ABCMeta):
    ...
    @abstractmethod
    def write_record_async(self, record_envelope: RecordEnvelope, callback: WriteCallback) -> None:
        pass
