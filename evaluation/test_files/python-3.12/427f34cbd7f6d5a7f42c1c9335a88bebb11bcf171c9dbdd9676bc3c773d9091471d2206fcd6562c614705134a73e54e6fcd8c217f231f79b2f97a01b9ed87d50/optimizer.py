...
def _dygraph_distributed_optimizer(optimizer, strategy=None):
    ...
    if strategy is not None:
        if fleet_env._is_collective:
            warnings.warn('It is recommended to use DistributedStrategy in fleet_env.init(). The strategy here is only for compatibility. If the strategy in fleet_env.distributed_optimizer() is not None, then it will overwrite the DistributedStrategy in fleet_env.init(), which will take effect in distributed training.')
    ...
    if fleet_env.worker_num() > 1:
        if fleet_env._user_defined_strategy.heter_ccl_mode == False:
            return HybridParallelOptimizer(optimizer, fleet_env._hcg, fleet_env._user_defined_strategy)
        else:
            return HeterParallelOptimizer(optimizer, fleet_env._user_defined_strategy)
    else:
        return optimizer
