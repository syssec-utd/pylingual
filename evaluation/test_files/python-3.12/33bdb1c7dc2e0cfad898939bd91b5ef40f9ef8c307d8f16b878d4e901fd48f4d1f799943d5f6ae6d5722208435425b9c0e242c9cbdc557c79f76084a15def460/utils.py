...
keyword_aliases = {b'author': AUTHOR_KEYWORDS, b'date': DATE_KEYWORDS, b'header': HEADER_KEYWORDS, b'headurl': URL_KEYWORDS, b'id': ID_KEYWORDS, b'revision': REVISION_KEYWORDS, b'lastchangedby': AUTHOR_KEYWORDS, b'lastchangeddate': DATE_KEYWORDS, b'lastchangedrevision': REVISION_KEYWORDS, b'rev': REVISION_KEYWORDS, b'url': URL_KEYWORDS}
...
def has_expanded_svn_keywords(data):
    ...
    Returns:
        ...
        does not.
    """
    assert isinstance(data, bytes), 'data must be a byte string, not %s' % type(data)
    return bool(re.search(b'\\$[A-Za-z0-9]+::?\\s+([^\\$\\s]+)[^\\$]*\\$', data))
...
def collapse_svn_keywords(data, keyword_str):
    ...
    assert isinstance(data, bytes), 'data must be a byte string, not %s' % type(data)
    assert isinstance(keyword_str, bytes), 'keyword_str must be a byte string, not %s' % type(keyword_str)
    keywords = [re.escape(keyword) for name in re.split(b'\\W+', keyword_str) for keyword in keyword_aliases.get(name.lower(), [])]
    return re.sub(b'\\$(%s):(:?)([^\\$\\n\\r]*)\\$' % b'|'.join(keywords), repl, data, flags=re.IGNORECASE)