...
from narg2p.modules import Fp32GroupNorm, Fp32LayerNorm, GradMultiply, GumbelVectorQuantizer, LayerNorm, MultiheadAttention, SamePad, TransposeLast
...
class Wav2Vec2Model(BaseFairseqModel):
    ...
    def __init__(self, cfg: Wav2Vec2Config):
        ...
        if cfg.quantize_targets:
            ...
            self.quantizer = GumbelVectorQuantizer(dim=self.embed, num_vars=cfg.latent_vars, temp=cfg.latent_temp, groups=cfg.latent_groups, combine_groups=False, vq_dim=vq_dim, time_first=True)
        ...
        if cfg.quantize_input:
            if cfg.same_quantizer and self.quantizer is not None:
                ...
                self.input_quantizer = GumbelVectorQuantizer(dim=self.embed, num_vars=cfg.latent_vars, temp=cfg.latent_temp, groups=cfg.latent_groups, combine_groups=False, vq_dim=vq_dim, time_first=True)
...
class TransformerSentenceEncoderLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, self_attn_mask: torch.Tensor=None, self_attn_padding_mask: torch.Tensor=None, need_weights: bool=False, att_args=None):
        ...
        if self.layer_norm_first:
            ...
            x, attn = self.self_attn(query=x, key=x, value=x, key_padding_mask=self_attn_padding_mask, need_weights=need_weights)
            x = self.dropout1(x)
            x = residual + x
            x = self.self_attn_layer_norm(x)
            residual = x
