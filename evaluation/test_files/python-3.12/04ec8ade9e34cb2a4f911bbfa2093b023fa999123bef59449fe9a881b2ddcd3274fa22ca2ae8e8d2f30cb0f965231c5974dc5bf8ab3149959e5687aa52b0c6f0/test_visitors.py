...
def test_extension():
    ...
    def assert_eq(expr, expected=None):
        sql = FugueSQLParser(expr, 'fugueExtension', ignore_case=True, parse_mode=_PARSE_MODE)
        v = _VisitorBase(sql)
        obj = v.visit(sql.tree)
        assert expected == obj
    assert_eq('abc', 'abc')
...
def test_wild_schema():
    ...
    assert_eq('a:int32, *, B:string +x:str, k:int\n-Y, z -t~ w , x', 'a:int,*,B:str+x:str,k:int-Y,z-t~w,x')
    assert_eq(' ` * ` :int32, *, `` : string +x:str, k:int\n-Y, z -t~ w , x', '` * `:int,*,``:str+x:str,k:int-Y,z-t~w,x')
...
def test_pre_partition():
    ...
    assert_eq('prepartition ROWCOUNT*\n3+ (12.5-CONCUrrency/2)', PartitionSpec(num='ROWCOUNT*3+(12.5-CONCURRENCY/2)'))
    assert_eq('prepartition by a, b', PartitionSpec(by=['a', 'b']))
    assert_eq('HASH PrePARTITION 100 BY a,b', PartitionSpec(algo='hash', num=100, by=['a', 'b']))
    assert_eq('EVEN prepartition 100 BY a,b', PartitionSpec(algo='even', num=100, by=['a', 'b']))
    assert_eq('rand prepartition 100 BY a,b', PartitionSpec(algo='rand', num=100, by=['a', 'b']))
...
def test_single_output_common_expr():
    ...
    assert_eq('using a.B.c params a:1,b:2 schema a:int,b:int', 'a.B.c', dict(a=1, b=2), 'a:int,b:int')
