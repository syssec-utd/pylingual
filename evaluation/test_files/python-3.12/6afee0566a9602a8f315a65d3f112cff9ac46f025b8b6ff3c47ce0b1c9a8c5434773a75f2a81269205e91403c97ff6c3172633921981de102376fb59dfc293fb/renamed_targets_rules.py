...
@rule
def determine_renamed_target_types(target_types: RegisteredTargetTypes) -> RenameTargetsInFileRequest:
    return RenameTargetsInFileRequest({tgt.deprecated_alias: tgt.alias for tgt in target_types.types if tgt.deprecated_alias is not None})

class RenameRequest(FixBUILDFileRequest):
    pass

@rule
def fix_single(request: RenameRequest, renamed_target_types: RenameTargetsInFileRequest) -> FixedBUILDFile:
    tokens = request.tokenize()

    def should_be_renamed(token: tokenize.TokenInfo) -> bool:
        no_indentation = token.start[1] == 0
        if not (token.type is tokenize.NAME and token.string in renamed_target_types and no_indentation):
            return False
        try:
...
async def fix(request: RenameTargetsInFilesRequest.Batch) -> FixResult:
    digest_contents = await Get(DigestContents, Digest, request.snapshot.digest)
    fixed_contents = await MultiGet((Get(FixedBUILDFile, RenameRequest(file_content.path, file_content.content)) for file_content in digest_contents))
    snapshot = await Get(Snapshot, CreateDigest((FileContent(content.path, content.content) for content in fixed_contents)))
    return FixResult(request.snapshot, snapshot, '', '', tool_name=RenameTargetsInFilesRequest.tool_name)

def rules():
    return [*collect_rules(), *RenameTargetsInFilesRequest.rules()]