...
def _alter_conv2d_layout(attrs, inputs, tinfos, out_type):
    ...
    if isinstance(dispatch_ctx, autotvm.task.ApplyGraphBest):
        ...
        impl, outs = relay.backend.te_compiler.select_implementation(relay.op.get('nn.conv2d'), attrs, tinfos, out_type, target)
        ...
        if workload is None:
            if impl.name.find('winograd') != -1:
                ...
                return relay.nn.contrib_conv2d_winograd_without_weight_transform(inputs[0], weight, **new_attrs)
            return None
    ...
    if 'conv2d_nchw_winograd' in topi_tmpl:
        ...
        if len(data_tensor.shape) == 5:
            ...
            new_weight = te.placeholder((KH + tile_size - 1, KW + tile_size - 1, CI * CB, CO, COB), dtype=kernel_tensor.dtype)
            new_workload = autotvm.task.args_to_workload([new_data, new_weight, strides, padding, dilation, out_dtype], wkl_name)
        ...
        weight = relay.transpose(weight, axes=[2, 3, 0, 1])
        new_attrs['tile_size'] = tile_size
        new_attrs['channels'] = CO
        new_data = data_tensor
        new_weight = te.placeholder((KH + tile_size - 1, KW + tile_size - 1, CI, CO), dtype=kernel_tensor.dtype)
        ...
        new_data = te.placeholder((N, CI // in_channel_block, H, W, in_channel_block), dtype=data_dtype)
        new_weight = te.placeholder((KH + tile_size - 1, KW + tile_size - 1, CI, CO // num_filter_block, num_filter_block), dtype=kernel_tensor.dtype)
        new_workload = autotvm.task.args_to_workload([new_data, new_weight, strides, padding, dilation, out_dtype], wkl_name)
        dispatch_ctx.update(target, new_workload, cfg)
        return relay.nn.contrib_conv2d_winograd_without_weight_transform(inputs[0], weight, **new_attrs)
    if 'conv2d_nhwc_winograd' in topi_tmpl:
        ...
        new_data = te.placeholder((N, H, W, CI // in_channel_block, in_channel_block), dtype=data_dtype)
        new_weight = te.placeholder((KH + tile_size - 1, KW + tile_size - 1, CI, CO // num_filter_block, num_filter_block), dtype=kernel_tensor.dtype)
        new_workload = autotvm.task.args_to_workload([new_data, new_weight, strides, padding, dilation, out_dtype], wkl_name)
        dispatch_ctx.update(target, new_workload, cfg)
        return relay.nn.contrib_conv2d_winograd_without_weight_transform(inputs[0], weight, **new_attrs)
    if 'conv2d_nchwc' in topi_tmpl:
        if data_layout == 'NCHW' and kernel_layout == 'OIHW':
            ...
            new_attrs['out_layout'] = 'NCHW%dc' % num_filter_block
            new_data = te.placeholder((batch_size, in_channel // in_channel_block, height, width, in_channel_block), dtype=data_dtype)
            new_kernel = te.placeholder((out_channel // num_filter_block, in_filter_channel, kh, kw, num_filter_block), dtype=kernel_tensor.dtype)
            new_workload = autotvm.task.args_to_workload([new_data, new_kernel, strides, padding, dilation, out_dtype], topi_tmpl)
            dispatch_ctx.update(target, new_workload, cfg)
        else:
            assert _NCHWc_matcher.match(data_layout)
            assert _OIHWo_matcher.match(kernel_layout)
        return relay.nn.conv2d(*inputs, **new_attrs)
    if 'conv2d_nhwc' in topi_tmpl:
        if data_layout == 'NHWC' and kernel_layout == 'HWIO' or (data_layout == 'NHWC' and kernel_layout == 'HWOI'):
            ...
            new_attrs['out_layout'] = 'NHWC%dc' % num_filter_block
            new_data = te.placeholder((batch_size, in_height, in_width, in_channels // in_channel_block, in_channel_block), dtype=data_dtype)
            if kernel_layout == 'HWIO':
                new_kernel = te.placeholder((kernel_h, kernel_w, in_filter_channel, out_channles // num_filter_block, num_filter_block), dtype=kernel_tensor.dtype)
            else:
                new_kernel = te.placeholder((kernel_h, kernel_w, out_channles // num_filter_block, in_filter_channel, num_filter_block), dtype=kernel_tensor.dtype)
            new_workload = autotvm.task.args_to_workload([new_data, new_kernel, strides, padding, dilation, out_dtype], topi_tmpl)
            dispatch_ctx.update(target, new_workload, cfg)
        else:
