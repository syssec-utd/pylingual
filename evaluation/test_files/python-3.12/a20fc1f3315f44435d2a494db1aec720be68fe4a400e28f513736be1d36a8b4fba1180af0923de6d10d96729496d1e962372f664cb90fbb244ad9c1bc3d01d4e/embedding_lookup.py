...
from torchrec.distributed.batched_embedding_kernel import BaseBatchedEmbedding, BaseBatchedEmbeddingBag, BatchedDenseEmbedding, BatchedDenseEmbeddingBag, BatchedFusedEmbedding, BatchedFusedEmbeddingBag
from torchrec.distributed.embedding_kernel import BaseEmbedding, GroupedEmbedding, GroupedEmbeddingBag
from torchrec.distributed.embedding_types import BaseEmbeddingLookup, BaseGroupedFeatureProcessor, EmbeddingComputeKernel, GroupedEmbeddingConfig, SparseFeatures, SparseFeaturesList
...
class GroupedEmbeddingsLookup(BaseEmbeddingLookup[SparseFeatures, torch.Tensor]):
    ...
    def __init__(self, grouped_configs: List[GroupedEmbeddingConfig], pg: Optional[dist.ProcessGroup]=None, device: Optional[torch.device]=None, fused_params: Optional[Dict[str, Any]]=None) -> None:
    ...
    def forward(self, sparse_features: SparseFeatures) -> torch.Tensor:
        ...
        id_list_features_by_group = sparse_features.id_list_features.split(self._id_list_feature_splits)
...
class GroupedPooledEmbeddingsLookup(BaseEmbeddingLookup[SparseFeatures, torch.Tensor]):
    ...
    def __init__(self, grouped_configs: List[GroupedEmbeddingConfig], grouped_score_configs: List[GroupedEmbeddingConfig], device: Optional[torch.device]=None, fused_params: Optional[Dict[str, Any]]=None, pg: Optional[dist.ProcessGroup]=None, feature_processor: Optional[BaseGroupedFeatureProcessor]=None) -> None:

        def _create_lookup(config: GroupedEmbeddingConfig, device: Optional[torch.device]=None) -> BaseEmbedding:
            if config.compute_kernel == EmbeddingComputeKernel.DENSE:
    ...
    def forward(self, sparse_features: SparseFeatures) -> torch.Tensor:
        assert sparse_features.id_list_features is not None or sparse_features.id_score_list_features is not None
        embeddings: List[torch.Tensor] = []
        if len(self._emb_modules) > 0:
            assert sparse_features.id_list_features is not None
            id_list_features_by_group = sparse_features.id_list_features.split(self._id_list_feature_splits)
            for config, emb_op, features in zip(self.grouped_configs, self._emb_modules, id_list_features_by_group):
        ...
        if len(self._score_emb_modules) > 0:
            ...
            id_score_list_features_by_group = sparse_features.id_score_list_features.split(self._id_score_list_feature_splits)
            for config, emb_op, features in zip(self.grouped_score_configs, self._score_emb_modules, id_score_list_features_by_group):
                if config.has_feature_processor and self._feature_processor is not None and isinstance(self._feature_processor, BaseGroupedFeatureProcessor):
                    features = self._feature_processor(features)
                embeddings.append(emb_op(features))
        if len(embeddings) == 0:
            batch_size: int = sparse_features.id_list_features.stride() if sparse_features.id_list_features is not None else sparse_features.id_score_list_features.stride()
...
class MetaInferGroupedEmbeddingsLookup(BaseEmbeddingLookup[SparseFeatures, torch.Tensor]):
    """
    ...
    def __init__(self, grouped_configs: List[GroupedEmbeddingConfig], device: Optional[torch.device]=None, fused_params: Optional[Dict[str, Any]]=None) -> None:
        ...
        def _create_lookup(config: GroupedEmbeddingConfig, device: Optional[torch.device]=None, fused_params: Optional[Dict[str, Any]]=None) -> BaseBatchedEmbedding:
        ...
        self._id_list_feature_splits: List[int] = [config.num_features() for config in grouped_configs]
        self.register_buffer('_dummy_embs_tensor', torch.empty([0], dtype=torch.float32, device=device))
        self.grouped_configs = grouped_configs
...
class MetaInferGroupedPooledEmbeddingsLookup(BaseEmbeddingLookup[SparseFeatures, torch.Tensor]):
    """
    ...
    def __init__(self, grouped_configs: List[GroupedEmbeddingConfig], grouped_score_configs: List[GroupedEmbeddingConfig], device: Optional[torch.device]=None, feature_processor: Optional[BaseGroupedFeatureProcessor]=None, fused_params: Optional[Dict[str, Any]]=None) -> None:

        def _create_lookup(config: GroupedEmbeddingConfig, device: Optional[torch.device]=None, fused_params: Optional[Dict[str, Any]]=None) -> BaseBatchedEmbeddingBag:
        ...
        for config in grouped_score_configs:
            self._score_emb_modules.append(_create_lookup(config, device))
        self._id_list_feature_splits: List[int] = [config.num_features() for config in grouped_configs]
        self._id_score_list_feature_splits: List[int] = [config.num_features() for config in grouped_score_configs]
        self.register_buffer('_dummy_embs_tensor', torch.empty([0], dtype=torch.float32, device=device))
        self.grouped_configs = grouped_configs
        self.grouped_score_configs = grouped_score_configs
        self._feature_processor = feature_processor
    ...
    def forward(self, sparse_features: SparseFeatures) -> torch.Tensor:
        ...
        if len(self._score_emb_modules) > 0:
            assert sparse_features.id_score_list_features is not None
            id_score_list_features_by_group = sparse_features.id_score_list_features.split(self._id_score_list_feature_splits)
            for emb_op, features in zip(self._score_emb_modules, id_score_list_features_by_group):
                embeddings.append(emb_op(features))
        if len(embeddings) == 0:
            batch_size: int = sparse_features.id_list_features.stride() if sparse_features.id_list_features is not None else sparse_features.id_score_list_features.stride()
            return self._dummy_embs_tensor.view(batch_size, 0)
...
class InferGroupedLookupMixin(ABC):
    ...
    def forward(self, sparse_features: SparseFeaturesList) -> List[torch.Tensor]:
        embeddings: List[torch.Tensor] = []
        for sparse_features_rank, embedding_lookup in zip(sparse_features, self._embedding_lookups_per_rank):
            assert sparse_features_rank.id_list_features is not None or sparse_features_rank.id_score_list_features is not None
...
class InferGroupedPooledEmbeddingsLookup(InferGroupedLookupMixin, BaseEmbeddingLookup[SparseFeaturesList, List[torch.Tensor]]):

    def __init__(self, grouped_configs_per_rank: List[List[GroupedEmbeddingConfig]], grouped_score_configs_per_rank: List[List[GroupedEmbeddingConfig]], world_size: int, fused_params: Optional[Dict[str, Any]]=None) -> None:
        super().__init__()
        self._embedding_lookups_per_rank: List[MetaInferGroupedPooledEmbeddingsLookup] = []
        for rank in range(world_size):
            self._embedding_lookups_per_rank.append(MetaInferGroupedPooledEmbeddingsLookup(grouped_configs=grouped_configs_per_rank[rank], grouped_score_configs=grouped_score_configs_per_rank[rank], device=torch.device('cuda', rank), fused_params=fused_params))

class InferGroupedEmbeddingsLookup(InferGroupedLookupMixin, BaseEmbeddingLookup[SparseFeaturesList, List[torch.Tensor]]):

    def __init__(self, grouped_configs_per_rank: List[List[GroupedEmbeddingConfig]], world_size: int, fused_params: Optional[Dict[str, Any]]=None) -> None:
        super().__init__()
        self._embedding_lookups_per_rank: List[MetaInferGroupedEmbeddingsLookup] = []
        for rank in range(world_size):
            self._embedding_lookups_per_rank.append(MetaInferGroupedEmbeddingsLookup(grouped_configs=grouped_configs_per_rank[rank], device=torch.device('cuda', rank), fused_params=fused_params))