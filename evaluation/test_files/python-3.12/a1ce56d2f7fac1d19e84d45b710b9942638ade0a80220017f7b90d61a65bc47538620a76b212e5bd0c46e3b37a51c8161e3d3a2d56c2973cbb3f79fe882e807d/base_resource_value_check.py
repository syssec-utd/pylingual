...
class BaseResourceValueCheck(BaseResourceCheck):
    ...
    def __init__(self, name: str, id: str, categories: 'Iterable[CheckCategories]', supported_resources: 'Iterable[str]', missing_block_result: CheckResult=CheckResult.FAILED) -> None:
    ...
    def _filter_key_path(path: str) -> List[str]:
        ...
        """
        return [x for x in path.split('/') if not re.search(re.compile('^\\[?\\d+]?$'), x)]
