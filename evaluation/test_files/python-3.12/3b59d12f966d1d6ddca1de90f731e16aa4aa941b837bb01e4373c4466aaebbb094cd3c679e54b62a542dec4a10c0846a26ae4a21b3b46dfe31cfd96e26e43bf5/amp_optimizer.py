...
class AMPOptimizer(MetaOptimizerBase):
    ...
    def __init__(self, optimizer):
        ...
        self.meta_optimizers_white_list = ['LarsOptimizer', 'LambOptimizer', 'RecomputeOptimizer', 'GraphExecutionOptimizer']
    ...
    def _init_wrapped_opt(self):
        ...
        amp_lists = mixed_precision.AutoMixedPrecisionLists(custom_white_list, custom_black_list, custom_black_varnames)
        self.wrapped_opt = mixed_precision.decorate(self.inner_opt, amp_lists, config['init_loss_scaling'], config['incr_every_n_steps'], config['decr_every_n_nan_or_inf'], config['incr_ratio'], config['decr_ratio'], config['use_dynamic_loss_scaling'], config['use_pure_fp16'], config['use_fp16_guard'])
    ...
    def _enable_strategy(self, dist_strategy, context):
        ...
        dist_strategy.amp_configs = {'init_loss_scaling': 32768.0, 'incr_every_n_steps': 1000, 'decr_every_n_nan_or_inf': 2, 'incr_ratio': 2.0, 'decr_ratio': 0.8, 'use_dynamic_loss_scaling': True}
