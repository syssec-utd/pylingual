...
class MobiIndex:
    ...
    def parseINDXHeader(self, data):
        ...
        ordt1 = None
        ordt2 = None
        ocnt, oentries, op1, op2, otagx = struct.unpack_from(b'>LLLLL', data, 164)
    ...
    def readCTOC(self, txtdata):
        ...
        while offset < len(txtdata):
            if PY2:
                if txtdata[offset] == b'\x00':
                    break
            elif txtdata[offset] == 0:
                break
            idx_offs = offset
...
def readTagSection(start, data):
    ...
    if data[start:start + 4] == b'TAGX':
        firstEntryOffset, = struct.unpack_from(b'>L', data, start + 4)
        controlByteCount, = struct.unpack_from(b'>L', data, start + 8)
