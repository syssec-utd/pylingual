...
@pytest.mark.parametrize('type_key', [('CanonicalUser', 'id', 'ID'), ('Group', 'url', 'URI')])
@pytest.mark.parametrize('value', ['AllUsers', 'http://acs.amazonaws.com/groups/global/AllUsers'])
...
def test_put_object_acl_using_grant(readwrite, type_key, value, has_quotes):
    ...
    args = {'Bucket': bucket_name, 'Key': keyname, f'Grant{readwrite}': f'{key}={header_value}'}
...
def test_acl_switching_boto3():
    ...
    grants.shouldnt.contain({'Grantee': {'Type': 'Group', 'URI': 'http://acs.amazonaws.com/groups/global/AllUsers'}, 'Permission': 'READ'})
...
def test_s3_object_in_private_bucket():
    s3 = boto3.resource('s3')
    bucket = s3.Bucket('test-bucket')
    bucket.create(ACL='private', CreateBucketConfiguration={'LocationConstraint': 'us-west-1'})
    bucket.put_object(ACL='private', Body=b'ABCD', Key='file.txt')
    s3_anonymous = boto3.resource('s3')
    s3_anonymous.meta.client.meta.events.register('choose-signer.s3.*', disable_signing)
    with pytest.raises(ClientError) as exc:
...
def test_put_bucket_acl_body():
    ...
    s3.put_bucket_acl(Bucket='bucket', AccessControlPolicy={'Grants': [{'Grantee': {'URI': 'http://acs.amazonaws.com/groups/s3/LogDelivery', 'Type': 'Group'}, 'Permission': 'WRITE'}, {'Grantee': {'URI': 'http://acs.amazonaws.com/groups/s3/LogDelivery', 'Type': 'Group'}, 'Permission': 'READ_ACP'}], 'Owner': bucket_owner})
    ...
    with pytest.raises(ClientError) as err:
        s3.put_bucket_acl(Bucket='bucket', AccessControlPolicy={'Grants': [{'Grantee': {'URI': 'http://acs.amazonaws.com/groups/s3/LogDelivery', 'Type': 'Group'}, 'Permission': 'WRITE'}]})
...
def test_object_acl_with_presigned_post():
    ...
    response = s3.generate_presigned_post(bucket_name, object_name, Fields=fields, ExpiresIn=60000)
...
def test_acl_setting_boto3():
    ...
    content = b'imafile'
    keyname = 'test.txt'
    bucket.put_object(Key=keyname, Body=content, ContentType='text/plain', ACL='public-read')
    grants = client.get_object_acl(Bucket='foobar', Key=keyname)['Grants']
    grants.should.contain({'Grantee': {'Type': 'Group', 'URI': 'http://acs.amazonaws.com/groups/global/AllUsers'}, 'Permission': 'READ'})
...
def test_raise_exception_for_grant_and_acl():
    ...
    with pytest.raises(ClientError) as exc:
        bucket.put_object(ACL='bucket-owner-full-control', Body='example-file-path', Key='example-key', ContentType='text/plain', GrantFullControl=f'id="{acl_grantee_id}"')
    err = exc.value.response['Error']
    err['Code'].should.equal('InvalidRequest')
    err['Message'].should.equal('Specifying both Canned ACLs and Header Grants is not allowed')