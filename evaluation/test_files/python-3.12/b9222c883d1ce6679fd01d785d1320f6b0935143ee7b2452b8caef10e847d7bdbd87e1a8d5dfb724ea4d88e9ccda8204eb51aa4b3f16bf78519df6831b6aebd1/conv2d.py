...
Workload = namedtuple('Workload', ['in_dtype', 'out_dtype', 'height', 'width', 'in_filter', 'groups', 'out_filter', 'kernel_h', 'kernel_w', 'padt', 'padl', 'padb', 'padr', 'dilation_h', 'dilation_w', 'stride_h', 'stride_w'])
...
def _get_workload(data, kernel, stride, padding, dilation, out_dtype, data_layout='NCHW'):
    ...
    pt, pl, pb, pr = get_pad_tuple(padding, (get_const_int((KH - 1) * dilation_h + 1), get_const_int((KW - 1) * dilation_w + 1)))
    GRPS = CI // CIG
    if isinstance(stride, (tuple, list)):
        HSTR, WSTR = stride
    else:
        HSTR, WSTR = (stride, stride)
    assert data.dtype == kernel.dtype or (data.dtype == 'uint8' and kernel.dtype == 'int8'), "Do not support inputs with different data types now. '         '{} vs. {}".format(data.dtype, kernel.dtype)
    return Workload(data.dtype, out_dtype, IH, IW, CI, GRPS, CO, KH, KW, pt, pl, pb, pr, dilation_h, dilation_w, HSTR, WSTR)
...
def conv2d_hwcn(Input, Filter, stride, padding, dilation, out_dtype=None):
    ...
    ry = te.reduce_axis((0, kernel_h), name='ry')
    rx = te.reduce_axis((0, kernel_w), name='rx')
    Output = te.compute((out_height, out_width, out_channel, batch), lambda yy, xx, ff, nn: te.sum(PaddedInput[yy * stride_h + ry * dilation_h, xx * stride_w + rx * dilation_w, rc, nn].astype(out_dtype) * Filter[ry, rx, rc, ff].astype(out_dtype), axis=[ry, rx, rc]), name='Conv2dOutput', tag='conv2d_hwcn')
    return Output

def conv2d_nhwc(Input, Filter, stride, padding, dilation, out_dtype='float32', auto_scheduler_rewritten_layout=''):
    ...
    if auto_scheduler_rewritten_layout:
        kernel_h, kernel_w, channel, num_filter = auto_scheduler.get_shape_from_rewritten_layout(auto_scheduler_rewritten_layout, ['ry', 'rx', 'rc', 'ff'])
    ...
    ry = te.reduce_axis((0, kernel_h), name='ry')
    rx = te.reduce_axis((0, kernel_w), name='rx')
    Output = te.compute((batch, out_height, out_width, out_channel), lambda nn, yy, xx, ff: te.sum(PaddedInput[nn, yy * stride_h + ry * dilation_h, xx * stride_w + rx * dilation_w, rc].astype(out_dtype) * Filter[ry, rx, rc, ff].astype(out_dtype), axis=[ry, rx, rc]), name='Conv2dOutput', tag='conv2d_nhwc', attrs={'layout_free_placeholders': [Filter]})
    if auto_scheduler_rewritten_layout:
...
def conv2d_NCHWc(data, kernel, stride, padding, dilation, layout, out_layout, out_dtype='float32'):
    ...
    return te.compute(oshape, lambda n, oc_chunk, oh, ow, oc_block: te.sum(data_pad[n, idxdiv(ic, ic_bn), oh * HSTR + kh * dilation_h, ow * WSTR + kw * dilation_w, idxmod(ic, ic_bn)].astype(out_dtype) * kernel[oc_chunk, idxdiv(ic, ic_bn), kh, kw, idxmod(ic, ic_bn), oc_block].astype(out_dtype), axis=[ic, kh, kw]), name='conv2d_NCHWc', tag='conv2d_NCHWc')

def conv2d_NCHWc_int8(data, kernel, stride, padding, dilation, layout, out_layout, out_dtype='int32', n_elems=4):
    """Conv2D operator for nChw[x]c layout.
    ...
    if groups == 1:
        ...
        return te.compute(oshape, lambda n, oc_chunk, oh, ow, oc_block: te.sum(data_pad[n, ic_outer, oh * HSTR + kh * dilation_h, ow * WSTR + kw * dilation_w, ic_f_inner * n_elems + ic_s_inner].astype(out_dtype) * kernel[oc_chunk, ic_outer, kh, kw, ic_f_inner, oc_block, ic_s_inner].astype(out_dtype), axis=[kh, kw, ic_outer, ic_f_inner, ic_s_inner]), name='conv2d_NCHWc_int8', tag='conv2d_NCHWc_int8')
    ic_chunk = in_channel // ic_bn
    ic_outer = te.reduce_axis((0, ic_chunk // groups), name='ic_outer')
    ic_f_inner = te.reduce_axis((0, ic_bn // n_elems), name='ic_f_inner')
    ic_s_inner = te.reduce_axis((0, n_elems), name='ic_s_inner')
    oshape = (n, oc_chunk, out_height, out_width, oc_bn)
    return te.compute(oshape, lambda n, occ, oh, ow, oc_block: te.sum(data_pad[n, occ * oc_bn // (oc_chunk * oc_bn // groups) * (ic_chunk // groups) + ic_outer, oh * HSTR + kh, ow * WSTR + kw, ic_f_inner * n_elems + ic_s_inner].astype(out_dtype) * kernel[occ, ic_outer, kh, kw, ic_f_inner, oc_block, ic_s_inner].astype(out_dtype), axis=[kh, kw, ic_outer, ic_f_inner, ic_s_inner]), name='conv2d_NCHWc_int8', tag='conv2d_NCHWc_int8')
...
def conv2d_gemm_weight_transform(kernel, tile_rows, tile_cols):
    ...
    return te.compute((N_padded // tile_rows, K_padded // tile_cols, tile_rows, tile_cols), lambda x, y, z, w: kernel_flat[w + tile_cols * y, z + tile_rows * x], name='weight_block_reshape')
...
def conv2d_winograd_weight_transform(kernel, tile_size):
    ...
    return te.compute(shape, lambda eps, nu, co, ci: te.sum(kernel[co][ci][r_kh][r_kw] * G[eps][r_kh] * G[nu][r_kw], axis=[r_kh, r_kw]), name='transform_weight')
...
def conv2d_winograd_nnpack_weight_transform(kernel, convolution_algorithm, out_dtype):
    ...
    return nnpack.convolution_inference_weight_transform(kernel, algorithm=convolution_algorithm, dtype=out_dtype)
...
def group_conv2d_nchw(Input, Filter, stride, padding, dilation, groups, out_dtype=None):
    ...
    out_channel = num_filter
    out_height = simplify((in_height - (kernel_h - 1) * dilation_h - 1 + pad_top + pad_down) // stride_h + 1)
    out_width = simplify((in_width - (kernel_w - 1) * dilation_w - 1 + pad_left + pad_right) // stride_w + 1)
    pad_before = [0, 0, pad_top, pad_left]
    pad_after = [0, 0, pad_down, pad_right]
    temp = pad(Input, pad_before, pad_after, name='pad_temp')
    ...
    ry = te.reduce_axis((0, kernel_h), name='ry')
    rx = te.reduce_axis((0, kernel_w), name='rx')
    return te.compute((batch, out_channel, out_height, out_width), lambda nn, ff, yy, xx: te.sum(temp[nn, ff // (num_filter // groups) * (in_channel // groups) + rc, yy * stride_h + ry * dilation_h, xx * stride_w + rx * dilation_w].astype(out_dtype) * Filter[ff, rc, ry, rx].astype(out_dtype), axis=[rc, ry, rx]), tag='group_conv2d_nchw')
...
def group_conv2d_nhwc(Input, Filter, stride, padding, dilation, groups, out_dtype=None):
    ...
    rx = te.reduce_axis((0, kernel_w), name='rx')
    rc = te.reduce_axis((0, in_channel // groups), name='rc')
    return te.compute((batch, out_height, out_width, out_channel), lambda nn, yy, xx, ff: te.sum(temp[nn, yy * stride_h + ry * dilation_h, xx * stride_w + rx * dilation_w, ff // (num_filter // groups) * (in_channel // groups) + rc].astype(out_dtype) * Filter[ry, rx, rc, ff].astype(out_dtype), axis=[ry, rx, rc]), tag='group_conv2d_nhwc')
...
def unpack_NCHWc_to_nchw(packed_out, out_dtype):
    ...
    unpacked_out = te.compute(oshape, lambda n, c, h, w: packed_out[n, idxdiv(c, oc_bn), h, w, idxmod(c, oc_bn)].astype(out_dtype), name='output_unpack', tag=tag.INJECTIVE + ',unpack_nchwc')
    return unpacked_out

def _conv2d_winograd_nhwc_impl(data, weight, strides, padding, dilation, out_dtype, tile_size, pre_computed=False, auto_scheduler_rewritten_layout=''):
    ...
    if not pre_computed:
        ...
        if auto_scheduler_rewritten_layout:
            H_CAT, W_CAT, CO, CI = get_const_tuple(auto_scheduler.get_shape_from_rewritten_layout(auto_scheduler_rewritten_layout, ['eps', 'nu', 'co', 'ci']))
    ...
    data_pad = pad(data, (0, pad_t, pad_l, 0), (0, pad_b + pad_extra, pad_r + pad_extra, 0), name='data_pad')
    if not pre_computed:
        r_kh = te.reduce_axis((0, KH), name='r_kh')
        r_kw = te.reduce_axis((0, KW), name='r_kw')
        kernel_pack = te.compute((alpha, alpha, CO, CI), lambda eps, nu, co, ci: te.sum(weight[r_kh][r_kw][ci][co] * G[eps][r_kh] * G[nu][r_kw], axis=[r_kh, r_kw]), name='kernel_pack')
        attrs = {}
    else:
        kernel_pack = weight
        attrs = {'layout_free_placeholders': [kernel_pack]}
    input_tile = te.compute((alpha, alpha, P, CI), lambda eps, nu, p, ci: data_pad[p // (nH * nW)][p // nW % nH * m + eps][p % nW * m + nu][ci], name='input_tile')
    r_a = te.reduce_axis((0, alpha), 'r_a')
    r_b = te.reduce_axis((0, alpha), 'r_b')
    data_pack = te.compute((alpha, alpha, P, CI), lambda eps, nu, p, ci: te.sum(input_tile[r_a][r_b][p][ci] * B[r_a][eps] * B[r_b][nu], axis=[r_a, r_b]), name='data_pack', attrs={'auto_scheduler_simplify_const_tensor_indices': ['eps', 'nu', 'r_a', 'r_b']})
    ci = te.reduce_axis((0, CI), name='ci')
    bgemm = te.compute((alpha, alpha, P, CO), lambda eps, nu, p, co: te.sum(data_pack[eps][nu][p][ci] * kernel_pack[eps][nu][co][ci], axis=[ci]), name='bgemm', attrs=attrs)
    if auto_scheduler_rewritten_layout:
        bgemm = auto_scheduler.rewrite_compute_body(bgemm, auto_scheduler_rewritten_layout)
    r_a = te.reduce_axis((0, alpha), 'r_a')
    r_b = te.reduce_axis((0, alpha), 'r_b')
    inverse = te.compute((m, m, P, CO), lambda vh, vw, p, co: te.sum(bgemm[r_a][r_b][p][co] * A[r_a][vh] * A[r_b][vw], axis=[r_a, r_b]), name='inverse', attrs={'auto_scheduler_simplify_const_tensor_indices': ['vh', 'vw', 'r_a', 'r_b']})
    output = te.compute((N, H, W, CO), lambda n, h, w, co: inverse[h % m, w % m, n * nH * nW + h // m * nW + w // m, co], name='conv2d_winograd')
    return output

@tvm.target.generic_func
def conv2d_winograd_nhwc(data, weight, strides, padding, dilation, out_dtype, pre_computed=False, auto_scheduler_rewritten_layout=''):
    ...
    """
    tile_size = 4
    return _conv2d_winograd_nhwc_impl(data, weight, strides, padding, dilation, out_dtype, tile_size, pre_computed, auto_scheduler_rewritten_layout)

def conv2d_winograd_nhwc_without_weight_transform(data, weight, strides, padding, dilation, out_dtype, auto_scheduler_rewritten_layout=''):
    ...
    output : tvm.Tensor
        4-D with shape [batch, out_height, out_width, out_channel]
    """
    return conv2d_winograd_nhwc(data, weight, strides, padding, dilation, out_dtype, pre_computed=True, auto_scheduler_rewritten_layout=auto_scheduler_rewritten_layout)