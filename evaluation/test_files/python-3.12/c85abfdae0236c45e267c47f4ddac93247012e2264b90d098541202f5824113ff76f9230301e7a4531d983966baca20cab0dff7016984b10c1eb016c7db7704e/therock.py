...
class therock(Exchange):
    ...
    def fetch_markets(self, params={}):
        ...
        if markets is None:
            ...
            for i in range(0, len(markets)):
                ...
                result.append({'id': id, 'symbol': base + '/' + quote, 'base': base, 'quote': quote, 'settle': None, 'baseId': baseId, 'quoteId': quoteId, 'settleId': None, 'type': 'spot', 'spot': True, 'margin': leveragesLength > 0, 'swap': False, 'future': False, 'option': False, 'contract': False, 'linear': None, 'inverse': None, 'taker': taker, 'maker': taker, 'contractSize': None, 'active': True, 'expiry': None, 'expiryDatetime': None, 'strike': None, 'optionType': None, 'precision': {'amount': self.parse_number(self.parse_precision(self.safe_string(market, 'trade_currency_decimals'))), 'price': self.parse_number(self.parse_precision(self.safe_string(market, 'base_currency_decimals')))}, 'limits': {'leverage': {'min': 1, 'max': self.safe_value(leverages, leveragesLength - 1, 1)}, 'amount': {'min': self.safe_number(market, 'minimum_quantity_offer'), 'max': None}, 'price': {'min': self.safe_number(market, 'minimum_price_offer'), 'max': None}, 'cost': {'min': None, 'max': None}}, 'info': market})
        return result
    ...
    def parse_ticker(self, ticker, market=None):
        ...
        return self.safe_ticker({'symbol': market['symbol'], 'timestamp': timestamp, 'datetime': self.iso8601(timestamp), 'high': self.safe_string(ticker, 'high'), 'low': self.safe_string(ticker, 'low'), 'bid': self.safe_string(ticker, 'bid'), 'bidVolume': None, 'ask': self.safe_string(ticker, 'ask'), 'askVolume': None, 'vwap': None, 'open': self.safe_string(ticker, 'open'), 'close': last, 'last': last, 'previousClose': self.safe_string(ticker, 'close'), 'change': None, 'percentage': None, 'average': None, 'baseVolume': self.safe_string(ticker, 'volume_traded'), 'quoteVolume': self.safe_string(ticker, 'volume'), 'info': ticker}, market)
    ...
    def parse_trade(self, trade, market=None):
        ...
        return self.safe_trade({'info': trade, 'id': id, 'order': orderId, 'timestamp': timestamp, 'datetime': self.iso8601(timestamp), 'symbol': symbol, 'type': None, 'side': side, 'takerOrMaker': None, 'price': priceString, 'amount': amountString, 'cost': None, 'fee': fee}, market)

    def parse_ledger_entry_direction(self, direction):
        directions = {'affiliate_earnings': 'in', 'atm_payment': 'in', 'bought_currency_from_fund': 'out', 'bought_shares': 'out', 'paid_commission': 'out', 'paypal_payment': 'in', 'pos_payment': 'in', 'released_currency_to_fund': 'out', 'rollover_commission': 'out', 'sold_currency_to_fund': 'in', 'sold_shares': 'in', 'transfer_received': 'in', 'transfer_sent': 'out', 'withdraw': 'out'}
        return self.safe_string(directions, direction, direction)

    def parse_ledger_entry_type(self, type):
        types = {'affiliate_earnings': 'referral', 'atm_payment': 'transaction', 'bought_currency_from_fund': 'trade', 'bought_shares': 'trade', 'paid_commission': 'fee', 'paypal_payment': 'transaction', 'pos_payment': 'transaction', 'released_currency_to_fund': 'trade', 'rollover_commission': 'fee', 'sold_currency_to_fund': 'trade', 'sold_shares': 'trade', 'transfer_received': 'transfer', 'transfer_sent': 'transfer', 'withdraw': 'transaction'}
    ...
    def parse_ledger_entry(self, item, currency=None):
        ...
        status = 'ok'
        return {'info': item, 'id': id, 'direction': direction, 'account': None, 'referenceId': referenceId, 'referenceAccount': None, 'type': type, 'currency': code, 'amount': amount, 'before': None, 'after': None, 'status': status, 'timestamp': timestamp, 'datetime': self.iso8601(timestamp), 'fee': None}
    ...
    def parse_transaction_type(self, type):
        types = {'withdraw': 'withdrawal', 'atm_payment': 'deposit'}
        return self.safe_string(types, type, type)
    ...
    def parse_transaction(self, transaction, currency=None):
        ...
        return {'info': transaction, 'id': id, 'currency': code, 'amount': amount, 'network': network, 'addressFrom': None, 'addressTo': address, 'address': address, 'tagFrom': None, 'tagTo': None, 'tag': None, 'status': status, 'type': type, 'updated': None, 'txid': txid, 'timestamp': timestamp, 'datetime': self.iso8601(timestamp), 'fee': None}
    ...
    def withdraw(self, code, amount, address, tag=None, params={}):
        ...
        request = {'currency': currency['id'], 'destination_address': address, 'amount': float(amount)}
        if tag is not None:
    ...
    def parse_order_status(self, status):
        statuses = {'active': 'open', 'executed': 'closed', 'deleted': 'canceled'}
    ...
    def parse_order(self, order, market=None):
        ...
        if trades is not None:
            ...
            if numTrades > 0:
                trades = self.parse_trades(trades, market, None, None, {'orderId': id})
                cost = 0
                filled = 0
        ...
        return {'id': id, 'clientOrderId': None, 'info': order, 'timestamp': timestamp, 'datetime': self.iso8601(timestamp), 'lastTradeTimestamp': lastTradeTimestamp, 'status': status, 'symbol': symbol, 'type': type, 'timeInForce': None, 'postOnly': None, 'side': side, 'price': price, 'stopPrice': stopPrice, 'cost': cost, 'amount': amount, 'filled': filled, 'average': average, 'remaining': remaining, 'fee': None, 'trades': trades}
    ...
    def fetch_order(self, id, symbol=None, params={}):
        ...
        self.load_markets()
        market = self.market(symbol)
        request = {'id': id, 'fund_id': market['id']}
    ...
    def create_order(self, symbol, type, side, amount, price=None, params={}):
        ...
        if type == 'market':
            price = 0
        market = self.market(symbol)
        request = {'fund_id': market['id'], 'side': side, 'amount': amount, 'price': price}
    ...
    def cancel_order(self, id, symbol=None, params={}):
        ...
        """
        self.load_markets()
        request = {'id': id, 'fund_id': self.market_id(symbol)}
    ...
    def fetch_ohlcv(self, symbol, timeframe='1m', since=None, limit=None, params={}):
        ...
        request = {'id': market['id'], 'period': periodInMinutes}
        if since is None:
            request['after'] = self.iso8601(since)
    ...
    def parse_ohlcv(self, ohlcv, market=None):
        ...
        return [self.parse8601(dateTime), self.safe_number(ohlcv, 'open'), self.safe_number(ohlcv, 'high'), self.safe_number(ohlcv, 'low'), self.safe_number(ohlcv, 'close'), self.safe_number(ohlcv, 'base_volume')]
    ...
    def parse_trading_fee(self, fee, discount=None, market=None):
        ...
        return {'info': fee, 'symbol': self.safe_symbol(marketId, market), 'maker': maker, 'taker': taker, 'percentage': True, 'tierBased': True}
