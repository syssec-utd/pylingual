...
from localstack.utils.container_utils.container_client import ContainerException, PortMappings, SimpleVolumeBind, VolumeBind, VolumeMappings
...
API_DEPENDENCIES = {'dynamodb': ['dynamodbstreams'], 'dynamodbstreams': ['kinesis'], 'es': ['opensearch'], 'lambda': ['logs', 'cloudwatch'], 'kinesis': ['dynamodb'], 'firehose': ['kinesis']}
API_COMPOSITES = {'serverless': ['cloudformation', 'cloudwatch', 'iam', 'sts', 'lambda', 'dynamodb', 'apigateway', 's3'], 'cognito': ['cognito-idp', 'cognito-identity']}
...
def get_docker_image_details(image_name: str=None) -> Dict[str, str]:
    ...
    try:
        ...
        return {}
    result = {'id': result['Id'].replace('sha256:', '')[:12], 'tag': (result.get('RepoTags') or ['latest'])[0].split(':')[-1], 'created': result['Created'].split('.')[0]}
    return result
...
def get_server_version_from_running_container() -> str:
    try:
        ...
        version, _ = DOCKER_CLIENT.exec_in_container(container_name, interactive=True, command=['bin/localstack', '--version'])
        version = to_str(version).strip().splitlines()[-1]
        return version
    except ContainerException:
        try:
            img_name = get_docker_image_to_start()
            version, _ = DOCKER_CLIENT.run_container(img_name, remove=True, interactive=True, entrypoint='', command=['bin/localstack', '--version'])
            version = to_str(version).strip().splitlines()[-1]
            return version
        except ContainerException:
...
def validate_localstack_config(name):
    ...
    if image_name.split(':')[0] not in constants.OFFICIAL_IMAGES:
        warns.append('Using custom image "%s", we recommend using an official image: %s' % (image_name, constants.OFFICIAL_IMAGES))
    image_name = ls_service_details.get('image')
    ...
    docker_env = dict(((env.split('=')[0], env.split('=')[1]) for env in ls_service_details.get('environment', {})))
    ...
    if docker_env.get('PORT_WEB_UI') not in ['${PORT_WEB_UI- }', None, ''] and image_name == 'localstack/localstack':
        warns.append('"PORT_WEB_UI" Web UI is now deprecated, and requires to use the "localstack/localstack-full" image.')
    if main_container not in container_name and (not docker_env.get('MAIN_CONTAINER_NAME')):
        warns.append('Please use "container_name: %s" or add "MAIN_CONTAINER_NAME" in "environment".' % main_container)

    def port_exposed(port):
        for exposed in docker_ports:
            if re.match('^([0-9]+-)?%s(-[0-9]+)?$' % port, exposed):
                return True
    if not port_exposed(edge_port):
        warns.append('Edge port %s is not exposed. You may have to add the entry to the "ports" section of the docker-compose file.' % edge_port)
...
def extract_port_flags(user_flags, port_mappings: PortMappings):
    ...
    if matches:
        ...
        user_flags = re.sub(regex, '', user_flags)
    return user_flags
...
class LocalstackContainerServer(Server):
    ...
    def do_run(self):
        if DOCKER_CLIENT.is_container_running(self.container.name):
            raise ContainerExists('LocalStack container named "%s" is already running' % self.container.name)
        return self.container.run()

    def do_shutdown(self):
        try:
            CmdDockerClient().stop_container(self.container.name, timeout=10)
...
def prepare_docker_start():
    ...
    if config.dirs.tmp != config.dirs.functions and (not config.LAMBDA_REMOTE_DOCKER):
        print(f'WARNING: The detected temp folder for localstack ({config.dirs.tmp}) is not equal to the HOST_TMP_FOLDER environment variable set ({config.dirs.functions}).')
    os.environ[ENV_SCRIPT_STARTING_DOCKER] = '1'
    mkdir(config.dirs.tmp)
    try:
        chmod_r(config.dirs.tmp, 511)
    except Exception:
        pass
...
def configure_container(container: LocalstackContainer):
    ...
    container.env_vars['HOST_TMP_FOLDER'] = config.dirs.functions
