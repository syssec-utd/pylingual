...
class SegmentationLoss:
    ...
    def __init__(self, label_smoothing, class_weights, ignore_label, use_groundtruth_dimension, use_binary_cross_entropy=False, top_k_percent_pixels=1.0, gt_is_matting_map=False):
    ...
    """
        ...
        if not self._use_binary_cross_entropy:
            if num_layers > 1:
                raise ValueError('Groundtruth mask must have only 1 layer if using categoricalcross entropy, but got {} layers.'.format(num_layers))
        if self._gt_is_matting_map:
            if num_classes != 2:
                raise ValueError('Groundtruth matting map only supports 2 classes, but got {} classes.'.format(num_classes))
            if num_layers > 1:
                raise ValueError('Groundtruth matting map must have only 1 layer, but got {} layers.'.format(num_layers))
        class_weights = self._class_weights if self._class_weights else [1] * num_classes
        if num_classes != len(class_weights):
            raise ValueError('Length of class_weights should be {}'.format(num_classes))
        class_weights = tf.constant(class_weights, tf.float32)
        if self._use_groundtruth_dimension:
            logits = tf.image.resize(logits, tf.shape(labels)[1:3], method=tf.image.ResizeMethod.BILINEAR)
        else:
            labels = tf.image.resize(labels, (height, width), method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
        if not self._gt_is_matting_map:
            labels = tf.cast(labels, tf.int32)
        valid_mask = tf.not_equal(tf.cast(labels, tf.int32), self._ignore_label)
        labels_with_prob = self.get_labels_with_prob(logits, labels, valid_mask, **kwargs)
        if self._use_binary_cross_entropy:
            cross_entropy_loss = tf.nn.sigmoid_cross_entropy_with_logits(labels=labels_with_prob, logits=logits)
            cross_entropy_loss = tf.reduce_sum(cross_entropy_loss * class_weights, axis=-1)
        else:
            cross_entropy_loss = tf.nn.softmax_cross_entropy_with_logits(labels=labels_with_prob, logits=logits)
            if self._gt_is_matting_map:
                labels = utils.binarize_matting_map(labels)
            weight_mask = tf.einsum('...y,y->...', tf.one_hot(tf.cast(tf.squeeze(labels, axis=-1), tf.int32), depth=num_classes, dtype=tf.float32), class_weights)
    ...
    """
        ...
        if self._gt_is_matting_map:
            ...
            if self._use_binary_cross_entropy:
                one_hot_labels_per_layer = tf.one_hot(labels, depth=num_classes, dtype=tf.float32)
                train_labels = tf.reduce_max(one_hot_labels_per_layer, axis=-2)
            else:
                train_labels = tf.one_hot(tf.squeeze(labels, axis=-1), depth=num_classes, dtype=tf.float32)
        return train_labels * (1 - self._label_smoothing) + self._label_smoothing / num_classes
    ...
    """
        ...
        top_k_pixels = tf.cast(self._top_k_percent_pixels * tf.cast(tf.size(pixelwise_loss), tf.float32), tf.int32)
        top_k_losses, _ = tf.math.top_k(pixelwise_loss, k=top_k_pixels, sorted=True)
        normalizer = tf.reduce_sum(tf.cast(tf.not_equal(top_k_losses, 0.0), tf.float32)) + EPSILON
...
def get_actual_mask_scores(logits, labels, ignore_label):
    ...
    one_hot_predictions = tf.one_hot(flat_predictions, num_classes, on_value=True, off_value=False)
    one_hot_labels = tf.one_hot(flat_labels, num_classes, on_value=True, off_value=False)
...
class MaskScoringLoss:
    ...
    def __init__(self, ignore_label):
        self._ignore_label = ignore_label
        self._mse_loss = tf.keras.losses.MeanSquaredError(reduction=tf.keras.losses.Reduction.NONE)
