...
def _df_eq(df: DataFrame, data: Any, schema: Any=None, metadata: Any=None, digits=8, check_order: bool=False, check_schema: bool=True, check_content: bool=True, check_metadata: bool=True, no_pandas: bool=False, throw=False) -> bool:
    ...
    try:
        ...
        assert not check_metadata or df.metadata == df2.metadata, f'metadata mismatch {df.metadata}, {df2.metadata}'
        ...
        pd.testing.assert_frame_equal(d1, d2, check_less_precise=digits, check_dtype=False)
        return True
...
def to_local_df(df: Any, schema: Any=None, metadata: Any=None) -> LocalDataFrame:
    ...
    if isinstance(df, DataFrame):
        aot(schema is None and metadata is None, ValueError('schema and metadata must be None when df is a DataFrame'))
        return df.as_local()
...
def to_local_bounded_df(df: Any, schema: Any=None, metadata: Any=None) -> LocalBoundedDataFrame:
...
def serialize_df(df: Optional[DataFrame], threshold: int=-1, file_path: Optional[str]=None, fs: Optional[FileSystem]=None) -> str:
    ...
    if threshold < 0 or size <= threshold:
        ...
        if fs is None:
            with open_fs(os.path.dirname(file_path), writeable=True, create=False) as _fs:
...
def unpickle_df(stream: bytes) -> LocalBoundedDataFrame:
    ...
    raise NotImplementedError(f'{o[1]} is not supported for unpickle')

def deserialize_df(json_str: str, fs: Optional[FileSystem]=None) -> Optional[LocalBoundedDataFrame]:
...
def get_join_schemas(df1: DataFrame, df2: DataFrame, how: str, on: Iterable[str]) -> Tuple[Schema, Schema]:
    ...
    if how != 'cross' and len(on) == 0:
        ...
        aot(len(on) > 0, lambda: SchemaError(f'no common columns between {df1.schema} and {df2.schema}'))
    schema2 = df2.schema
    ...
    if how == 'cross':
        aot(len(df1.schema.intersect(schema2)) == 0, SchemaError("can't specify on for cross join"))
    else:
