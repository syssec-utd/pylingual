...
class ApiPipelineBlockHandler(BaseHandler):
    ...
    def get(self, pipeline_uuid, block_uuid):
        ...
        self.write(dict(block=block.to_dict(include_content=True, include_outputs=include_outputs)))
        self.finish()
...
class ApiPipelineBlockListHandler(BaseHandler):
    ...
    def get(self, pipeline_uuid):
        pipeline = Pipeline.get(pipeline_uuid)
        include_outputs = self.get_bool_argument('include_outputs', True)
        self.write(dict(blocks=pipeline.to_dict(include_content=True, include_outputs=include_outputs, sample_count=DATAFRAME_SAMPLE_COUNT_PREVIEW)['blocks']))
        self.finish()
    ...
    def post(self, pipeline_uuid):
        ...
        block = Block.create(payload.get('name') or payload.get('uuid'), payload.get('type'), get_repo_path(), config=payload.get('config'), configuration=payload.get('configuration'), language=payload.get('language'), pipeline=pipeline, priority=payload.get('priority'), upstream_block_uuids=payload.get('upstream_blocks', []))
