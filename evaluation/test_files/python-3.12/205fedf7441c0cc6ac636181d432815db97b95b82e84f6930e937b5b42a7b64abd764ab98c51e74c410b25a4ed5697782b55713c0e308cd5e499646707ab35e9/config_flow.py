...
from homeassistant.const import CONF_CLIENT_ID, CONF_HOST, CONF_PORT, CONF_SOURCE, CONF_SSL, CONF_TOKEN, CONF_URL, CONF_VERIFY_SSL
from homeassistant.core import callback
from homeassistant.data_entry_flow import FlowResult
from homeassistant.helpers import discovery_flow
from homeassistant.helpers.aiohttp_client import async_get_clientsession
import homeassistant.helpers.config_validation as cv
from .const import AUTH_CALLBACK_NAME, AUTH_CALLBACK_PATH, AUTOMATIC_SETUP_STRING, CONF_IGNORE_NEW_SHARED_USERS, CONF_IGNORE_PLEX_WEB_CLIENTS, CONF_MONITORED_USERS, CONF_SERVER, CONF_SERVER_IDENTIFIER, CONF_USE_EPISODE_ART, DEFAULT_PORT, DEFAULT_SSL, DEFAULT_VERIFY_SSL, DOMAIN, MANUAL_SETUP_STRING, PLEX_SERVER_CONFIG, SERVERS, X_PLEX_DEVICE_NAME, X_PLEX_PLATFORM, X_PLEX_PRODUCT, X_PLEX_VERSION
...
def configured_servers(hass):
    ...
    return {entry.data[CONF_SERVER_IDENTIFIER] for entry in hass.config_entries.async_entries(DOMAIN)}
...
async def async_discover(hass):
    ...
    for server_data in gdm.entries:
        discovery_flow.async_create_flow(hass, DOMAIN, context={CONF_SOURCE: config_entries.SOURCE_INTEGRATION_DISCOVERY}, data=server_data)
...
class PlexFlowHandler(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    @staticmethod
    @callback
    def async_get_options_flow(config_entry: config_entries.ConfigEntry) -> PlexOptionsFlowHandler:
    ...
    async def async_step_user_advanced(self, user_input=None, errors=None):
        ...
        if user_input is not None:
            ...
            return await self.async_step_plex_website_auth()
        data_schema = vol.Schema({vol.Required('setup_method', default=AUTOMATIC_SETUP_STRING): vol.In([AUTOMATIC_SETUP_STRING, MANUAL_SETUP_STRING])})
    ...
    async def async_step_manual_setup(self, user_input=None, errors=None):
        ...
        if user_input is not None and errors is None:
            ...
            if (host := user_input.get(CONF_HOST)):
                ...
                return await self.async_step_manual_setup(user_input=user_input, errors={'base': 'host_or_token'})
            return await self.async_step_server_validate(user_input)
        previous_input = user_input or {}
        data_schema = vol.Schema({vol.Optional(CONF_HOST, description={'suggested_value': previous_input.get(CONF_HOST)}): str, vol.Required(CONF_PORT, default=previous_input.get(CONF_PORT, DEFAULT_PORT)): int, vol.Required(CONF_SSL, default=previous_input.get(CONF_SSL, DEFAULT_SSL)): bool, vol.Required(CONF_VERIFY_SSL, default=previous_input.get(CONF_VERIFY_SSL, DEFAULT_VERIFY_SSL)): bool, vol.Optional(CONF_TOKEN, description={'suggested_value': previous_input.get(CONF_TOKEN)}): str})
        return self.async_show_form(step_id='manual_setup', data_schema=data_schema, errors=errors)
    ...
    async def async_step_server_validate(self, server_config):
        ...
        try:
            ...
            server_identifier = server_config.get(CONF_URL) or plex_server.server_choice or 'Unknown'
        ...
        if errors:
            if self._manual:
                return await self.async_step_manual_setup(user_input=server_config, errors=errors)
        ...
        if url.startswith('https'):
            entry_config[CONF_VERIFY_SSL] = server_config.get(CONF_VERIFY_SSL, DEFAULT_VERIFY_SSL)
        data = {CONF_SERVER: plex_server.friendly_name, CONF_SERVER_IDENTIFIER: server_id, PLEX_SERVER_CONFIG: entry_config}
    ...
    async def async_step_select_server(self, user_input=None):
        ...
        available_servers = {server_id: f'{name} ({owner})' if owner else name for name, server_id, owner in self.available_servers if server_id not in configured}
        if not available_servers:
            return self.async_abort(reason='all_configured')
        if len(available_servers) == 1:
            config[CONF_SERVER_IDENTIFIER] = next(iter(available_servers))
            return await self.async_step_server_validate(config)
        return self.async_show_form(step_id='select_server', data_schema=vol.Schema({vol.Required(CONF_SERVER_IDENTIFIER): vol.In(available_servers)}), errors={})
    ...
    async def async_step_integration_discovery(self, discovery_info):
        ...
        self.context['title_placeholders'] = {'host': host, 'name': name}
    ...
    async def async_step_plex_website_auth(self):
        ...
        payload = {'X-Plex-Device-Name': X_PLEX_DEVICE_NAME, 'X-Plex-Version': X_PLEX_VERSION, 'X-Plex-Product': X_PLEX_PRODUCT, 'X-Plex-Device': self.hass.config.location_name, 'X-Plex-Platform': X_PLEX_PLATFORM, 'X-Plex-Model': 'Plex OAuth'}
    ...
    async def async_step_reauth(self, entry_data: Mapping[str, Any]) -> FlowResult:
        ...
        self._reauth_config = {CONF_SERVER_IDENTIFIER: entry_data[CONF_SERVER_IDENTIFIER]}
        return await self.async_step_user()
...
class PlexOptionsFlowHandler(config_entries.OptionsFlow):
    ...
    async def async_step_plex_mp_settings(self, user_input=None):
        ...
        if user_input is not None:
            self.options[MP_DOMAIN][CONF_USE_EPISODE_ART] = user_input[CONF_USE_EPISODE_ART]
            self.options[MP_DOMAIN][CONF_IGNORE_NEW_SHARED_USERS] = user_input[CONF_IGNORE_NEW_SHARED_USERS]
            self.options[MP_DOMAIN][CONF_IGNORE_PLEX_WEB_CLIENTS] = user_input[CONF_IGNORE_PLEX_WEB_CLIENTS]
            account_data = {user: {'enabled': bool(user in user_input[CONF_MONITORED_USERS])} for user in plex_server.accounts}
        ...
        if known_accounts:
            default_accounts = {user for user in plex_server.option_monitored_users if plex_server.option_monitored_users[user]['enabled']}
        ...
        return self.async_show_form(step_id='plex_mp_settings', data_schema=vol.Schema({vol.Required(CONF_USE_EPISODE_ART, default=plex_server.option_use_episode_art): bool, vol.Optional(CONF_MONITORED_USERS, default=default_accounts): cv.multi_select(available_accounts), vol.Required(CONF_IGNORE_NEW_SHARED_USERS, default=plex_server.option_ignore_new_shared_users): bool, vol.Required(CONF_IGNORE_PLEX_WEB_CLIENTS, default=plex_server.option_ignore_plexweb_clients): bool}))
...
class PlexAuthorizationCallbackView(HomeAssistantView):
    ...
    async def get(self, request):
        ...
        await hass.config_entries.flow.async_configure(flow_id=request.query['flow_id'], user_input=None)
        return web_response.Response(headers={'content-type': 'text/html'}, text='<script>window.close()</script>Success! This window can be closed')