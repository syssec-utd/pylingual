...
class AnchorGenerator(CustomPaddleOp):
    ...
    def compute_shapes(self):
        ...
        for r in range(len(self.aspect_ratios)):
            ...
            for s in range(len(self.anchor_sizes)):
                ...
                w = scale_w * base_w
                h = scale_h * base_h
                shapes.append([-0.5 * (w - 1), -0.5 * (h - 1), 0.5 * (w - 1), 0.5 * (h - 1)])
        return shapes
    ...
    def forward(self):
        ...
        tensor_len_shape = paddle.full(shape=[1], fill_value=len(self.shapes), dtype='int64')
        ...
        variance_tensor = paddle.assign(np.asarray(self.variances).astype('float32'))
        vars = paddle.reshape(variance_tensor, shape=[1, 1, 1, -1])
        vars = paddle.tile(vars, repeat_times=(h, w, tensor_len_shape, tensor_one))
...
class Anchors_generator:
    ...
    def opset_1(cls, graph, node, **kw):
        node = graph.make_node('anchor_generator', inputs=node.input('Input'), outputs=node.output('Anchors') + node.output('Variances'), anchor_sizes=node.attr('anchor_sizes'), aspect_ratios=node.attr('aspect_ratios'), offset=node.attr('offset'), strides=node.attr('stride'), variances=node.attr('variances'), domain='custom')
