...
data_dict = {}
main_dict = {}
DDL_REGEX = recompile('DDL\\(([^),]+)\\, (([^),]+)), (([^),]+)), (([^),]+))\\)')
POST_ID_REGEX = recompile('"postId":"(\\d+)"')
...
def scrapper(update, context):
    ...
    try:
        link = rematch('^(?i)\\b((?:https?://|www\\d{0,3}[.]|[a-z0-9.\\-]+[.][a-z]{2,4}/)(?:[^\\s()<>]+|\\(([^\\s()<>]+|(\\([^\\s()<>]+\\)))*\\))+(?:\\(([^\\s()<>]+|(\\([^\\s()<>]+\\)))*\\)|[^\\s`!()\\[\\]{};:\'\\".,<>?«»“”‘’]))', link)[0]
    except TypeError:
    ...
    if 'sharespark' in link:
        ...
        for match in payload_data_matches:
            ...
            payload = {'action': 'DDL', 'post_id': post_id, 'div_id': payload_data[0].strip(), 'tab_id': payload_data[1].strip(), 'num': payload_data[2].strip(), 'folder': payload_data[3].strip()}
            del payload['num']
        ...
        res = rget(link)
        soup = BeautifulSoup(res.text, 'html.parser')
        mystx = soup.select('a[href^="magnet:?xt=urn:btih:"]')
        for hy in mystx:
...
def looper(dict_key, click):
    ...
    data = {'action': 'DDL', 'post_id': post_id, 'div_id': payload_data[0].strip(), 'tab_id': payload_data[1].strip(), 'num': payload_data[2].strip(), 'folder': payload_data[3].strip()}
...
def ouo_parse(dict_key, button, loop_soup):
    try:
        ouo_encrypt = recompile('openInNewTab\\(([^),]+\\))').search(str(loop_soup)).group(0).strip().split('"')[1]
...
def indexScrape(payload_input, url, auth, folder_mode=False):
    ...
    try:
        decrypted_response = json.loads(b64decode(encrypted_response.text[::-1][24:-20]).decode('utf-8'))
    except:
    ...
    if not folder_mode:
        ...
        return (result, False)
srp_handler = CommandHandler(BotCommands.ScrapeCommand, scrapper, filters=CustomFilters.authorized_chat | CustomFilters.authorized_user)
