...
class WatchdogConfig(object):
    ...
    def __init__(self, config):
        ...
        self.driver_config = dict(((k, v) for k, v in config['watchdog'].items() if k not in ['mode', 'safety_margin', 'driver']))

    def __eq__(self, other):
        return isinstance(other, WatchdogConfig) and all((getattr(self, attr) == getattr(other, attr) for attr in ['mode', 'ttl', 'loop_wait', 'safety_margin', 'driver', 'driver_config']))
...
class Watchdog(object):
    ...
    def reload_config(self, config):
        ...
        if self.config.mode == MODE_OFF:
            ...
            self.impl = NullWatchdog()
        if not self.active:
            if self.config.driver != self.active_config.driver or self.config.driver_config != self.active_config.driver_config:
                self.impl = self.config.get_impl()
            self.active_config = self.config
    ...
    def _activate(self):
        ...
        if self.config.timing_slack < 0:
            logger.warning('Watchdog not supported because leader TTL {0} is less than 2x loop_wait {1}'.format(self.config.ttl, self.config.loop_wait))
            self.impl = NullWatchdog()
        try:
            self.impl.open()
        ...
        if not self.impl.is_running or actual_timeout > self.config.timeout:
            if self.config.mode == MODE_REQUIRED:
                if self.impl.is_null:
                    ...
                    logger.error('Configuration requires watchdog, but a safe watchdog timeout {0} could not be configured. Watchdog timeout is {1}.'.format(self.config.timeout, actual_timeout))
                return False
            elif not self.impl.is_null:
                logger.warning('Watchdog timeout {0} seconds does not ensure safe termination within {1} seconds'.format(actual_timeout, self.config.timeout))
        if self.is_running:
            logger.info('{0} activated with {1} second timeout, timing slack {2} seconds'.format(self.impl.describe(), actual_timeout, self.config.timing_slack))
        elif self.config.mode == MODE_REQUIRED:
    ...
    def _set_timeout(self):
        ...
        if self.impl.is_running and actual_timeout < self.config.loop_wait:
            logger.error('loop_wait of {0} seconds is too long for watchdog {1} second timeout'.format(self.config.loop_wait, actual_timeout))
    ...
    def _disable(self):
        try:
            if self.impl.is_running and (not self.impl.can_be_disabled):
                self.impl.keepalive()
                logger.warning("Watchdog implementation can't be disabled. System will reboot after {0} seconds when watchdog times out.".format(self.impl.get_timeout()))
            self.impl.close()
        except WatchdogError as e:
        ...
        try:
            ...
            if self.active and self.config != self.active_config:
                ...
                if self.config.timeout != self.active_config.timeout:
                    self.impl.set_timeout(self.config.timeout)
                    if self.is_running:
                        logger.info('{0} updated with {1} second timeout, timing slack {2} seconds'.format(self.impl.describe(), self.impl.get_timeout(), self.config.timing_slack))
                self.active_config = self.config
        except WatchdogError as e:
