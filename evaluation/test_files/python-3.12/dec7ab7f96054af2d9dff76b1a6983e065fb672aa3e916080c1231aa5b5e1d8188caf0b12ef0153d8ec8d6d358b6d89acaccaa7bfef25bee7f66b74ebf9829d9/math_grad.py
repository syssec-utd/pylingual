  ...
  Returns:
    A pair of tuples, containing:
      ...
      * A 3-tuple of broadcast information for y, containing the respective
        details for y.
  """
    if context.executing_eagerly() or not (isinstance(x, ops.Tensor) and isinstance(y, ops.Tensor) and isinstance(grad, ops.Tensor)):
...
def _MulGrad(op, grad):
    ...
    try:
        ...
        pass
    x = op.inputs[0]
    if isinstance(grad, ops.Tensor) and _ShapesFullySpecifiedAndEqual(x, y, grad) and (grad.dtype in (dtypes.int32, dtypes.float32)):
