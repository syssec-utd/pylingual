...
@bind_hass
async def async_handle(hass: HomeAssistant, platform: str, intent_type: str, slots: _SlotsType | None=None, text_input: str | None=None, context: Context | None=None, language: str | None=None) -> IntentResponse:
    """Handle an intent."""
    ...
    intent = Intent(hass, platform, intent_type, slots or {}, text_input, context, language)
    try:
...
class ServiceIntentHandler(IntentHandler):
    ...
    async def async_handle(self, intent_obj: Intent) -> IntentResponse:
        ...
        states = list(async_match_states(hass, name=name, area=area, domains=domains, device_classes=device_classes))
        if not states:
    ...
    async def async_call_service(self, intent_obj: Intent, state: State) -> None:
        ...
        hass = intent_obj.hass
        await hass.services.async_call(self.domain, self.service, {ATTR_ENTITY_ID: state.entity_id}, context=intent_obj.context, blocking=True, limit=self.service_timeout)
...
class Intent:
    """Hold the intent."""
    __slots__ = ['hass', 'platform', 'intent_type', 'slots', 'text_input', 'context', 'language', 'category']

    def __init__(self, hass: HomeAssistant, platform: str, intent_type: str, slots: _SlotsType, text_input: str | None, context: Context, language: str, category: IntentCategory | None=None) -> None:
