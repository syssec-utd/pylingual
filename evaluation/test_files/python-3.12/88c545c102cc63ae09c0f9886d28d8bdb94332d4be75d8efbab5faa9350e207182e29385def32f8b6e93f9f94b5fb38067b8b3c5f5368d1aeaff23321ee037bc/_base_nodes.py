...
class ImportNode(FilterStmtsBaseNode, NoChildrenNode, Statement):
    ...
    def real_name(self, asname):
        ...
        for name, _asname in self.names:
            ...
            if asname == _asname:
                return name
        raise AttributeInferenceError('Could not find original name for {attribute} in {target!r}', target=self, attribute=asname)
...
class MultiLineBlockNode(NodeNG):
    ...
    def _get_assign_nodes(self):
        children_assign_nodes = (child_node._get_assign_nodes() for block in self._multi_line_blocks for child_node in block)
