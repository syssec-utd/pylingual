...
from pyoverkiz.exceptions import BadCredentialsException, MaintenanceException, TooManyAttemptsBannedException, TooManyRequestsException
...
class ConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_validate_input(self, user_input: dict[str, Any]) -> None:
        ...
        client = OverkizClient(username=username, password=password, server=server, session=session)
    ...
    async def async_step_user(self, user_input: dict[str, Any] | None=None) -> FlowResult:
        ...
        if user_input:
            ...
            try:
                ...
                if self._config_entry:
                    ...
                    self.hass.config_entries.async_update_entry(self._config_entry, data={**self._config_entry.data, **user_input})
        ...
        return self.async_show_form(step_id='user', data_schema=vol.Schema({vol.Required(CONF_USERNAME, default=self._default_user): str, vol.Required(CONF_PASSWORD): str, vol.Required(CONF_HUB, default=self._default_hub): vol.In({key: hub.name for key, hub in SUPPORTED_SERVERS.items()})}), errors=errors)
    ...
    async def async_step_reauth(self, entry_data: Mapping[str, Any]) -> FlowResult:
        """Handle reauth."""
        self._config_entry = cast(ConfigEntry, self.hass.config_entries.async_get_entry(self.context['entry_id']))
        self.context['title_placeholders'] = {'gateway_id': self._config_entry.unique_id}
