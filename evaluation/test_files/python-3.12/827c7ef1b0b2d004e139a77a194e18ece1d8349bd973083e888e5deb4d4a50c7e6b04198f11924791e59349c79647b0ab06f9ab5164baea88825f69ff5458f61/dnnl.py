...
def make_dnnl_pattern(op_name, with_bias, with_eltwise):
    ...
    if 'conv' in op_name:
        ...
        logger.warning('Currently, only conv1d, conv2d, conv2d_transpose, conv3d_transpose, dense op are supported, but got %s.', op_name)
        dnnl_pattern = ()
    return dnnl_pattern
...
def make_qnn_conv2d_pattern():
    ...
    pat = is_op('clip')(pat)
    pat = is_op('multiply')(pat, act_scl) | pat
    pat = is_op('add')(pat, sum_scl * is_op('cast')(sum_src)) | pat
    pat = is_op('add')(pat, dst_zp) | pat
    pat = is_op('cast')(pat)
    ...
    for with_bias in [True, False]:
        for elt in elt_list:
            if not with_bias and (not elt):
                continue
            for conv_name in ['nn.conv1d', 'nn.conv2d', 'nn.conv3d', 'nn.conv2d_transpose', 'nn.conv3d_transpose']:
                if elt != 'gelu':
...
def get_optimal_layout_for_conv(data_layout, kernel_layout, weight_shape, out_shape, paddings, strides, dilates, groups, dtype):
    ...
    layouts : string
              The result.
    """
    return _ffi_api.get_optimal_layout_for_conv(data_layout, kernel_layout, weight_shape, out_shape, paddings, strides, dilates, groups, dtype)

def get_optimal_layout_for_conv_transpose(data_layout, kernel_layout, weight_shape, out_shape, paddings, output_paddings, strides, dilates, groups, dtype):
    ...
    layouts : string
              The result.
    """
    return _ffi_api.get_optimal_layout_for_conv_transpose(data_layout, kernel_layout, weight_shape, out_shape, paddings, output_paddings, strides, dilates, groups, dtype)
...
def alter_conv(attrs, inputs, tinfos, out_type):
    ...
    res = get_optimal_layout_for_conv(attrs['data_layout'], attrs['kernel_layout'], weight_shape, out_shape, paddings, strides, dilates, groups, dtype)
...
def alter_conv_transpose(attrs, inputs, tinfos, out_type):
    ...
    new_attrs = dict(attrs)
    conv_type = type(attrs).__name__.split('Attrs')[0]
    res = get_optimal_layout_for_conv_transpose(attrs['data_layout'], attrs['kernel_layout'], weight_shape, out_shape, paddings, output_paddings, strides, dilates, groups, dtype)
    src_df, weight_df, dst_df = res.split(',')
    new_attrs['data_layout'] = tag2layout(src_df, is_weight=False, conv_type=conv_type)
    new_attrs['kernel_layout'] = tag2layout(weight_df, is_weight=True, conv_type=conv_type)
    new_attrs['out_layout'] = tag2layout(dst_df, is_weight=False, conv_type=conv_type)
...
class IsComputeIntensiveGraph(ExprVisitor):
    ...
    def visit_call(self, call):
        compute_intensive_ops = set(['nn.conv1d', 'nn.conv2d', 'nn.conv2d_transpose', 'nn.conv3d', 'nn.conv3d_transpose', 'nn.dense', 'nn.layer_norm', 'nn.batch_matmul'])
...
class LayerNormRewrite(DFPatternCallback):
    ...
    def __init__(self):
        ...
        eps = is_expr(relay.const(1e-05)) | is_expr(relay.const(1e-06))
...
def rewrite_dense_bias_gelu_reshape_last(mod):
    ...
    them to byoc part.
    """
    mod['main'] = rewrite([DenseReshapeBiasGeluRewrite(), DenseReshapeBiasGeluRewrite(has_gelu=False)], mod['main'])
    return mod
...
class LegalizeQnnOpForDnnl(DFPatternCallback):
    ...
    def __init__(self):
        ...
        self.root = (is_op('qnn.conv2d') | is_op('qnn.dense'))(self.src, self.wgh, self.src_zp, self.wgh_zp, self.src_scl, self.wgh_scl)
        pat = is_op('add')(self.root, self.bias) | self.root
        pat = is_op('qnn.requantize')(pat, self.rq_in_scl, self.rq_in_zp, self.rq_out_scl, self.rq_out_zp)
        pat = is_op('clip')(pat)
        cast = is_op('cast')(pat)
        pat = is_op('qnn.add')(cast, self.sum_src, self.sum_lhs_scl, self.sum_lhs_zp, self.sum_rhs_scl, self.sum_rhs_zp, self.sum_out_scl, self.sum_out_zp)
        pat = is_op('clip')(pat)
    ...
    def callback(self, pre, post, node_map):
        ...
        act_scl = sum_lhs_scl / sum_out_scl
        sum_scl = sum_rhs_scl / sum_out_scl
        dst_zp = cast_fp(sum_out_zp) - cast_fp(sum_lhs_zp) * sum_lhs_scl / sum_out_scl - cast_fp(sum_rhs_zp) * sum_rhs_scl / sum_out_scl
        bias = self.squeeze_bias(bias, dst_layout)
        bias = cast_fp(bias) - cast_fp(self.fake_op(src_zp, wgh, wgh_layout)) - cast_fp(rq_in_zp) + cast_fp(rq_out_zp) * rq_out_scl / rq_in_scl
        bias = self.broadcast_to_rank(bias, bias_rank)
        zero_zp = relay.const(0, dtype='int32')
        one_scl = relay.const(1.0, dtype='float32')
        gr = tvm.relay.Call(root.op, [src, wgh, zero_zp, zero_zp, one_scl, one_scl], root.attrs, root.type_args, root.span)
    ...
    def fake_op(zp, wgh, layout):
        ...
        reduced_kernel = relay.op.sum(wgh_int, axis=[layout.index('O')], keepdims=False, exclude=True)
...
def legalize_qnn_for_dnnl(mod):
    ...
    seq = tvm.transform.Sequential([transform.InferType(), transform.FoldConstant()])
