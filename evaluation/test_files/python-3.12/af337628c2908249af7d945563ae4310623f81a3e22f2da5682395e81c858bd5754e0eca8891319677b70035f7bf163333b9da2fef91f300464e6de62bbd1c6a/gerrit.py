...
from reviewboard.hostingsvcs.service import HostingService, HostingServiceClient, HostingServiceHTTPResponse
    ...
    def json(self):
        ...
        if data:
            if data.startswith(self._JSON_PREFIX):
                ...
                logger.error('JSON response from Gerrit URL %s does not begin with %r: %s', self.url, self._JSON_PREFIX, data)
        return data
...
class Gerrit(HostingService):
    ...
    def check_repository(self, gerrit_url=None, gerrit_project_name=None, *args, **kwargs):
        ...
        try:
            ...
            if e.http_code == 404:
                raise RepositoryError(gettext('The project "%s" does not exist or you do not have access to it.') % gerrit_project_name)
            raise
        ...
        try:
            ...
            raise RepositoryError(gettext('Could not retrieve the list of Gerrit plugins from %(url).') % {'url': url})
        if 'gerrit-reviewboard' not in rsp:
            raise RepositoryError(gettext('The "gerrit-reviewboard" plugin is not installed on the server. See %(plugin_url)s for installation instructions.') % {'plugin_url': _PLUGIN_URL})
        else:
            ...
            try:
                ...
                raise RepositoryError(gettext('Could not parse gerrit-reviewboard version: "%(version)s" from URL %(url)s: %(error)s') % {'version': version, 'error': e, 'url': url})
            if version < self.REQUIRED_PLUGIN_VERSION:
                raise RepositoryError(gettext('The "gerrit-reviewboard" plugin on the server is an incompatible version: found %(found)s but version %(required)s or higher is required.') % {'found': version_str, 'required': self.REQUIRED_PLUGIN_VERSION_STR})
    ...
    def authorize(self, username, password, credentials, local_site_name=None, gerrit_url=None, *args, **kwargs):
        ...
        try:
            ...
            if e.http_code != 401:
                ...
                raise AuthorizationError(gettext('Could not authenticate with Gerrit at %(url): %(error)s') % {'url': url, 'error': e.message}, http_code=e.http_code)
            raise AuthorizationError(gettext('Unable to authenticate to Gerrit at %(url)s. The username or password used may be invalid.') % {'url': url}, http_code=e.http_code)
    ...
    def get_commits(self, repository, branch=None, start=None, limit=None):
        ...
        if not start:
            start = branch
        query = {field: str(value).encode('utf-8') for field, value in (('start', start), ('limit', limit)) if value is not None}
        url = self._build_project_api_url(repository, 'all-commits')
        try:
            rsp = self.client.http_get(url, query=query).json
        except HostingServiceAPIError as e:
            if limit == 1:
                raise HostingServiceAPIError(gettext('Could not retrieve commit "%(rev)s": %(error)s') % {'rev': start, 'error': e.message}, http_code=e.http_code, rsp=e.rsp)
            else:
                raise HostingServiceAPIError(gettext('Could not retrieve commits starting at "%(rev)s": %(error)s') % {'rev': start, 'error': e.message}, http_code=e.getcode(), rsp=e.rsp)
    ...
    def get_file(self, repository, path, revision, *args, **kwargs):
        ...
        try:
            ...
            raise HostingServiceAPIError(gettext('Could not get file "%(file)s" at revision "%(rev)s": %(error)s') % {'file': path, 'rev': revision, 'error': e.read()}, http_code=e.http_code)
        try:
            return base64.b64decode(rsp)
        except Exception as e:
            raise HostingServiceAPIError(gettext('An error occurred while retrieving "%(file)s" at revision "%(rev)s" from Gerrit: the response could not be decoded: %(error)s') % {'file': path, 'rev': revision, 'error': e})
    ...
    def get_change(self, repository, revision):
        ...
        try:
            ...
            raise RepositoryError(gettext('Could not retrieve change "%(rev)s" from repository %(repo)d: %(error)s"') % {'rev': revision, 'repo': repository.id, 'error': e})
        commit.diff = diff
        return commit
    ...
    def _build_project_api_url(self, repository, *rest_parts):
        ...
        Returns:
            ...
            The full URL.
        """
        parts = ['a', 'projects', quote_plus(repository.extra_data['gerrit_project_name'])] + list(rest_parts)
        return '%s/' % urljoin(repository.extra_data['gerrit_url'], '/'.join(parts))