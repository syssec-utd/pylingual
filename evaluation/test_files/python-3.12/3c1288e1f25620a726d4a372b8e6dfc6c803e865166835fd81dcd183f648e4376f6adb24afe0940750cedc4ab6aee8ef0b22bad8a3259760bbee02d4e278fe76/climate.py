...
from homeassistant.components.climate import PRESET_BOOST, PRESET_NONE, ClimateEntity, ClimateEntityFeature, HVACAction, HVACMode
...
from .const import ATTR_TIME_PERIOD, DOMAIN, SERVICE_BOOST_HEATING_OFF, SERVICE_BOOST_HEATING_ON
HIVE_TO_HASS_STATE = {'SCHEDULE': HVACMode.AUTO, 'MANUAL': HVACMode.HEAT, 'OFF': HVACMode.OFF}
HASS_TO_HIVE_STATE = {HVACMode.AUTO: 'SCHEDULE', HVACMode.HEAT: 'MANUAL', HVACMode.OFF: 'OFF'}
HIVE_TO_HASS_HVAC_ACTION = {'UNKNOWN': HVACAction.OFF, False: HVACAction.IDLE, True: HVACAction.HEATING}
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry, async_add_entities: AddEntitiesCallback) -> None:
    ...
    platform.async_register_entity_service('boost_heating', {vol.Required(ATTR_TIME_PERIOD): vol.All(cv.time_period, cv.positive_timedelta, lambda td: td.total_seconds() // 60), vol.Optional(ATTR_TEMPERATURE, default='25.0'): vol.Coerce(float)}, 'async_heating_boost')
    platform.async_register_entity_service(SERVICE_BOOST_HEATING_ON, {vol.Required(ATTR_TIME_PERIOD): vol.All(cv.time_period, cv.positive_timedelta, lambda td: td.total_seconds() // 60), vol.Optional(ATTR_TEMPERATURE, default='25.0'): vol.Coerce(float)}, 'async_heating_boost_on')
    platform.async_register_entity_service(SERVICE_BOOST_HEATING_OFF, {}, 'async_heating_boost_off')
...
class HiveClimateEntity(HiveEntity, ClimateEntity):
    ...
    async def async_heating_boost(self, time_period, temperature):
        ...
        _LOGGER.warning('Hive Service heating_boost will be removed in 2021.7.0, please update to heating_boost_on')
    ...
    async def async_update(self) -> None:
        ...
        if self._attr_available:
            ...
            self._attr_hvac_action = HIVE_TO_HASS_HVAC_ACTION[self.device['status']['action']]
