...
def num_clients() -> int:
    ...
    if num_clients_value <= 0:
        raise ValueError(f'Environment variable {_DT_NUM_CLIENTS} must be > 0, got {num_clients_value}.')
...
def local_devices(device_type: str, for_client_id: Optional[int]=None) -> List[tf_device.DeviceSpec]:
    ...
    logical_devices = [tf_device.DeviceSpec.from_string(d.name) for d in tf_config.list_logical_devices(device_type)]
    device_count = 0
    for d in logical_devices:
        if (d.job is None or d.job == job_name()) and (d.task is None or d.task == for_client_id):
            device_count = device_count + 1
    return [tf_device.DeviceSpec(job=job_name(), replica=0, task=for_client_id, device_type=device_type, device_index=i) for i in range(device_count)]
