...
from typing import TYPE_CHECKING, Any, AsyncGenerator, Coroutine, Dict, Literal, Optional, Union, cast
...
async def async_file_iterator(file_path: 'PathType', chunk_size: int, adapter: 'FileSystemAdapter') -> AsyncGenerator[bytes, None]:
...
def create_etag_for_file(path: 'PathType', modified_time: float, file_size: int) -> str:
    ...
    Returns:
        An etag.
    """
    check = adler32(str(path).encode('utf-8')) & 4294967295
    return f'"{modified_time}-{file_size}-{check}"'

class FileResponse(StreamingResponse):
    """A response, streaming a file as response body."""
    __slots__ = ('chunk_size', 'content_disposition_type', 'etag', 'file_path', 'filename', 'adapter', 'file_info')

    def __init__(self, path: Union[str, 'PathLike', 'Path'], *, background: Optional[Union['BackgroundTask', 'BackgroundTasks']]=None, chunk_size: int=ONE_MEGA_BYTE, content_disposition_type: Literal['attachment', 'inline']='attachment', cookies: Optional['ResponseCookies']=None, encoding: str='utf-8', etag: Optional['ETag']=None, file_system: Optional['FileSystemProtocol']=None, filename: Optional[str]=None, file_info: Optional['FileInfo']=None, headers: Optional[Dict[str, Any]]=None, is_head_response: bool=False, media_type: Optional[Union[Literal[MediaType.TEXT], str]]=None, stat_result: Optional['stat_result_type']=None, status_code: int=HTTP_200_OK) -> None:
        ...
        super().__init__(content=async_file_iterator(file_path=path, chunk_size=chunk_size, adapter=self.adapter), status_code=status_code, media_type=media_type, background=background, headers=headers, cookies=cookies, encoding=encoding, is_head_response=is_head_response)
        if file_info:
    ...
    async def start_response(self, send: 'Send') -> None:
        ...
        """
        try:
            fs_info = self.file_info = cast('FileInfo', await self.file_info if iscoroutine(self.file_info) else self.file_info)
        ...
        self.set_etag(self.etag or create_etag_for_file(path=self.file_path, modified_time=fs_info['mtime'], file_size=fs_info['size']))
