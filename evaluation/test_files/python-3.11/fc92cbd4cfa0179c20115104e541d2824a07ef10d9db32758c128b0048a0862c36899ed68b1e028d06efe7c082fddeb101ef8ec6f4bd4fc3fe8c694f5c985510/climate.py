...
from homeassistant.components.climate.const import PRESET_COMFORT, PRESET_ECO, PRESET_NONE, ClimateEntityFeature, HVACAction, HVACMode
from homeassistant.config_entries import ConfigEntry
from homeassistant.const import ATTR_TEMPERATURE, PRECISION_TENTHS, PRECISION_WHOLE, TEMP_CELSIUS
from homeassistant.core import HomeAssistant
from homeassistant.helpers import entity_platform
import homeassistant.helpers.config_validation as cv
from homeassistant.helpers.entity import DeviceInfo
from homeassistant.helpers.entity_platform import AddEntitiesCallback
from .const import CONF_HEATING_TYPE, DOMAIN, VICARE_API, VICARE_DEVICE_CONFIG, VICARE_NAME
...
VICARE_TO_HA_HVAC_HEATING = {VICARE_MODE_FORCEDREDUCED: HVACMode.OFF, VICARE_MODE_OFF: HVACMode.OFF, VICARE_MODE_DHW: HVACMode.OFF, VICARE_MODE_DHWANDHEATINGCOOLING: HVACMode.AUTO, VICARE_MODE_DHWANDHEATING: HVACMode.AUTO, VICARE_MODE_HEATING: HVACMode.AUTO, VICARE_MODE_FORCEDNORMAL: HVACMode.HEAT}
VICARE_TO_HA_PRESET_HEATING = {VICARE_PROGRAM_COMFORT: PRESET_COMFORT, VICARE_PROGRAM_ECO: PRESET_ECO, VICARE_PROGRAM_NORMAL: PRESET_NONE}
HA_TO_VICARE_PRESET_HEATING = {PRESET_COMFORT: VICARE_PROGRAM_COMFORT, PRESET_ECO: VICARE_PROGRAM_ECO, PRESET_NONE: VICARE_PROGRAM_NORMAL}
...
async def async_setup_entry(hass: HomeAssistant, config_entry: ConfigEntry, async_add_entities: AddEntitiesCallback) -> None:
    ...
    for circuit in circuits:
        ...
        if len(circuits) > 1:
            suffix = f' {circuit.id}'
        entity = ViCareClimate(f'{name} Heating{suffix}', api, circuit, hass.data[DOMAIN][config_entry.entry_id][VICARE_DEVICE_CONFIG], config_entry.data[CONF_HEATING_TYPE])
...
class ViCareClimate(ClimateEntity):
    ...
    def device_info(self) -> DeviceInfo:
        ...
        return DeviceInfo(identifiers={(DOMAIN, self._device_config.getConfig().serial)}, name=self._device_config.getModel(), manufacturer='Viessmann', model=self._device_config.getModel(), configuration_url='https://developer.viessmann.com/')
    ...
    def update(self):
        ...
        try:
            ...
            with suppress(PyViCareNotSupportedFeatureError):
                self._attributes['heating_curve_slope'] = self._circuit.getHeatingCurveSlope()
            with suppress(PyViCareNotSupportedFeatureError):
                self._attributes['heating_curve_shift'] = self._circuit.getHeatingCurveShift()
    ...
    def set_preset_mode(self, preset_mode):
        ...
        if vicare_program is None:
            raise ValueError(f'Cannot set invalid vicare program: {preset_mode}/{vicare_program}')
