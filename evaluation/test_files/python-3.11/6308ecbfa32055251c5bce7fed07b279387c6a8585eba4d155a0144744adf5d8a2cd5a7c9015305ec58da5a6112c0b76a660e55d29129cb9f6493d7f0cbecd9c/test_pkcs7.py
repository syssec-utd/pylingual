...
def _load_cert_key():
    key = load_vectors_from_file(os.path.join('x509', 'custom', 'ca', 'ca_key.pem'), lambda pemfile: serialization.load_pem_private_key(pemfile.read(), None), mode='rb')
...
class TestPKCS7Builder:
    ...
    def test_multiple_signers(self, backend):
        data = b'hello world'
        cert, key = _load_cert_key()
        rsa_key = load_vectors_from_file(os.path.join('x509', 'custom', 'ca', 'rsa_key.pem'), lambda pemfile: serialization.load_pem_private_key(pemfile.read(), None), mode='rb')
...
class TestPKCS7SerializeCerts:
    ...
    def test_invalid_types(self):
        certs = load_vectors_from_file(os.path.join('pkcs7', 'amazon-roots.der'), lambda derfile: pkcs7.load_der_pkcs7_certificates(derfile.read()), mode='rb')
        with pytest.raises(TypeError):
            pkcs7.serialize_certificates('not a list of certs', serialization.Encoding.PEM)
        with pytest.raises(TypeError):
