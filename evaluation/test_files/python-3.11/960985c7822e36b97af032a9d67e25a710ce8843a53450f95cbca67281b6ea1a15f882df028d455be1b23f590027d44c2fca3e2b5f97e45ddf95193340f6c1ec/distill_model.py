...
__all__ = ['DistillModel', 'FGDDistillModel', 'CWDDistillModel', 'LDDistillModel', 'PPYOLOEDistillModel']
...
class FGDDistillModel(DistillModel):
    ...
    def __init__(self, cfg, slim_cfg):
        ...
        assert self.arch in ['RetinaNet', 'PicoDet'], 'Unsupported arch: {}'.format(self.arch)
    ...
    def forward(self, inputs):
        if self.training:
            ...
            for idx, k in enumerate(self.distill_loss):
                loss_dict[k] = self.distill_loss[k](s_neck_feats[idx], t_neck_feats[idx], inputs)
            ...
            if self.arch == 'RetinaNet':
                bbox, bbox_num = self.student_model.head.post_process(head_outs, inputs['im_shape'], inputs['scale_factor'])
                return {'bbox': bbox, 'bbox_num': bbox_num}
            elif self.arch == 'PicoDet':
                head_outs = self.student_model.head(neck_feats, self.student_model.export_post_process)
                scale_factor = inputs['scale_factor']
                bboxes, bbox_num = self.student_model.head.post_process(head_outs, scale_factor, export_nms=self.student_model.export_nms)
...
class CWDDistillModel(DistillModel):
    ...
    def __init__(self, cfg, slim_cfg):
        ...
        assert self.arch in ['GFL', 'RetinaNet'], 'Unsupported arch: {}'.format(self.arch)
    ...
    def get_loss_retinanet(self, stu_fea_list, tea_fea_list, inputs):
        ...
        loss_dict = {}
        for idx, k in enumerate(self.distill_loss):
            loss_dict[k] = self.distill_loss[k](stu_fea_list[idx], tea_fea_list[idx])
            loss['loss'] += loss_dict[k]
            loss[k] = loss_dict[k]
        return loss
    ...
    def get_loss_gfl(self, stu_fea_list, tea_fea_list, inputs):
        ...
        for idx, k in enumerate(self.distill_loss):
            loss_dict[k] = self.distill_loss[k](s_cls_feat[idx], t_cls_feat[idx])
            feat_loss[f'neck_f_{idx}'] = self.distill_loss[k](stu_fea_list[idx], tea_fea_list[idx])
        for k in feat_loss:
    ...
    def forward(self, inputs):
        if self.training:
            ...
            with paddle.no_grad():
                ...
                t_neck_feats = self.teacher_model.neck(t_body_feats)
            if self.arch == 'RetinaNet':
                loss = self.get_loss_retinanet(s_neck_feats, t_neck_feats, inputs)
            elif self.arch == 'GFL':
...
class LDDistillModel(DistillModel):
    ...
    def forward(self, inputs):
        if self.training:
            ...
            student_loss = self.student_model.head.get_loss(s_head_outs, inputs, soft_label_list, soft_targets_list)
...
class PPYOLOEDistillModel(DistillModel):
    ...
    def __init__(self, cfg, slim_cfg):
        ...
        assert self.arch in ['PPYOLOE'], 'Unsupported arch: {}'.format(self.arch)
    ...
    def forward(self, inputs, alpha=0.125):
        if self.training:
            ...
            if hasattr(self.teacher_model.yolo_head, 'assigned_labels'):
                self.student_model.yolo_head.assigned_labels, self.student_model.yolo_head.assigned_bboxes, self.student_model.yolo_head.assigned_scores, self.student_model.yolo_head.mask_positive = (self.teacher_model.yolo_head.assigned_labels, self.teacher_model.yolo_head.assigned_bboxes, self.teacher_model.yolo_head.assigned_scores, self.teacher_model.yolo_head.mask_positive)
            ...
            student_loss = self.student_model(inputs)
            logits_loss, feat_loss = self.distill_loss(self.teacher_model, self.student_model)
