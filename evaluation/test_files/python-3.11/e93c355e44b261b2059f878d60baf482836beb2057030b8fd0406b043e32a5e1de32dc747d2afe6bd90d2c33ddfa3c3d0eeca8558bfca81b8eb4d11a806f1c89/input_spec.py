...
@keras_export('keras.layers.InputSpec', v1=['keras.layers.InputSpec', 'keras.__internal__.legacy.layers.InputSpec'])
...
class InputSpec:
    ...
    def __init__(self, dtype=None, shape=None, ndim=None, max_ndim=None, min_ndim=None, axes=None, allow_last_axis_squeeze=False, name=None):
        ...
        try:
            ...
            raise TypeError(f'Argument `axes` must be a dict with integer keys. Received: axes={axes}')
...
def assert_input_compatibility(input_spec, inputs, layer_name):
    ...
    for input_index, (x, spec) in enumerate(zip(inputs, input_spec)):
        ...
        if spec.ndim is not None and (not spec.allow_last_axis_squeeze):
            ndim = shape.rank
            if ndim != spec.ndim:
                raise ValueError(f'Input {input_index} of layer "{layer_name}" is incompatible with the layer: expected ndim={spec.ndim}, found ndim={ndim}. Full shape received: {tuple(shape)}')
        ...
        if spec.min_ndim is not None:
            ...
            if ndim is not None and ndim < spec.min_ndim:
                raise ValueError(f'Input {input_index} of layer "{layer_name}" is incompatible with the layer: expected min_ndim={spec.min_ndim}, found ndim={ndim}. Full shape received: {tuple(shape)}')
        if spec.dtype is not None:
            if x.dtype.name != spec.dtype:
                raise ValueError(f'Input {input_index} of layer "{layer_name}" is incompatible with the layer: expected dtype={spec.dtype}, found dtype={x.dtype}')
        ...
        if spec.axes:
            for axis, value in spec.axes.items():
                ...
                if value is not None and shape_as_list[int(axis)] not in {value, None}:
                    raise ValueError(f'Input {input_index} of layer "{layer_name}" is incompatible with the layer: expected axis {axis} of input shape to have value {value}, but received input with shape {display_shape(x.shape)}')
        if spec.shape is not None and shape.rank is not None:
            ...
            for spec_dim, dim in zip(spec_shape, shape_as_list):
                if spec_dim is not None and dim is not None:
                    if spec_dim != dim:
                        raise ValueError(f'Input {input_index} of layer "{layer_name}" is incompatible with the layer: expected shape={spec.shape}, found shape={display_shape(x.shape)}')
