...
from edx_rest_framework_extensions.auth.jwt.decoder import decode_jwt_filters, decode_jwt_is_restricted, decode_jwt_scopes
...
class JwtHasScope(BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        if not allowed:
            log.warning("Permission JwtHasScope: required scopes '%s' are not a subset of the token's scopes '%s'.", required_scopes, jwt_scopes)
        return allowed
    ...
    def has_permission(self, request, view):
        ...
        for filter_type, filter_value in jwt_filters:
            if filter_type == 'content_org' and filter_value == course_key.org:
                return True
        log.warning('Permission JwtHasContentOrgFilterForRequestedCourse: no filter found for %s.', course_key.org)
        return False
...
class JwtHasUserFilterForRequestedUser(BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        if not allowed:
            log.warning("Permission JwtHasUserFilterForRequestedUser: user_filter %s doesn't match username %s.", user_filter, username_param)
        return allowed
...
_JWT_RESTRICTED_PERMISSIONS = JwtRestrictedApplication & JwtHasScope & JwtHasContentOrgFilterForRequestedCourse & JwtHasUserFilterForRequestedUser
JWT_RESTRICTED_APPLICATION_OR_USER_ACCESS = IsAuthenticated & (_NOT_JWT_RESTRICTED_PERMISSIONS | _JWT_RESTRICTED_PERMISSIONS)

def get_username_param(request):
    user_parameter_name = 'username'
    url_username = getattr(request, 'parser_context', {}).get('kwargs', {}).get(user_parameter_name, '') or request.GET.get(user_parameter_name, '')
