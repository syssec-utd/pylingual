...
from superset.utils.core import extract_column_dtype, extract_dataframe_dtypes, ExtraFiltersReasonType, get_column_name, get_time_filter_status, is_adhoc_column
if TYPE_CHECKING:
...
def _get_full(query_context: QueryContext, query_obj: QueryObject, force_cached: Optional[bool]=False) -> Dict[str, Any]:
    datasource = _get_datasource(query_context, query_obj)
    ...
    payload['applied_filters'] = [{'column': get_column_name(col)} for col in filter_columns if is_adhoc_column(col) or col in columns or col in applied_template_filters] + applied_time_columns
    payload['rejected_filters'] = [{'reason': ExtraFiltersReasonType.COL_NOT_IN_DATASOURCE, 'column': col} for col in filter_columns if not is_adhoc_column(col) and col not in columns and (col not in applied_template_filters)] + rejected_time_columns
