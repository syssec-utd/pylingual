...
def parameterized_truncated_normal(shape, means=0.0, stddevs=1.0, minvals=-2.0, maxvals=2.0, dtype=dtypes.float32, seed=None, name=None):
...
def random_uniform(shape, minval=0, maxval=None, dtype=dtypes.float32, seed=None, name=None):
  ...
  """
    ...
    accepted_dtypes = (dtypes.float16, dtypes.bfloat16, dtypes.float32, dtypes.float64, dtypes.int32, dtypes.int64)
    ...
    with ops.name_scope(name, 'random_uniform', [shape, minval, maxval]) as name:
        ...
        seed1, seed2 = random_seed.get_seed(seed)
        if dtype.is_integer:
            result = gen_random_ops.random_uniform_int(shape, minval, maxval, seed=seed1, seed2=seed2, name=name)
        else:
            result = gen_random_ops.random_uniform(shape, dtype, seed=seed1, seed2=seed2)
            if minval_is_zero:
...
def random_crop(value, size, seed=None, name=None):
  ...
  """
    with ops.name_scope(name, 'random_crop', [value, size]) as name:
        ...
        check = control_flow_ops.Assert(math_ops.reduce_all(shape >= size), ['Need value.shape >= size, got ', shape, size], summarize=1000)
        shape = control_flow_ops.with_dependencies([check], shape)
        limit = shape - size + 1
        offset = random_uniform(array_ops.shape(shape), dtype=size.dtype, maxval=size.dtype.max, seed=seed) % limit
...
def random_gamma(shape, alpha, beta=None, dtype=dtypes.float32, seed=None, name=None):
  ...
  """
    with ops.name_scope(name, 'random_gamma', [shape, alpha, beta]):
        ...
        seed1, seed2 = random_seed.get_seed(seed)
        result = math_ops.maximum(np.finfo(alpha.dtype.as_numpy_dtype).tiny, gen_random_ops.random_gamma(shape, alpha_broadcast, seed=seed1, seed2=seed2) / beta)
