...
class TSDataset:
    ...
    @staticmethod
    def from_pandas(df, dt_col, target_col, id_col=None, extra_feature_col=None, with_split=False, val_ratio=0, test_ratio=0.1, largest_look_back=0, largest_horizon=1):
        """
        ...
        if with_split:
            tsdataset_dfs = split_timeseries_dataframe(df=tsdataset_df, id_col=id_col, val_ratio=val_ratio, test_ratio=test_ratio, look_back=largest_look_back, horizon=largest_horizon)
            return [TSDataset(data=tsdataset_dfs[i], id_col=id_col, dt_col=dt_col, target_col=target_col, feature_col=feature_col) for i in range(3)]
        return TSDataset(data=tsdataset_df, id_col=id_col, dt_col=dt_col, target_col=target_col, feature_col=feature_col)

    @staticmethod
    def from_parquet(path, dt_col, target_col, id_col=None, extra_feature_col=None, with_split=False, val_ratio=0, test_ratio=0.1, largest_look_back=0, largest_horizon=1, **kwargs):
        """
        ...
        columns = _to_list(dt_col, name='dt_col') + _to_list(target_col, name='target_col') + _to_list(id_col, name='id_col') + _to_list(extra_feature_col, name='extra_feature_col')
        df = parquet2pd(path, columns=columns, **kwargs)
        return TSDataset.from_pandas(df, dt_col=dt_col, target_col=target_col, id_col=id_col, extra_feature_col=extra_feature_col, with_split=with_split, val_ratio=val_ratio, test_ratio=test_ratio, largest_look_back=largest_look_back, largest_horizon=largest_horizon)
    ...
    def impute(self, mode='last', const_num=0):
        ...
        """
        self.df = self.df.groupby([self.id_col]).apply(lambda df: impute_timeseries_dataframe(df=df, dt_col=self.dt_col, mode=mode, const_num=const_num))
    ...
    def resample(self, interval, start_time=None, end_time=None, merge_mode='mean'):
        ...
        self.df = self.df.groupby([self.id_col]).apply(lambda df: resample_timeseries_dataframe(df=df, dt_col=self.dt_col, interval=interval, start_time=start_time, end_time=end_time, id_col=self.id_col, merge_mode=merge_mode))
    ...
    def roll(self, horizon, lookback='auto', feature_col=None, target_col=None, id_sensitive=False, time_enc=False, label_len=0, is_predict=False):
        ...
        rolling_result = self.df.groupby([self.id_col]).apply(lambda df: roll_timeseries_dataframe(df=df, roll_feature_df=roll_feature_df, lookback=self.lookback, horizon=self.horizon, feature_col=feature_col, target_col=target_col, label_len=label_len))
        concat_axis = 2 if id_sensitive else 0
        self.numpy_x = np.concatenate([rolling_result[i][0] for i in self._id_list], axis=concat_axis).astype(np.float32)
        if horizon != 0 or time_enc:
            self.numpy_y = np.concatenate([rolling_result[i][1] for i in self._id_list], axis=concat_axis).astype(np.float32)
        else:
            self.numpy_y = None
        if time_enc:
            ...
            self.numpy_y_timeenc, _ = _roll_timeseries_ndarray(self.data_stamp[lookback - label_len:], horizon_time + label_len)
        else:
    ...
    def to_torch_data_loader(self, batch_size=32, roll=False, lookback='auto', horizon=None, feature_col=None, target_col=None, shuffle=True, time_enc=False, label_len=0, is_predict=False):
        """
        ...
        if roll:
            if horizon is None:
                invalidInputError(False, 'You must input horizon if roll is True')
            ...
            if self.numpy_x is None:
                invalidInputError(False, "Please call 'roll' method before transforming a TSDataset to torch DataLoader without rolling (default roll=False)!")
            x, y = self.to_numpy()
    ...
    def scale(self, scaler, fit=True):
        ...
        if fit:
            ...
            self.df[self.target_col + feature_col] = scaler.transform(self.df[self.target_col + feature_col])
        self.scaler = scaler
        return self
    ...
    def get_cycle_length(self, aggregate='mode', top_k=3):
        ...
        if len(self.target_col) == 1:
            res = self.df.groupby(self.id_col).apply(lambda x: cycle_length_est(x[self.target_col[0]].values, top_k))
        else:
            res = self.df.groupby(self.id_col).apply(lambda x: pd.DataFrame({'cycle_length': [cycle_length_est(x[col].values, top_k) for col in self.target_col]}))
