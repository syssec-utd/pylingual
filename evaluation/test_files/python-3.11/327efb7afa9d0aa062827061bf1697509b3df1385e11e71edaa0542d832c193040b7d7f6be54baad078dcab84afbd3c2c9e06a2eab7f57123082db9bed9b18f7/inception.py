...
def inception_layer_v1(input_size, config, name_prefix=''):
    concat = Concat(2)
    conv1 = Sequential()
    conv1.add(SpatialConvolution(input_size, config[1][1], 1, 1, 1, 1).set_init_method(weight_init_method=Xavier(), bias_init_method=ConstInitMethod(0.1)).set_name(name_prefix + '1x1'))
    conv1.add(ReLU(True).set_name(name_prefix + 'relu_1x1'))
    concat.add(conv1)
    conv3 = Sequential()
    conv3.add(SpatialConvolution(input_size, config[2][1], 1, 1, 1, 1).set_init_method(weight_init_method=Xavier(), bias_init_method=ConstInitMethod(0.1)).set_name(name_prefix + '3x3_reduce'))
    conv3.add(ReLU(True).set_name(name_prefix + 'relu_3x3_reduce'))
    conv3.add(SpatialConvolution(config[2][1], config[2][2], 3, 3, 1, 1, 1, 1).set_init_method(weight_init_method=Xavier(), bias_init_method=ConstInitMethod(0.1)).set_name(name_prefix + '3x3'))
    conv3.add(ReLU(True).set_name(name_prefix + 'relu_3x3'))
    concat.add(conv3)
    conv5 = Sequential()
    conv5.add(SpatialConvolution(input_size, config[3][1], 1, 1, 1, 1).set_init_method(weight_init_method=Xavier(), bias_init_method=ConstInitMethod(0.1)).set_name(name_prefix + '5x5_reduce'))
    conv5.add(ReLU(True).set_name(name_prefix + 'relu_5x5_reduce'))
    conv5.add(SpatialConvolution(config[3][1], config[3][2], 5, 5, 1, 1, 2, 2).set_init_method(weight_init_method=Xavier(), bias_init_method=ConstInitMethod(0.1)).set_name(name_prefix + '5x5'))
    conv5.add(ReLU(True).set_name(name_prefix + 'relu_5x5'))
    concat.add(conv5)
    pool = Sequential()
    pool.add(SpatialMaxPooling(3, 3, 1, 1, 1, 1, to_ceil=True).set_name(name_prefix + 'pool'))
    pool.add(SpatialConvolution(input_size, config[4][1], 1, 1, 1, 1).set_init_method(weight_init_method=Xavier(), bias_init_method=ConstInitMethod(0.1)).set_name(name_prefix + 'pool_proj'))
...
def inception_v1_no_aux_classifier(class_num, has_dropout=True):
    model = Sequential()
    model.add(SpatialConvolution(3, 64, 7, 7, 2, 2, 3, 3, 1, False).set_init_method(weight_init_method=Xavier(), bias_init_method=ConstInitMethod(0.1)).set_name('conv1/7x7_s2'))
    ...
    model.add(SpatialConvolution(64, 64, 1, 1, 1, 1).set_init_method(weight_init_method=Xavier(), bias_init_method=ConstInitMethod(0.1)).set_name('conv2/3x3_reduce'))
    model.add(ReLU(True).set_name('conv2/relu_3x3_reduce'))
    model.add(SpatialConvolution(64, 192, 3, 3, 1, 1, 1, 1).set_init_method(weight_init_method=Xavier(), bias_init_method=ConstInitMethod(0.1)).set_name('conv2/3x3'))
    ...
    model.add(Linear(1024, class_num).set_init_method(weight_init_method=Xavier(), bias_init_method=Zeros()).set_name('loss3/classifier'))
...
def inception_v1(class_num, has_dropout=True):
    feature1 = Sequential()
    feature1.add(SpatialConvolution(3, 64, 7, 7, 2, 2, 3, 3, 1, False).set_init_method(weight_init_method=Xavier(), bias_init_method=ConstInitMethod(0.1)).set_name('conv1/7x7_s2'))
    ...
    feature1.add(SpatialConvolution(64, 64, 1, 1, 1, 1).set_init_method(weight_init_method=Xavier(), bias_init_method=ConstInitMethod(0.1)).set_name('conv2/3x3_reduce'))
    feature1.add(ReLU(True).set_name('conv2/relu_3x3_reduce'))
    feature1.add(SpatialConvolution(64, 192, 3, 3, 1, 1, 1, 1).set_init_method(weight_init_method=Xavier(), bias_init_method=ConstInitMethod(0.1)).set_name('conv2/3x3'))
    ...
    output3 = Sequential()
    output3.add(inception_layer_v1(528, t([t([256]), t([160, 320]), t([32, 128]), t([128])]), 'inception_4e/'))
    output3.add(SpatialMaxPooling(3, 3, 2, 2, to_ceil=True).set_name('pool4/3x3_s2'))
    output3.add(inception_layer_v1(832, t([t([256]), t([160, 320]), t([32, 128]), t([128])]), 'inception_5a/'))
    output3.add(inception_layer_v1(832, t([t([384]), t([192, 384]), t([48, 128]), t([128])]), 'inception_5b/'))
    ...
    output3.add(Linear(1024, class_num).set_init_method(weight_init_method=Xavier(), bias_init_method=Zeros()).set_name('loss3/classifier'))
...
def config_option_parser():
    parser = OptionParser()
    parser.add_option('-f', '--folder', type=str, dest='folder', default='', help='url of hdfs folder store the hadoop sequence files')
    ...
    parser.add_option('-o', '--overwrite', action='store_true', dest='overwrite', default=False, help='overwrite checkpoint files')
    ...
    parser.add_option('--checkpointIteration', type=int, dest='checkpointIteration', default=620, help='checkpoint interval of iterations')
...
if __name__ == '__main__':
    ...
    conf = create_spark_conf().set('spark.executor.memory', options.executorMemory).set('spark.executor.cores', options.cores).set('spark.executor.instances', options.executors).set('spark.driver.memory', options.driverMemory)
    sc = init_nncontext(conf, cluster_mode=options.deployMode, hadoop_conf=hadoop_conf)
    image_size = 224
    train_transformer = Pipeline([PixelBytesToMat(), Resize(256, 256), RandomCropper(image_size, image_size, True, 'Random', 3), ChannelNormalize(123.0, 117.0, 104.0), MatToTensor(to_rgb=False), ImageFrameToSample(input_keys=['imageTensor'], target_keys=['label'])])
    ...
    val_transformer = Pipeline([PixelBytesToMat(), Resize(256, 256), RandomCropper(image_size, image_size, False, 'Center', 3), ChannelNormalize(123.0, 117.0, 104.0), MatToTensor(to_rgb=False), ImageFrameToSample(input_keys=['imageTensor'], target_keys=['label'])])
    ...
    if options.state != '':
        ...
        optim = SGD(learningrate=options.learningRate, learningrate_decay=0.0, weightdecay=options.weightDecay, momentum=0.9, dampening=0.0, nesterov=False, leaningrate_schedule=lrSchedule)
    if options.maxEpoch:
    ...
    optimizer = Optimizer.create(model=inception_model, training_set=train_data, optim_method=optim, criterion=ClassNLLCriterion(), end_trigger=end_trigger, batch_size=options.batchSize)
    ...
    optimizer.set_validation(trigger=test_trigger, val_rdd=val_data, batch_size=options.batchSize, val_method=[Top1Accuracy(), Top5Accuracy()])
