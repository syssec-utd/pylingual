...
class TornadoAsyncTransport(Transport):
    ...
    def __init__(self, cache=None, timeout=300, operation_timeout=None, session=None):
        ...
        self.session.headers['User-Agent'] = 'Zeep/%s (www.python-zeep.org)' % get_version()

    def _load_remote_data(self, url):
        client = httpclient.HTTPClient()
        kwargs = {'method': 'GET', 'connect_timeout': self.load_timeout, 'request_timeout': self.load_timeout}
    ...
    def fetch(self, address, method, headers, message=None):
        ...
        kwargs = {'method': method, 'connect_timeout': self.operation_timeout, 'request_timeout': self.operation_timeout, 'headers': dict(headers, **session_headers), 'auth_username': auth_username, 'auth_password': auth_password, 'auth_mode': auth_mode, 'validate_cert': bool(self.session.verify), 'ca_certs': ca_certs, 'client_key': client_key, 'client_cert': client_cert}
        if message:
            kwargs['body'] = message
