...
class PPYOLOEHead(nn.Layer):
    ...
    def _init_weights(self):
        ...
        proj = paddle.linspace(self.reg_range[0], self.reg_range[1] - 1, self.reg_channels).reshape([1, self.reg_channels, 1, 1])
    ...
    def forward_train(self, feats, targets, aux_pred=None):
        ...
        if targets.get('get_data', False):
            pred_deltas, pred_dfls = self._bbox_decode_fake(reg_distri_list)
            return (cls_score_list, pred_deltas * stride_tensor, pred_dfls)
        return self.get_loss([cls_score_list, reg_distri_list, anchors, anchor_points, num_anchors_list, stride_tensor], targets, aux_pred)
    ...
    def _generate_anchors(self, feats=None, dtype='float32'):
        ...
        for i, stride in enumerate(self.fpn_strides):
            ...
            anchor_point = paddle.cast(paddle.stack([shift_x, shift_y], axis=-1), dtype=dtype)
    ...
    def forward_eval(self, feats):
        ...
        for i, feat in enumerate(feats):
            ...
            reg_dist = reg_dist.reshape([-1, 4, self.reg_channels, l]).transpose([0, 2, 3, 1])
    ...
    def _bbox_decode_fake(self, pred_dist):
        ...
        pred_dist = self.proj_conv(pred_dist_dfl.transpose([0, 3, 1, 2])).squeeze(1)
    ...
    def _bbox2distance(self, points, bbox):
        ...
        lt = points - x1y1
        rb = x2y2 - points
        return paddle.concat([lt, rb], -1).clip(self.reg_range[0], self.reg_range[1] - 1 - 0.01)
    ...
    def _df_loss(self, pred_dist, target, lower_bound=0):
        ...
        loss_left = F.cross_entropy(pred_dist, target_left - lower_bound, reduction='none') * weight_left
        loss_right = F.cross_entropy(pred_dist, target_right - lower_bound, reduction='none') * weight_right
    ...
    def _bbox_loss(self, pred_dist, pred_bboxes, anchor_points, assigned_labels, assigned_bboxes, assigned_scores, assigned_scores_sum):
        ...
        if num_pos > 0:
            ...
            dist_mask = mask_positive.unsqueeze(-1).tile([1, 1, self.reg_channels * 4])
            pred_dist_pos = paddle.masked_select(pred_dist, dist_mask).reshape([-1, 4, self.reg_channels])
    ...
    def get_loss(self, head_outs, gt_meta, aux_pred=None):
        ...
        if gt_meta['epoch_id'] < self.static_assigner_epoch:
            assigned_labels, assigned_bboxes, assigned_scores, mask_positive = self.static_assigner(anchors, num_anchors_list, gt_labels, gt_bboxes, pad_gt_mask, bg_index=self.num_classes, pred_bboxes=pred_bboxes.detach() * stride_tensor)
            alpha_l = 0.25
        else:
            if self.sm_use:
                assigned_labels, assigned_bboxes, assigned_scores, mask_positive = self.assigner(pred_scores.detach(), pred_bboxes.detach() * stride_tensor, anchor_points, stride_tensor, gt_labels, gt_bboxes, pad_gt_mask, bg_index=self.num_classes)
            elif aux_pred is None:
                if not hasattr(self, 'assigned_labels'):
                    assigned_labels, assigned_bboxes, assigned_scores, mask_positive = self.assigner(pred_scores.detach(), pred_bboxes.detach() * stride_tensor, anchor_points, num_anchors_list, gt_labels, gt_bboxes, pad_gt_mask, bg_index=self.num_classes)
                    if self.for_distill:
                ...
                assigned_labels, assigned_bboxes, assigned_scores, mask_positive = self.assigner(pred_scores_aux.detach(), pred_bboxes_aux.detach() * stride_tensor, anchor_points, num_anchors_list, gt_labels, gt_bboxes, pad_gt_mask, bg_index=self.num_classes)
            alpha_l = -1
        assigned_bboxes /= stride_tensor
        assign_out_dict = self.get_loss_from_assign(pred_scores, pred_distri, pred_bboxes, anchor_points_s, assigned_labels, assigned_bboxes, assigned_scores, mask_positive, alpha_l)
        if aux_pred is not None:
            assign_out_dict_aux = self.get_loss_from_assign(aux_pred[0], aux_pred[1], pred_bboxes_aux, anchor_points_s, assigned_labels, assigned_bboxes, assigned_scores, mask_positive, alpha_l)
            loss = {}
    ...
    def get_loss_from_assign(self, pred_scores, pred_distri, pred_bboxes, anchor_points_s, assigned_labels, assigned_bboxes, assigned_scores, mask_positive, alpha_l):
        ...
        out_dict = {'loss': loss, 'loss_cls': loss_cls, 'loss_iou': loss_iou, 'loss_dfl': loss_dfl, 'loss_l1': loss_l1}
        return out_dict
    ...
    def post_process(self, head_outs, scale_factor):
        ...
        if self.exclude_post_process:
            return (paddle.concat([pred_bboxes, pred_scores.transpose([0, 2, 1])], axis=-1), None, None)
        else:
            scale_y, scale_x = paddle.split(scale_factor, 2, axis=-1)
            scale_factor = paddle.concat([scale_x, scale_y, scale_x, scale_y], axis=-1).reshape([-1, 1, 4])
...
class ConvNormLayer(nn.Layer):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, norm_type='gn', activation='LeakyReLU'):
        super(ConvNormLayer, self).__init__()
        assert norm_type in ['bn', 'sync_bn', 'syncbn', 'gn', None]
        self.conv = nn.Conv2D(in_channels, out_channels, kernel_size, stride=stride, padding=padding, dilation=dilation, groups=groups, bias_attr=False, weight_attr=ParamAttr(initializer=KaimingNormal()))
...
class ScaleReg(nn.Layer):
    ...
    def __init__(self, scale=1.0):
        ...
        self.scale = self.create_parameter(shape=[1], dtype='float32', default_initializer=nn.initializer.Assign(scale))
...
class SimpleConvHead(nn.Layer):
    ...
    def __init__(self, num_classes=80, feat_in=288, feat_out=288, num_convs=1, fpn_strides=[32, 16, 8, 4], norm_type='gn', act='LeakyReLU', prior_prob=0.01, reg_max=16):
        ...
        for i in range(self.num_convs):
            ...
            self.cls_convs.append(ConvNormLayer(in_c, feat_out, 3, stride=1, padding=1, norm_type=norm_type, activation=act))
            self.reg_convs.append(ConvNormLayer(in_c, feat_out, 3, stride=1, padding=1, norm_type=norm_type, activation=act))
        bias_cls = bias_init_with_prob(prior_prob)
        self.gfl_cls = nn.Conv2D(feat_out, self.num_classes, kernel_size=3, stride=1, padding=1, weight_attr=ParamAttr(initializer=Normal(mean=0.0, std=0.01)), bias_attr=ParamAttr(initializer=Constant(value=bias_cls)))
        self.gfl_reg = nn.Conv2D(feat_out, 4 * (self.reg_max + 1), kernel_size=3, stride=1, padding=1, weight_attr=ParamAttr(initializer=Normal(mean=0.0, std=0.01)), bias_attr=ParamAttr(initializer=Constant(value=0)))
