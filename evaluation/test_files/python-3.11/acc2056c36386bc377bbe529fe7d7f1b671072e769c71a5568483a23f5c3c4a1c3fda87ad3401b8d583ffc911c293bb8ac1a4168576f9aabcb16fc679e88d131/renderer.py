...
from checkov.terraform.graph_builder.utils import get_referenced_vertices_in_value, remove_index_pattern_from_str, attribute_has_nested_attributes
from checkov.terraform.graph_builder.variable_rendering.vertex_reference import VertexReference
from checkov.terraform.graph_builder.variable_rendering.evaluate_terraform import replace_string_value, evaluate_terraform
if TYPE_CHECKING:
    from checkov.terraform.graph_builder.local_graph import TerraformLocalGraph
VAR_TYPE_DEFAULT_VALUES: dict[str, list[Any] | dict[str, Any]] = {'list': [], 'map': {}}
TYPE_REGEX = re.compile('^(\\${)?([a-z]+)')
...
class TerraformVariableRenderer(VariableRenderer):
    ...
    def evaluate_vertex_attribute_from_edge(self, edge_list: List[Edge]) -> None:
        ...
        referenced_vertices = get_referenced_vertices_in_value(value=val_to_eval, aliases={}, resources_types=self.local_graph.get_resources_types_in_graph())
        if not referenced_vertices:
            ...
            if origin_vertex.block_type == BlockType.VARIABLE and destination_vertex.block_type == BlockType.MODULE:
                self.update_evaluated_value(changed_attribute_key=edge.label, changed_attribute_value=destination_vertex.attributes[origin_vertex.name], vertex=edge.origin, change_origin_id=edge.dest, attribute_at_dest=edge.label)
                return
            if origin_vertex.block_type == BlockType.VARIABLE and destination_vertex.block_type == BlockType.TF_VARIABLE:
                ...
                self.update_evaluated_value(changed_attribute_key=edge.label, changed_attribute_value=destination_vertex.attributes['default'], vertex=edge.origin, change_origin_id=edge.dest, attribute_at_dest=edge.label)
                return
        ...
        if referenced_vertices:
            for edge in edge_list:
                ...
                key_path_in_dest_vertex, replaced_key = self.find_path_from_referenced_vertices(referenced_vertices, dest_vertex_attributes)
                if not key_path_in_dest_vertex or not replaced_key:
                    continue
                if not first_key_path:
                    first_key_path = key_path_in_dest_vertex
                evaluated_attribute_value = self.extract_value_from_vertex(key_path_in_dest_vertex, dest_vertex_attributes)
                ...
                if not multiple_edges and val_to_eval != origin_val:
                    self.update_evaluated_value(changed_attribute_key=edge.label, changed_attribute_value=val_to_eval, vertex=edge.origin, change_origin_id=edge.dest, attribute_at_dest=key_path_in_dest_vertex)
        if multiple_edges and val_to_eval != origin_val:
            self.update_evaluated_value(changed_attribute_key=edge.label, changed_attribute_value=val_to_eval, vertex=edge.origin, change_origin_id=edge.dest, attribute_at_dest=first_key_path)
        if self.local_graph.vertices[edge.origin].block_type == BlockType.OUTPUT and self.local_graph.vertices[edge.dest].block_type == BlockType.OUTPUT:
    ...
    def extract_value_from_vertex(self, key_path: List[str], attributes: Dict[str, Any]) -> Any:
        ...
        if attributes.get(CustomAttributes.BLOCK_TYPE) in (BlockType.VARIABLE, BlockType.TF_VARIABLE):
            ...
            elif isinstance(var_type, str) and var_type.startswith('${object') and isinstance(default_val, str):
                try:
    ...
    @staticmethod
    def find_path_from_referenced_vertices(referenced_vertices: List[VertexReference], vertex_attributes: Dict[str, Any]) -> Tuple[List[str], str]:
        """
    ...
    def update_evaluated_value(self, changed_attribute_key: str, changed_attribute_value: Union[str, List[str]], vertex: int, change_origin_id: int, attribute_at_dest: Optional[Union[str, List[str]]]=None) -> None:
        """
        The function updates the value of changed_attribute_key with changed_attribute_value for vertex
        """
        str_to_evaluate = str(changed_attribute_value) if self.attributes_no_eval(changed_attribute_key, vertex) else f'"{str(changed_attribute_value)}"'
        str_to_evaluate = str_to_evaluate.replace('\\\\', '\\')
        evaluated_attribute_value = str_to_evaluate if self.attributes_no_eval(changed_attribute_key, vertex) else evaluate_terraform(str_to_evaluate)
    ...
    def replace_value(self, edge: Edge, original_val: List[Any], replaced_key: str, replaced_value: Any, keep_origin: bool, count: int=0) -> Union[Any, List[Any]]:
        if count > 1:
            return original_val
        new_val = replace_string_value(original_str=original_val, str_to_replace=replaced_key, replaced_value=replaced_value, keep_origin=keep_origin)
        return new_val
    ...
    def _process_dynamic_blocks(dynamic_blocks: list[dict[str, Any]]) -> dict[str, list[dict[str, Any]]]:
        ...
        for block in dynamic_blocks:
            block_name, block_values = next(iter(block.items()))
            ...
            dynamic_arguments = [argument for argument, value in block_content.items() if value == dynamic_value_ref]
    ...
    def evaluate_non_rendered_values(self) -> None:
        for index, vertex in enumerate(self.local_graph.vertices):
            ...
            filtered_attributes = [attr for attr in vertex.attributes if attr not in reserved_attribute_names and (not attribute_has_nested_attributes(attr, vertex.attributes))]
            for attribute in filtered_attributes:
                ...
                for inner_val in lst_curr_val:
                    if isinstance(inner_val, str) and (not any((c in inner_val for c in ('{', '}', '[', ']', '=')))) or self.attributes_no_eval(attribute, index):
    ...
    def evaluate_value(self, val: Any) -> Any:
        ...
        if CHECKOV_RENDER_MAX_LEN and 0 < CHECKOV_RENDER_MAX_LEN < val_length:
            logging.debug(f'Rendering was skipped for a {val_length}-character-long string. If you wish to have it evaluated, please set the environment variable CHECKOV_RENDER_MAX_LEN to {str(val_length + 1)} or to 0 to allow rendering of any length')
            return val
