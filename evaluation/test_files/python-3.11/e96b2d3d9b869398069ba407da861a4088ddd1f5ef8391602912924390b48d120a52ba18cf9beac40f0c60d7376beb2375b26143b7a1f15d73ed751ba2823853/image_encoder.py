...
class ImageEncoder(EncoderBase):
    ...
    def __init__(self, num_layers, bidirectional, rnn_size, dropout, image_chanel_size=3):
        ...
        self.layer1 = nn.Conv2d(image_chanel_size, 64, kernel_size=(3, 3), padding=(1, 1), stride=(1, 1))
        ...
        self.rnn = nn.LSTM(src_size, int(rnn_size / self.num_directions), num_layers=num_layers, dropout=dropout, bidirectional=bidirectional)
    ...
    def from_opt(cls, opt, embeddings=None):
        ...
        return cls(opt.enc_layers, opt.brnn, opt.enc_rnn_size, opt.dropout[0] if type(opt.dropout) is list else opt.dropout, image_channel_size)
