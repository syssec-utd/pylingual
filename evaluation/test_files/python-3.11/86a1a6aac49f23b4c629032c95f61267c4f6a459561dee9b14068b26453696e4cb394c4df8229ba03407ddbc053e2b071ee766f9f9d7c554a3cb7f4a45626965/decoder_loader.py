...
def unassociated_shard_skipper(tarfiles, embeddings_url, handler=wds.handlers.reraise_exception):
    ...
    embedding_shards = set([get_embedding_shard(filename) for filename in embedding_files])
...
class ImageEmbeddingDataset(wds.DataPipeline, wds.compat.FluidInterface):
    ...
    def __init__(self, urls, img_embedding_folder_url=None, text_embedding_folder_url=None, index_width=None, img_preproc=None, extra_keys=[], handler=wds.handlers.reraise_exception, resample=False, shuffle_shards=True):
        """
...
def create_image_embedding_dataloader(tar_url, num_workers, batch_size, img_embeddings_url=None, text_embeddings_url=None, index_width=None, shuffle_num=None, shuffle_shards=True, resample_shards=False, img_preproc=None, extra_keys=[], handler=wds.handlers.reraise_exception):
    """
    ...
    :param handler: A webdataset handler.
    """
    ds = ImageEmbeddingDataset(tar_url, img_embedding_folder_url=img_embeddings_url, text_embedding_folder_url=text_embeddings_url, index_width=index_width, shuffle_shards=shuffle_shards, resample=resample_shards, extra_keys=extra_keys, img_preproc=img_preproc, handler=handler)
    if shuffle_num is not None and shuffle_num > 0:
        ds.shuffle(1000)
    return DataLoader(ds, num_workers=num_workers, batch_size=batch_size, prefetch_factor=2, pin_memory=True, shuffle=False)