...
def adjust_saturation_eager_fallback(images, scale, name, ctx):
    ...
    _result, = _result
    return _result
_CombinedNonMaxSuppressionOutput = collections.namedtuple('CombinedNonMaxSuppression', ['nmsed_boxes', 'nmsed_scores', 'nmsed_classes', 'valid_detections'])
...
def decode_and_crop_jpeg_eager_fallback(contents, crop_window, channels, ratio, fancy_upscaling, try_recover_truncated, acceptable_fraction, dct_method, name, ctx):
    ...
    _result = _execute.execute(b'DecodeAndCropJpeg', 1, inputs=_inputs_flat, attrs=_attrs, ctx=ctx, name=name)
    if _execute.must_record_gradient():
        _execute.record_gradient('DecodeAndCropJpeg', _inputs_flat, _attrs, _result)
    _result, = _result
    return _result
...
def decode_gif(contents, name=None):
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'DecodeGif', name, contents)
            return _result
        except _core._NotOkStatusException as e:
            _ops.raise_from_not_ok_status(e, name)
        except _core._FallbackException:
            pass
        try:
            return decode_gif_eager_fallback(contents, name=name, ctx=_ctx)
        except _core._SymbolicException:
            pass
    _, _, _op, _outputs = _op_def_library._apply_op_helper('DecodeGif', contents=contents, name=name)
    _result = _outputs[:]
    if _execute.must_record_gradient():
        _attrs = ()
        _inputs_flat = _op.inputs
...
def decode_image(contents, channels=0, dtype=_dtypes.uint8, expand_animations=True, name=None):
  ...
  *NOTE*: If the first frame of an animated GIF does not occupy the entire
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'DecodeImage', name, contents, 'channels', channels, 'dtype', dtype, 'expand_animations', expand_animations)
            return _result
        except _core._NotOkStatusException as e:
            _ops.raise_from_not_ok_status(e, name)
        except _core._FallbackException:
            pass
        try:
            return decode_image_eager_fallback(contents, channels=channels, dtype=dtype, expand_animations=expand_animations, name=name, ctx=_ctx)
        except _core._SymbolicException:
            pass
    if channels is None:
        channels = 0
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('DecodeImage', contents=contents, channels=channels, dtype=dtype, expand_animations=expand_animations, name=name)
    _result = _outputs[:]
    if _execute.must_record_gradient():
        _attrs = ('channels', _op._get_attr_int('channels'), 'dtype', _op._get_attr_type('dtype'), 'expand_animations', _op._get_attr_bool('expand_animations'))
        _inputs_flat = _op.inputs
        _execute.record_gradient('DecodeImage', _inputs_flat, _attrs, _result)
    _result, = _result
    return _result
...
def decode_image_eager_fallback(contents, channels, dtype, expand_animations, name, ctx):
    ...
    contents = _ops.convert_to_tensor(contents, _dtypes.string)
    _inputs_flat = [contents]
    _attrs = ('channels', channels, 'dtype', dtype, 'expand_animations', expand_animations)
    _result = _execute.execute(b'DecodeImage', 1, inputs=_inputs_flat, attrs=_attrs, ctx=ctx, name=name)
    if _execute.must_record_gradient():
        _execute.record_gradient('DecodeImage', _inputs_flat, _attrs, _result)
    _result, = _result
    return _result
...
def encode_jpeg_variable_quality_eager_fallback(images, quality, name, ctx):
    ...
    _attrs = None
    _result = _execute.execute(b'EncodeJpegVariableQuality', 1, inputs=_inputs_flat, attrs=_attrs, ctx=ctx, name=name)
    if _execute.must_record_gradient():
        _execute.record_gradient('EncodeJpegVariableQuality', _inputs_flat, _attrs, _result)
    _result, = _result
    return _result
...
def extract_jpeg_shape_eager_fallback(contents, output_type, name, ctx):
    ...
    _result, = _result
    return _result
_GenerateBoundingBoxProposalsOutput = collections.namedtuple('GenerateBoundingBoxProposals', ['rois', 'roi_probabilities'])
...
def non_max_suppression_v3_eager_fallback(boxes, scores, max_output_size, iou_threshold, score_threshold, name, ctx):
    ...
    _result, = _result
    return _result
_NonMaxSuppressionV4Output = collections.namedtuple('NonMaxSuppressionV4', ['selected_indices', 'valid_outputs'])
...
def non_max_suppression_v4_eager_fallback(boxes, scores, max_output_size, iou_threshold, score_threshold, pad_to_max_output_size, name, ctx):
    ...
    _result = _NonMaxSuppressionV4Output._make(_result)
    return _result
_NonMaxSuppressionV5Output = collections.namedtuple('NonMaxSuppressionV5', ['selected_indices', 'selected_scores', 'valid_outputs'])
...
def non_max_suppression_with_overlaps_eager_fallback(overlaps, scores, max_output_size, overlap_threshold, score_threshold, name, ctx):
    ...
    _result, = _result
    return _result
_QuantizedResizeBilinearOutput = collections.namedtuple('QuantizedResizeBilinear', ['resized_images', 'out_min', 'out_max'])
...
def random_crop_eager_fallback(image, size, seed, seed2, name, ctx):
    ...
    _result = _execute.execute(b'RandomCrop', 1, inputs=_inputs_flat, attrs=_attrs, ctx=ctx, name=name)
    if _execute.must_record_gradient():
        _execute.record_gradient('RandomCrop', _inputs_flat, _attrs, _result)
    _result, = _result
    return _result
...
def resize_nearest_neighbor_grad_eager_fallback(grads, size, align_corners, half_pixel_centers, name, ctx):
    ...
    _result, = _result
    return _result
_SampleDistortedBoundingBoxOutput = collections.namedtuple('SampleDistortedBoundingBox', ['begin', 'size', 'bboxes'])
...
def sample_distorted_bounding_box_eager_fallback(image_size, bounding_boxes, seed, seed2, min_object_covered, aspect_ratio_range, area_range, max_attempts, use_image_if_no_bounding_boxes, name, ctx):
    ...
    _result = _SampleDistortedBoundingBoxOutput._make(_result)
    return _result
_SampleDistortedBoundingBoxV2Output = collections.namedtuple('SampleDistortedBoundingBoxV2', ['begin', 'size', 'bboxes'])
...
def sample_distorted_bounding_box_v2_eager_fallback(image_size, bounding_boxes, min_object_covered, seed, seed2, aspect_ratio_range, area_range, max_attempts, use_image_if_no_bounding_boxes, name, ctx):
    ...
    _attrs = ('T', _attr_T, 'seed', seed, 'seed2', seed2, 'aspect_ratio_range', aspect_ratio_range, 'area_range', area_range, 'max_attempts', max_attempts, 'use_image_if_no_bounding_boxes', use_image_if_no_bounding_boxes)
    _result = _execute.execute(b'SampleDistortedBoundingBoxV2', 3, inputs=_inputs_flat, attrs=_attrs, ctx=ctx, name=name)
    if _execute.must_record_gradient():
        _execute.record_gradient('SampleDistortedBoundingBoxV2', _inputs_flat, _attrs, _result)
    _result = _SampleDistortedBoundingBoxV2Output._make(_result)
    return _result
...
def scale_and_translate_grad_eager_fallback(grads, original_image, scale, translation, kernel_type, antialias, name, ctx):
    ...
    _result, = _result
    return _result
_StatelessSampleDistortedBoundingBoxOutput = collections.namedtuple('StatelessSampleDistortedBoundingBox', ['begin', 'size', 'bboxes'])
