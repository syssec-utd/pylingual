...
SPECIAL_CHARACTERS = ['[\x00-\x08\x0b\x0c\x0e-\x1f\x7f-\x9f]']
...
class Response(res):
    ...
    def from_dict(cls, response_dict):
        ...
        response_dict['elapsed'] = datetime.timedelta(0, 0, response_dict['elapsed'])
    ...
    @property
    def to_dict(self):
        response_dict = {'_content': self.content, 'cookies': self.cookies.get_dict(), 'encoding': self.encoding, 'headers': self.headers, 'status_code': self.status_code, 'elapsed': self.elapsed.microseconds, 'url': self.url}
    ...
    def encoding(self):
        ...
        """
        self._encoding = self._encoding or self._headers_encoding() or self._body_declared_encoding() or self.apparent_encoding
    ...
    def _make_absolute(self, link):
        ...
        try:
            ...
            log.error("Invalid URL <{}> can't make absolute_link. exception: {}".format(link, e))
        return link

    def _absolute_links(self, text):
        regexs = ['(<(?i)a.*?href\\s*?=\\s*?["\\\'])(.+?)(["\\\'])', '(<(?i)img.*?src\\s*?=\\s*?["\\\'])(.+?)(["\\\'])', '(<(?i)link.*?href\\s*?=\\s*?["\\\'])(.+?)(["\\\'])', '(<(?i)script.*?src\\s*?=\\s*?["\\\'])(.+?)(["\\\'])']
        for regex in regexs:
