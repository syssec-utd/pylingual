...
def check(node: ForStmt, errors: list[Error]) -> None:
    match node:
        case ForStmt(index=NameExpr(name=for_name), body=Block(body=[ExpressionStmt(expr=CallExpr(callee=MemberExpr(expr=NameExpr(node=Var(type=ty)) as set_name, name='add' | 'discard' as name), args=[arg]))])) if str(ty).startswith('builtins.set[') and set_name.name != for_name:
            ...
            if isinstance(arg, NameExpr):
                ...
                expr = '...'
                new_expr = '... for x in y'
            errors.append(ErrorInfo(node.line, node.column, f'Replace `for x in y: s.{name}({expr})` with `s.{new_func}({new_expr})`'))