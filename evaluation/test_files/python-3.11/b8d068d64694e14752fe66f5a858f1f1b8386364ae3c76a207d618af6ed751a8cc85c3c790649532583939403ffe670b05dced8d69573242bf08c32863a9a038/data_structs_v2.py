...
class V2IndexStruct(DataClassJsonMixin):
    ...
    def to_dict(self, encode_json: bool=False) -> Dict[str, Json]:
        out_dict = {TYPE_KEY: self.get_type(), DATA_KEY: super().to_dict(encode_json)}
        return out_dict
...
class IndexGraph(V2IndexStruct):
    ...
    def get_children(self, parent_node: Optional[Node]) -> Dict[int, str]:
        ...
        if parent_node is None:
            ...
            return {self.node_id_to_index[child_id]: child_id for child_id in children_ids}
...
class IndexDict(V2IndexStruct):
    ...
    def add_node(self, node: Node, text_id: Optional[str]=None) -> str:
        """Add text to table, return current position in list."""
...
class CompositeIndex(V2IndexStruct):
    ...
    def to_dict(self, encode_json: bool=False) -> Dict[str, Json]:
        data_dict = {'all_index_structs': {id_: struct.to_dict(encode_json=encode_json) for id_, struct in self.all_index_structs.items()}, 'root_id': self.root_id}
        out_dict = {TYPE_KEY: self.get_type(), DATA_KEY: data_dict}
        return out_dict