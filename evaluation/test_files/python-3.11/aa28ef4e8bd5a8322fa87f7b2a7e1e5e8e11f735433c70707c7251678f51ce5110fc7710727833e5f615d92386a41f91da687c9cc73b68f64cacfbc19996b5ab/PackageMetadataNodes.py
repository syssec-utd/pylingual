...
from .ExpressionBases import ExpressionBase, ExpressionChildHavingBase, ExpressionChildTupleHavingBase, ExpressionNoSideEffectsMixin
from .ImportHardNodes import ExpressionImportModuleNameHardExists
pkg_resources_require_spec = BuiltinParameterSpec('pkg_resources.require', (), default_count=0, list_star_arg='requirements')
pkg_resources_get_distribution_spec = BuiltinParameterSpec('pkg_resources.get_distribution', ('dist',), default_count=0)
...
class ExpressionPkgResourcesRequireRef(ExpressionImportModuleNameHardExists):
    ...
    def __init__(self, source_ref):
        ExpressionImportModuleNameHardExists.__init__(self, module_name='pkg_resources', import_name='require', module_guaranteed=not shallMakeModule(), source_ref=source_ref)
...
class ExpressionPkgResourcesRequireCall(ExpressionChildTupleHavingBase):
    ...
    def __init__(self, requirements, source_ref):
        ExpressionChildTupleHavingBase.__init__(self, value=requirements, source_ref=source_ref)
    ...
    def _replaceWithCompileTimeValue(self, trace_collection):
        ...
        try:
            ...
            inclusion_logger.warning("Cannot find requirement '%s' at '%s', expect potential run time problem. Could also be dead code." % (','.join((repr(s) for s in args)), self.source_ref.getAsString()))
            self.attempted = True
            trace_collection.onExceptionRaiseExit(BaseException)
            ...
            result = makeExpressionMakeList(elements=[ExpressionPkgResourcesDistributionValueRef(distribution=distribution, source_ref=self.source_ref) for distribution in distributions], source_ref=self.source_ref)
...
class ExpressionPkgResourcesGetDistributionRef(ExpressionImportModuleNameHardExists):
    ...
    def __init__(self, source_ref):
        ExpressionImportModuleNameHardExists.__init__(self, module_name='pkg_resources', import_name='get_distribution', module_guaranteed=not shallMakeModule(), source_ref=source_ref)
...
class ExpressionPkgResourcesGetDistributionCall(ExpressionChildHavingBase):
    ...
    def _replaceWithCompileTimeValue(self, trace_collection):
        ...
        try:
            ...
            inclusion_logger.warning("Cannot find distribution '%s' at '%s', expect potential run time problem, could also be dead code." % (arg, self.source_ref.getAsString()))
            self.attempted = True
            trace_collection.onExceptionRaiseExit(BaseException)
            return (self, None, None)
        except Exception as e:
    ...
    def computeExpression(self, trace_collection):
        ...
        with withNoDeprecationWarning():
            return self._replaceWithCompileTimeValue(trace_collection)
importlib_metadata_version_spec = BuiltinParameterSpec('importlib.metadata.version', ('distribution_name',), default_count=0)
...
class ExpressionImportlibMetadataVersionRef(ExpressionImportModuleNameHardExists):
    ...
    def __init__(self, source_ref):
        ExpressionImportModuleNameHardExists.__init__(self, module_name='importlib.metadata', import_name='version', module_guaranteed=not shallMakeModule(), source_ref=source_ref)
...
class ExpressionImportlibMetadataBackportVersionRef(ExpressionImportModuleNameHardExists):
    """Function reference importlib_metadata.version"""
    kind = 'EXPRESSION_IMPORTLIB_METADATA_BACKPORT_VERSION_REF'

    def __init__(self, source_ref):
        ExpressionImportModuleNameHardExists.__init__(self, module_name='importlib_metadata', import_name='version', module_guaranteed=not shallMakeModule(), source_ref=source_ref)

    def computeExpressionCall(self, call_node, call_args, call_kw, trace_collection):
        trace_collection.onExceptionRaiseExit(BaseException)
        result = extractBuiltinArgs(node=call_node, builtin_class=ExpressionImportlibMetadataBackportVersionCall, builtin_spec=importlib_metadata_version_spec)
...
class ExpressionImportlibMetadataVersionCall(ExpressionChildHavingBase):
    ...
    def __init__(self, distribution_name, source_ref):
        ExpressionChildHavingBase.__init__(self, value=distribution_name, source_ref=source_ref)
        self.attempted = shallMakeModule()
    ...
    def _replaceWithCompileTimeValue(self, trace_collection):
        ...
        try:
            ...
            return (result, 'new_expression', "Compile time predicted 'importlib.metadata.version' result")

    def computeExpression(self, trace_collection):
        if self.attempted or not pkg_resources_get_distribution_spec.isCompileTimeComputable((self.subnode_distribution_name,)):
            trace_collection.onExceptionRaiseExit(BaseException)
            return (self, None, None)
        with withNoDeprecationWarning():
            return self._replaceWithCompileTimeValue(trace_collection)
...
class ExpressionImportlibMetadataBackportVersionCall(ExpressionChildHavingBase):
    ...
    def _replaceWithCompileTimeValue(self, trace_collection):
        ...
        PackageNotFoundError = _getImportlibMetadataBackportModule().PackageNotFoundError
        arg = self.subnode_distribution_name.getCompileTimeConstant()
        try:
            distribution = version(arg)
        except PackageNotFoundError:
            ...
            from .ConstantRefNodes import makeConstantRefNode
            result = makeConstantRefNode(constant=distribution, source_ref=self.source_ref)
            return (result, 'new_expression', "Compile time predicted 'importlib_metadata.version' result")
...
class ExpressionPkgResourcesDistributionValueRef(ExpressionNoSideEffectsMixin, ExpressionBase):
    ...
    def __init__(self, distribution, source_ref):
        with withNoDeprecationWarning():
            Distribution = importFromCompileTime('pkg_resources', must_exist=True).Distribution
    ...
    def computeExpressionAttribute(self, lookup_node, attribute_name, trace_collection):
        if self.computed_attribute is None:
            self.computed_attribute = hasattr(self.distribution, attribute_name)
        if not self.computed_attribute or isCompileTimeConstantValue(getattr(self.distribution, attribute_name, None)):
