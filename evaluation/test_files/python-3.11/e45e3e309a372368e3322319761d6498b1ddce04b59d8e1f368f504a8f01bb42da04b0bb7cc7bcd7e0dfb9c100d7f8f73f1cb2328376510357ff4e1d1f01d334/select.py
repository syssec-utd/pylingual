...
async def async_setup_entry(hass: HomeAssistant, config_entry: ConfigEntry, async_add_entities: AddEntitiesCallback) -> None:
    ...
    def async_add_select(info: ZwaveDiscoveryInfo) -> None:
        ...
        if info.platform_hint == 'Default tone':
            ...
            entities.append(ZwaveMultilevelSwitchSelectEntity(config_entry, driver, info))
        else:
            entities.append(ZwaveSelectEntity(config_entry, driver, info))
        async_add_entities(entities)
    config_entry.async_on_unload(async_dispatcher_connect(hass, f'{DOMAIN}_{config_entry.entry_id}_add_{SELECT_DOMAIN}', async_add_select))
...
class ZwaveSelectEntity(ZWaveBaseEntity, SelectEntity):
    ...
    def current_option(self) -> str | None:
        ...
        if self.info.primary_value.value is None:
            return None
        return str(self.info.primary_value.metadata.states.get(str(self.info.primary_value.value), self.info.primary_value.value))

    async def async_select_option(self, option: str) -> None:
        """Change the selected option."""
        key = next((key for key, val in self.info.primary_value.metadata.states.items() if val == option))
...
class ZwaveDefaultToneSelectEntity(ZWaveBaseEntity, SelectEntity):
    ...
    def __init__(self, config_entry: ConfigEntry, driver: Driver, info: ZwaveDiscoveryInfo) -> None:
        ...
        super().__init__(config_entry, driver, info)
        self._tones_value = self.get_zwave_value('toneId', command_class=CommandClass.SOUND_SWITCH)
    ...
    def options(self) -> list[str]:
        ...
        return [val for key, val in self._tones_value.metadata.states.items() if int(key) not in (ToneID.DEFAULT, ToneID.OFF)]

    @property
    def current_option(self) -> str | None:
        """Return the selected entity option to represent the entity state."""
        assert self._tones_value
        return str(self._tones_value.metadata.states.get(str(self.info.primary_value.value), self.info.primary_value.value))

    async def async_select_option(self, option: str) -> None:
        """Change the selected option."""
        assert self._tones_value
        key = next((key for key, val in self._tones_value.metadata.states.items() if val == option))
        await self.info.node.async_set_value(self.info.primary_value, int(key))
...
class ZwaveMultilevelSwitchSelectEntity(ZWaveBaseEntity, SelectEntity):
    ...
    def current_option(self) -> str | None:
        ...
        if self.info.primary_value.value is None:
            return None
        return str(self._lookup_map.get(int(self.info.primary_value.value), self.info.primary_value.value))
