...
def parse_html(html: str):
    ...
    for tag in selector.xpath('//div[@id="pages"]/div[position()>1]/table'):
        if (pic_url := tag.xpath('./tr[2]/td/a/img/@src')):
            pic_url = urljoin('http://iqdb.org/', pic_url[0])
        else:
            pic_url = '没有最相似的'
        similarity = tag.xpath('./tr[last()]/td/text()')[0]
        href: List[str] = tag.xpath('./tr/td/a/@href')
        href.extend(tag.xpath('./tr/td/span/a/@href'))
