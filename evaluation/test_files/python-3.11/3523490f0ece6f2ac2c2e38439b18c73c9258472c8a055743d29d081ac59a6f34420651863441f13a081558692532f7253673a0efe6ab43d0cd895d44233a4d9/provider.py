    ...
    def extract_images_from_workflow(self) -> list[Image]:
        ...
        for job_object in jobs.values():
            if isinstance(job_object, dict):
                ...
                if image:
                    image_obj = Image(file_path=self.file_path[self.file_path.index('/.github'):], name=image, start_line=start_line, end_line=end_line, related_resource_id=self.generate_resource_key(start_line, end_line))
