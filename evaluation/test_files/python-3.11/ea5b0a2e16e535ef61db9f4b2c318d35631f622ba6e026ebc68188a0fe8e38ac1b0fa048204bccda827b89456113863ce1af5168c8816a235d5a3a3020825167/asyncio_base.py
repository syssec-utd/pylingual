...
class AsyncioContextBase(abc.ABC):
    ...
    def _on_done(self, future, result):
        ...
        if future.done():
            return
        self.loop.call_soon_threadsafe(lambda: future.done() or future.set_result(True))

    def read(self, nbytes: int, fd: int, offset: int, priority: int=0) -> typing.Awaitable[bytes]:
        return self.submit(self.OPERATION_CLASS.read(nbytes, fd, offset, priority))

    def write(self, payload: bytes, fd: int, offset: int, priority: int=0) -> typing.Awaitable[int]:
        return self.submit(self.OPERATION_CLASS.write(payload, fd, offset, priority))
