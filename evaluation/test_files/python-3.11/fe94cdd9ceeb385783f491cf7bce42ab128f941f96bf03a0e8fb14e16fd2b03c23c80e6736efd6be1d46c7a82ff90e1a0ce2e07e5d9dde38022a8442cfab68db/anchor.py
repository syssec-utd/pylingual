...
class Anchor(object):
    ...
    def unpack_labels(self, labels: tf.Tensor) -> Dict[str, tf.Tensor]:
        ...
        for level in range(self.min_level, self.max_level + 1):
            ...
            unpacked_labels[str(level)] = tf.reshape(labels[count:count + steps], [feat_size_y, feat_size_x, -1])
            count += steps
...
class AnchorLabeler(object):
    ...
    """
        ...
        self.matcher = box_matcher.BoxMatcher(thresholds=[unmatched_threshold, match_threshold], indicators=[-1, -2, 1], force_match_for_each_col=True)
        self.box_coder = faster_rcnn_box_coder.FasterRcnnBoxCoder()

    def label_anchors(self, anchor_boxes: Dict[str, tf.Tensor], gt_boxes: tf.Tensor, gt_labels: tf.Tensor, gt_attributes: Optional[Dict[str, tf.Tensor]]=None, gt_weights: Optional[tf.Tensor]=None) -> Tuple[Dict[str, tf.Tensor], Dict[str, tf.Tensor], Dict[str, Dict[str, tf.Tensor]], tf.Tensor, tf.Tensor]:
    ...
    """
        ...
        weights = tf.cond(tf.greater(num_gt_labels, 0), lambda: tf.squeeze(tf.ones_like(gt_labels, dtype=tf.float32), -1), lambda: tf.ones([1], dtype=tf.float32))
        if gt_weights is not None:
            weights = tf.cond(tf.greater(num_gt_labels, 0), lambda: tf.math.multiply(weights, gt_weights), lambda: weights)
...
class RpnAnchorLabeler(AnchorLabeler):
    ...
    def __init__(self, match_threshold=0.7, unmatched_threshold=0.3, rpn_batch_size_per_im=256, rpn_fg_fraction=0.5):
    ...
    def label_anchors(self, anchor_boxes: Dict[str, tf.Tensor], gt_boxes: tf.Tensor, gt_labels: tf.Tensor) -> Tuple[Dict[str, tf.Tensor], Dict[str, tf.Tensor]]:
        """Labels anchors with ground truth inputs.
    ...
    """
        ...
        similarity_matrix = self.similarity_calc(flattened_anchor_boxes, gt_boxes)
        match_indices, match_indicators = self.matcher(similarity_matrix)
        box_mask = tf.tile(tf.expand_dims(tf.less_equal(match_indicators, 0), -1), [1, 4])
        box_targets = self.target_gather(gt_boxes, match_indices, box_mask)
        box_targets_list = box_list.BoxList(box_targets)
        anchor_box_list = box_list.BoxList(flattened_anchor_boxes)
        box_targets = self.box_coder.encode(box_targets_list, anchor_box_list)
        ...
        matched_anchors_mask = tf.tile(tf.expand_dims(matched_anchors_mask, 1), [1, tf.shape(box_targets)[1]])
        box_targets = tf.where(matched_anchors_mask, box_targets, unmatched_ignored_box_targets)
...
def build_anchor_generator(min_level, max_level, num_scales, aspect_ratios, anchor_size):
    ...
    anchor_gen = anchor_generator.AnchorGenerator(anchor_sizes=anchor_sizes, scales=scales, aspect_ratios=aspect_ratios, strides=strides)
    return anchor_gen

def unpack_targets(targets: tf.Tensor, anchor_boxes_dict: Dict[str, tf.Tensor]) -> Dict[str, tf.Tensor]:
    """Unpacks an array of labels into multiscales labels.
  ...
  """
    ...
    for level, anchor_boxes in anchor_boxes_dict.items():
        ...
        unpacked_targets[level] = tf.reshape(targets[count:count + steps], [feat_size_y, feat_size_x, -1])
        count += steps
