...
class Prompt:
    ...
    def __init__(self, template: Optional[str]=None, langchain_prompt: Optional[BaseLangchainPrompt]=None, langchain_prompt_selector: Optional[ConditionalPromptSelector]=None, stop_token: Optional[str]=None, **prompt_kwargs: Any) -> None:
        """Init params."""
        if langchain_prompt_selector is not None:
            ...
            if template is None:
                raise ValueError('`template` must be specified if `langchain_prompt` is None')
            tmpl_vars = {v for _, v, _, _ in Formatter().parse(template) if v is not None}
            if tmpl_vars != set(self.input_variables):
                raise ValueError(f'Invalid template: {template}, variables do not match the required input_variables: {self.input_variables}')
            self.prompt = LangchainPrompt(input_variables=self.input_variables, template=template, **prompt_kwargs)
            self.prompt_selector = ConditionalPromptSelector(default_prompt=self.prompt)
        else:
            if template:
                raise ValueError(f'Both template ({template}) and langchain_prompt ({langchain_prompt}) are provided, only one should be.')
        ...
        for lc_prompt in all_lc_prompts:
            if set(lc_prompt.input_variables) != set(self.input_variables):
                raise ValueError(f'Invalid prompt: {langchain_prompt}, variables do not match the required input_variables: {self.input_variables}')
        self.partial_dict: Dict[str, Any] = {}
        self.prompt_kwargs = prompt_kwargs
        self.stop_token = stop_token

    @classmethod
    def from_langchain_prompt(cls: Type[PMT], prompt: BaseLangchainPrompt, **kwargs: Any) -> PMT:
        """Load prompt from LangChain prompt."""
        return cls(langchain_prompt=prompt, **kwargs)

    @classmethod
    def from_langchain_prompt_selector(cls: Type[PMT], prompt_selector: ConditionalPromptSelector, **kwargs: Any) -> PMT:
        """Load prompt from LangChain prompt."""
    ...
    def partial_format(self: PMT, **kwargs: Any) -> PMT:
        ...
        for k in kwargs.keys():
            if k not in self.input_variables:
                raise ValueError(f'Invalid input variable: {k}, not found in input_variables')
        copy_obj = deepcopy(self)
        copy_obj.partial_dict.update(kwargs)
        return copy_obj

    @classmethod
    def from_prompt(cls: Type[PMT], prompt: 'Prompt', llm: Optional[BaseLanguageModel]=None) -> PMT:
    ...
    def get_langchain_prompt(self, llm: Optional[BaseLanguageModel]=None) -> BaseLangchainPrompt:
