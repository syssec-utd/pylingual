...
class SWFResponse(BaseResponse):
    ...
    def _check_exclusivity(self, **kwargs):
        ...
        if len(keys) == 2:
            ...
            message = 'Cannot specify more than one exclusive filters in the same query: {0}'.format(keys)
    ...
    def list_domains(self):
        ...
        return json.dumps({'domainInfos': [domain.to_short_dict() for domain in domains]})
    ...
    def list_closed_workflow_executions(self):
        ...
        self._check_exclusivity(executionFilter=execution_filter, typeFilter=type_filter, tagFilter=tag_filter, closeStatusFilter=close_status_filter)
        self._check_exclusivity(startTimeFilter=start_time_filter, closeTimeFilter=close_time_filter)
        ...
        workflow_executions = self.swf_backend.list_closed_workflow_executions(domain_name=domain, tag_filter=tag_filter, close_status_filter=close_status_filter, maximum_page_size=maximum_page_size, reverse_order=reverse_order)
        return json.dumps({'executionInfos': [wfe.to_list_dict() for wfe in workflow_executions]})
    ...
    def list_open_workflow_executions(self):
        ...
        self._check_string(domain)
        self._check_none_or_string(workflow_id)
        self._check_exclusivity(executionFilter=execution_filter, typeFilter=type_filter, tagFilter=tag_filter)
        self._check_float_or_int(start_time_filter['oldestDate'])
        if 'latestDate' in start_time_filter:
            self._check_float_or_int(start_time_filter['latestDate'])
        if tag_filter:
        ...
        if type_filter:
            ...
            self._check_string(type_filter['version'])
        self._check_int(maximum_page_size)
        workflow_executions = self.swf_backend.list_open_workflow_executions(domain_name=domain, maximum_page_size=maximum_page_size, tag_filter=tag_filter, reverse_order=reverse_order)
        return json.dumps({'executionInfos': [wfe.to_list_dict() for wfe in workflow_executions]})
    ...
    def register_workflow_type(self):
        ...
        self._check_none_or_string(description)
        self.swf_backend.register_type('workflow', domain, name, version, task_list=task_list, default_child_policy=default_child_policy, default_task_start_to_close_timeout=default_task_start_to_close_timeout, default_execution_start_to_close_timeout=default_execution_start_to_close_timeout, default_task_priority=default_task_priority, default_lambda_role=default_lambda_role, description=description)
        return ''
    ...
    def poll_for_decision_task(self):
        ...
        decision = self.swf_backend.poll_for_decision_task(domain_name, task_list, identity=identity)
        if decision:
    ...
    def respond_decision_task_completed(self):
        ...
        self.swf_backend.respond_decision_task_completed(task_token, decisions=decisions, execution_context=execution_context)
        return ''
    ...
    def poll_for_activity_task(self):
        ...
        activity_task = self.swf_backend.poll_for_activity_task(domain_name, task_list, identity=identity)
        if activity_task:
    ...
    def respond_activity_task_failed(self):
        ...
        self.swf_backend.respond_activity_task_failed(task_token, reason=reason, details=details)
        return ''
    ...
    def terminate_workflow_execution(self):
        ...
        self.swf_backend.terminate_workflow_execution(domain_name, workflow_id, child_policy=child_policy, details=details, reason=reason, run_id=run_id)
        return ''
    ...
    def signal_workflow_execution(self):
        ...
        self._check_none_or_string(run_id)
        self.swf_backend.signal_workflow_execution(domain_name, signal_name, workflow_id, _input, run_id)
        return ''