...
from localstack.services.cloudformation.deployment_utils import PLACEHOLDER_AWS_NO_VALUE, PLACEHOLDER_RESOURCE_NAME, dump_json_params, generate_default_name, param_defaults, remove_none_values, select_parameters
...
class IAMManagedPolicy(GenericBaseModel):
    ...
    def add_defaults(resource, stack_name: str):
        ...
        if not role_name:
            resource['Properties']['ManagedPolicyName'] = generate_default_name(stack_name, resource['LogicalResourceId'])
...
class IAMUser(GenericBaseModel):
    ...
    def update_resource(self, new_resource, stack_name, resources):
        ...
        return client.update_user(UserName=props.get('UserName'), NewPath=props.get('NewPath') or '', NewUserName=props.get('NewUserName') or '')

    @staticmethod
    def add_defaults(resource, stack_name: str):
        role_name = resource['Properties'].get('UserName')
        if not role_name:
            resource['Properties']['UserName'] = generate_default_name(stack_name, resource['LogicalResourceId'])
    ...
    def get_deploy_templates():
        ...
        def _post_create(resource_id, resources, resource_type, func, stack_name):
            ...
            for policy in props.get('Policies', []):
                ...
                client.put_user_policy(UserName=username, PolicyName=policy.get('PolicyName'), PolicyDocument=policy_doc)
            login_profile = props.get('LoginProfile')
            if login_profile:
                client.create_login_profile(UserName=username, Password=login_profile.get('Password'), PasswordResetRequired=login_profile.get('PasswordResetRequired'))
        ...
        def _pre_delete(resource_id, resources, resource_type, func, stack_name):
            ...
            for inline_policy in props.get('Policies', []):
                client.delete_user_policy(UserName=user_name, PolicyName=inline_policy.get('PolicyName'))
        ...
        return {'create': [{'function': 'create_user', 'parameters': ['Path', 'UserName', 'PermissionsBoundary', 'Tags']}, {'function': _post_create}], 'delete': [{'function': _pre_delete}, {'function': 'delete_user', 'parameters': ['UserName']}]}
...
class IAMRole(GenericBaseModel):
    ...
    def update_resource(self, new_resource, stack_name, resources):
        ...
        if _states:
            ...
            if name_changed or policy_changed:
                ...
                dummy_resources = {resource_id: {'Properties': {'RoleName': _states.get('RoleName')}}}
                self._pre_delete(resource_id, dummy_resources, None, None, None)
                client.delete_role(RoleName=_states.get('RoleName'))
                role = client.create_role(RoleName=props.get('RoleName'), AssumeRolePolicyDocument=str(props_policy))
    ...
    def add_defaults(resource, stack_name):
        ...
        if not role_name:
            resource['Properties']['RoleName'] = generate_default_name(stack_name, resource['LogicalResourceId'])

    @staticmethod
    def _post_create(resource_id, resources, resource_type, func, stack_name):
        """attaches managed policies from the template to the role"""
        from localstack.utils.cloudformation.template_deployer import find_stack, resolve_refs_recursively
        iam = aws_stack.connect_to_service('iam')
        resource = resources[resource_id]
        props = resource['Properties']
        ...
        for policy in inline_policies:
            ...
            if not isinstance(policy, dict):
                LOG.info('Invalid format of policy for IAM role "%s": %s', props.get('RoleName'), policy)
                continue
            ...
            doc = json.dumps(doc)
            iam.put_role_policy(RoleName=props['RoleName'], PolicyName=pol_name, PolicyDocument=doc)
    ...
    def _pre_delete(resource_id, resources, resource_type, func, stack_name):
        ...
        try:
            ...
            for instance_profile in rs['InstanceProfiles']:
                ...
                call_safe(iam_client.remove_role_from_instance_profile, kwargs={'InstanceProfileName': ip_name, 'RoleName': role_name})
    ...
    @classmethod
    def get_deploy_templates(cls):
        return {'create': [{'function': 'create_role', 'parameters': param_defaults(dump_json_params(select_parameters('Path', 'RoleName', 'AssumeRolePolicyDocument', 'Description', 'MaxSessionDuration', 'PermissionsBoundary', 'Tags'), 'AssumeRolePolicyDocument'), {'RoleName': PLACEHOLDER_RESOURCE_NAME})}, {'function': IAMRole._post_create}], 'delete': [{'function': IAMRole._pre_delete}, {'function': 'delete_role', 'parameters': {'RoleName': 'RoleName'}}]}
...
class IAMServiceLinkedRole(GenericBaseModel):
    ...
    def fetch_state(self, stack_name, resources):
        iam = aws_stack.connect_to_service('iam')
        service_name = self.resolve_refs_recursively(stack_name, self.props['AWSServiceName'], resources)
    ...
    @classmethod
    def get_deploy_templates(cls):
        return {'create': {'function': 'create_service_linked_role'}, 'delete': {'function': 'delete_service_linked_role', 'parameters': ['RoleName']}}
...
class IAMPolicy(GenericBaseModel):
    ...
    def get_deploy_templates(cls):
        ...
        return {'create': {'function': _create}, 'delete': {'function': 'delete_policy', 'parameters': _delete_params}}
    ...
    def get_policy_state(cls, obj, stack_name, resources, managed_policy=False):
        ...
        for role in roles:
            ...
            policies = _filter(iam.list_attached_role_policies(RoleName=role)) if managed_policy else iam.get_role_policy(RoleName=role, PolicyName=policy_name)
            result['role:%s' % role] = policies
        for user in users:
            user = obj.resolve_refs_recursively(stack_name, user, resources)
            policies = _filter(iam.list_attached_user_policies(UserName=user)) if managed_policy else iam.get_user_policy(UserName=user, PolicyName=policy_name)
            result['user:%s' % user] = policies
        for group in groups:
            group = obj.resolve_refs_recursively(stack_name, group, resources)
            policies = _filter(iam.list_attached_group_policies(GroupName=group)) if managed_policy else iam.get_group_policy(GroupName=group, PolicyName=policy_name)
...
class InstanceProfile(GenericBaseModel):
    ...
    def add_defaults(resource, stack_name):
        ...
        if not role_name:
            resource['Properties']['InstanceProfileName'] = generate_default_name(stack_name, resource['LogicalResourceId'])
    ...
    def get_deploy_templates():
        ...
        def _add_roles(resource_id, resources, resource_type, func, stack_name):
            ...
            if roles:
                ...
                client.add_role_to_instance_profile(InstanceProfileName=props['InstanceProfileName'], RoleName=roles[0])
        ...
        def _pre_delete(resource_id, resources, resource_type, func, stack_name):
            ...
            assert len(roles) == 1
            try:
                iam_client.remove_role_from_instance_profile(InstanceProfileName=props['InstanceProfileName'], RoleName=roles[0])
            except Exception as e:
                if 'NoSuchEntity' not in str(e):
                    raise
        return {'create': [{'function': 'create_instance_profile', 'parameters': {'InstanceProfileName': 'InstanceProfileName', 'Path': 'Path'}}, {'function': _add_roles}], 'delete': [{'function': _pre_delete}, {'function': 'delete_instance_profile', 'parameters': {'InstanceProfileName': 'InstanceProfileName'}}]}
...
class IAMGroup(GenericBaseModel):
    ...
    def update_resource(self, new_resource, stack_name, resources):
        props = new_resource['Properties']
        return aws_stack.connect_to_service('iam').update_group(GroupName=props.get('GroupName'), NewPath=props.get('NewPath') or '', NewGroupName=props.get('NewGroupName') or '')
    ...
    def get_deploy_templates():
        ...
        def _post_create(resource_id, resources, resource_type, func, stack_name):
            ...
            for inline_policy in props.get('Policies', []):
                ...
                client.put_group_policy(GroupName=group_name, PolicyName=inline_policy.get('PolicyName'), PolicyDocument=doc)
        ...
        def _pre_delete(resource_id, resources, resource_type, func, stack_name):
            ...
            for inline_policy in props.get('Policies', []):
                client.delete_group_policy(GroupName=group_name, PolicyName=inline_policy.get('PolicyName'))
        ...
        return {'create': [{'function': 'create_group', 'parameters': ['GroupName', 'Path']}, {'function': _post_create}], 'delete': [{'function': _pre_delete}, {'function': 'delete_group', 'parameters': ['GroupName']}]}