...
def fastmnmf(X, n_src=None, n_iter=30, n_components=8, mic_index=0, W0=None, accelerate=True, callback=None):
    """
    ...
    def separate():
        ...
        if mic_index == 'all':
            return np.einsum('fij, ftj, nftj -> itfn', Qinv_FMM, Qx_FTM / Y_NFTM.sum(axis=0), Y_NFTM)
        elif type(mic_index) is int:
            return np.einsum('fj, ftj, nftj -> tfn', Qinv_FMM[:, mic_index], Qx_FTM / Y_NFTM.sum(axis=0), Y_NFTM)
        else:
    ...
    for epoch in range(n_iter):
        ...
        if interval_update_Q <= 0 or epoch % interval_update_Q == 0:
            for m in range(n_chan):
                V_FMM = np.einsum('ftij, ft -> fij', XX_FTMM, 1 / Y_FTM[..., m]) / n_frames
                try:
                    ...
                    rhs = np.broadcast_to(np.eye(n_chan)[None, :, [m]], (mat_inv.shape[0], n_chan, 1))
                    tmp_FM = np.matmul(mat_inv, rhs)
                    tmp_FM = tmp_FM[..., 0]
                Q_FMM[:, m] = (tmp_FM / (np.sqrt(np.einsum('fi, fij, fj -> f', tmp_FM.conj(), V_FMM, tmp_FM))[:, None] + eps)).conj()
                Qx_power_FTM = np.abs(np.einsum('fij, ftj -> fti', Q_FMM, X_FTM)) ** 2
