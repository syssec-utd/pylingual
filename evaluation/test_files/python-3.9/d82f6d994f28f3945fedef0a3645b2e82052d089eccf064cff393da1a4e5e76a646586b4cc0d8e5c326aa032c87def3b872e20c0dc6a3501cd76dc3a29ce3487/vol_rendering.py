...
def rendering(t_starts: torch.Tensor, t_ends: torch.Tensor, ray_indices: torch.Tensor, n_rays: int, rgb_sigma_fn: Optional[Callable]=None, rgb_alpha_fn: Optional[Callable]=None, render_bkgd: Optional[torch.Tensor]=None) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    Args:
        ...
        rgb_sigma_fn: A function that takes in samples {t_starts (N, 1), t_ends (N, 1),             ray indices (N,)} and returns the post-activation rgb (N, 3) and density             values (N, 1). 
        rgb_alpha_fn: A function that takes in samples {t_starts (N, 1), t_ends (N, 1),             ray indices (N,)} and returns the post-activation rgb (N, 3) and opacity             values (N, 1).
    ...
    if rgb_sigma_fn is not None:
        ...
        assert sigmas.shape == t_starts.shape, 'sigmas must have shape of (N, 1)! Got {}'.format(sigmas.shape)
        weights = render_weight_from_density(t_starts, t_ends, sigmas, ray_indices=ray_indices, n_rays=n_rays)
    elif rgb_alpha_fn is not None:
        (rgbs, alphas) = rgb_alpha_fn(t_starts, t_ends, ray_indices.long())
        assert rgbs.shape[-1] == 3, 'rgbs must have 3 channels, got {}'.format(rgbs.shape)
        assert alphas.shape == t_starts.shape, 'alphas must have shape of (N, 1)! Got {}'.format(alphas.shape)
    ...
    depths = accumulate_along_rays(weights, ray_indices, values=(t_starts + t_ends) / 2.0, n_rays=n_rays)
    if render_bkgd is not None:
        colors = colors + render_bkgd * (1.0 - opacities)
    return (colors, opacities, depths)

def accumulate_along_rays(weights: Tensor, ray_indices: Tensor, values: Optional[Tensor]=None, n_rays: Optional[int]=None) -> Tensor:
    ...
    Args:
        ...
        ray_indices: Ray index of each sample. IntTensor with shape (n_samples).
        values: The values to be accmulated. Tensor with shape (n_samples, D). If             None, the accumulated values are just weights. Default is None.
        n_rays: Total number of rays. This will decide the shape of the ouputs. If             None, it will be inferred from `ray_indices.max() + 1`.  If specified             it should be at least larger than `ray_indices.max()`. Default is None.
    ...
    if values is not None:
        assert values.dim() == 2 and values.shape[0] == weights.shape[0], 'Invalid shapes: {} vs {}'.format(values.shape, weights.shape)
        src = weights * values
    else:
        src = weights
...
def render_transmittance_from_density(t_starts: Tensor, t_ends: Tensor, sigmas: Tensor, *, packed_info: Optional[torch.Tensor]=None, ray_indices: Optional[torch.Tensor]=None, n_rays: Optional[int]=None) -> Tensor:
    ...
    Args:
        t_starts: Where the frustum-shape sample starts along a ray. Tensor with             shape (n_samples, 1).
        t_ends: Where the frustum-shape sample ends along a ray. Tensor with             shape (n_samples, 1).
        sigmas: The density values of the samples. Tensor with shape (n_samples, 1).
        packed_info: Optional. Stores information on which samples belong to the same ray.             See :func:`nerfacc.ray_marching` for details. LongTensor with shape (n_rays, 2).
        ray_indices: Optional. Ray index of each sample. LongTensor with shape (n_sample).
        n_rays: Optional. Number of rays. Only useful when `ray_indices` is provided yet             CUB acceleration is not available. We will implicitly convert `ray_indices` to             `packed_info` and use the naive implementation. If not provided, we will infer             it from `ray_indices` but it will be slower.
    ...
    """
    assert ray_indices is not None or packed_info is not None, 'Either ray_indices or packed_info should be provided.'
    if ray_indices is not None and _C.is_cub_available():
        transmittance = _RenderingTransmittanceFromDensityCUB.apply(ray_indices, t_starts, t_ends, sigmas)
    else:
        if packed_info is None:
            packed_info = pack_info(ray_indices, n_rays=n_rays)
        transmittance = _RenderingTransmittanceFromDensityNaive.apply(packed_info, t_starts, t_ends, sigmas)
    return transmittance

def render_transmittance_from_alpha(alphas: Tensor, *, packed_info: Optional[torch.Tensor]=None, ray_indices: Optional[torch.Tensor]=None, n_rays: Optional[int]=None) -> Tensor:
    ...
    assert ray_indices is not None or packed_info is not None, 'Either ray_indices or packed_info should be provided.'
    if ray_indices is not None and _C.is_cub_available():
        transmittance = _RenderingTransmittanceFromAlphaCUB.apply(ray_indices, alphas)
    else:
        if packed_info is None:
            packed_info = pack_info(ray_indices, n_rays=n_rays)
...
def render_weight_from_density(t_starts: Tensor, t_ends: Tensor, sigmas: Tensor, *, packed_info: Optional[torch.Tensor]=None, ray_indices: Optional[torch.Tensor]=None, n_rays: Optional[int]=None) -> torch.Tensor:
    ...
    if ray_indices is not None and _C.is_cub_available():
        ...
        if packed_info is None:
            packed_info = pack_info(ray_indices, n_rays=n_rays)
        weights = _RenderingWeightFromDensityNaive.apply(packed_info, t_starts, t_ends, sigmas)
    return weights

def render_weight_from_alpha(alphas: Tensor, *, packed_info: Optional[torch.Tensor]=None, ray_indices: Optional[torch.Tensor]=None, n_rays: Optional[int]=None) -> torch.Tensor:
...
@torch.no_grad()
def render_visibility(alphas: torch.Tensor, *, ray_indices: Optional[torch.Tensor]=None, packed_info: Optional[torch.Tensor]=None, n_rays: Optional[int]=None, early_stop_eps: float=0.0001, alpha_thre: float=0.0) -> torch.Tensor:
...
class _RenderingTransmittanceFromDensityCUB(torch.autograd.Function):
    ...
    def forward(ctx, ray_indices, t_starts, t_ends, sigmas):
        ...
        transmittance = _C.transmittance_from_sigma_forward_cub(ray_indices, t_starts, t_ends, sigmas)
    ...
    def backward(ctx, transmittance_grads):
        ...
        grad_sigmas = _C.transmittance_from_sigma_backward_cub(ray_indices, t_starts, t_ends, transmittance, transmittance_grads)
...
class _RenderingTransmittanceFromDensityNaive(torch.autograd.Function):
    ...
    def forward(ctx, packed_info, t_starts, t_ends, sigmas):
        ...
        t_ends = t_ends.contiguous()
        sigmas = sigmas.contiguous()
        transmittance = _C.transmittance_from_sigma_forward_naive(packed_info, t_starts, t_ends, sigmas)
        if ctx.needs_input_grad[3]:
    ...
    def backward(ctx, transmittance_grads):
        ...
        grad_sigmas = _C.transmittance_from_sigma_backward_naive(packed_info, t_starts, t_ends, transmittance, transmittance_grads)
        return (None, None, None, grad_sigmas)
...
class _RenderingTransmittanceFromAlphaCUB(torch.autograd.Function):
    ...
    def forward(ctx, ray_indices, alphas):
        ...
        transmittance = _C.transmittance_from_alpha_forward_cub(ray_indices, alphas)
    ...
    def backward(ctx, transmittance_grads):
        ...
        grad_alphas = _C.transmittance_from_alpha_backward_cub(ray_indices, alphas, transmittance, transmittance_grads)
...
class _RenderingTransmittanceFromAlphaNaive(torch.autograd.Function):
    ...
    def forward(ctx, packed_info, alphas):
        packed_info = packed_info.contiguous().int()
        alphas = alphas.contiguous()
        transmittance = _C.transmittance_from_alpha_forward_naive(packed_info, alphas)
        if ctx.needs_input_grad[1]:
    ...
    def backward(ctx, transmittance_grads):
        ...
        grad_alphas = _C.transmittance_from_alpha_backward_naive(packed_info, alphas, transmittance, transmittance_grads)
        return (None, grad_alphas)
...
class _RenderingWeightFromDensityNaive(torch.autograd.Function):
    ...
    def forward(ctx, packed_info, t_starts, t_ends, sigmas):
        ...
        t_ends = t_ends.contiguous()
        sigmas = sigmas.contiguous()
        weights = _C.weight_from_sigma_forward_naive(packed_info, t_starts, t_ends, sigmas)
        if ctx.needs_input_grad[3]:
            ctx.save_for_backward(packed_info, t_starts, t_ends, sigmas, weights)
        return weights
    ...
    def backward(ctx, grad_weights):
        ...
        grad_sigmas = _C.weight_from_sigma_backward_naive(weights, grad_weights, packed_info, t_starts, t_ends, sigmas)
        return (None, None, None, grad_sigmas)
...
class _RenderingWeightFromAlphaNaive(torch.autograd.Function):
    ...
    def backward(ctx, grad_weights):
        ...
        grad_alphas = _C.weight_from_alpha_backward_naive(weights, grad_weights, packed_info, alphas)
        return (None, grad_alphas)