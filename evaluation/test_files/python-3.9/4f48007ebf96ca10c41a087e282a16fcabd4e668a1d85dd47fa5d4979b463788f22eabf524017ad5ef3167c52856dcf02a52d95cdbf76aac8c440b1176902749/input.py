...
def match_filenames_once(pattern, name=None):
  ...
  """
    with ops.name_scope(name, 'matching_filenames', [pattern]) as name:
        return vs.variable(name=name, initial_value=io_ops.matching_files(pattern), trainable=False, validate_shape=False, collections=[ops.GraphKeys.LOCAL_VARIABLES])
...
def limit_epochs(tensor, num_epochs=None, name=None):
  ...
  """
    ...
    with ops.name_scope(name, 'limit_epochs', [tensor]) as name:
        ...
        epochs = vs.variable(zero64, name='epochs', trainable=False, collections=[ops.GraphKeys.LOCAL_VARIABLES])
...
def string_input_producer(string_tensor, num_epochs=None, shuffle=True, seed=None, capacity=32, shared_name=None, name=None, cancel_op=None):
  ...
  """
    ...
    with ops.name_scope(name, 'input_producer', [string_tensor]) as name:
        ...
        with ops.control_dependencies([control_flow_ops.Assert(math_ops.greater(array_ops.size(string_tensor), 0), [not_null_err])]):
...
def _validate_join(tensor_list_list):
    tensor_list_list = [ops.convert_n_to_tensor_or_indexed_slices(tl) for tl in tensor_list_list]
...
def _shapes(tensor_list_list, shapes, enqueue_many):
  ...
  """
    if shapes is None:
        ...
        shapes = [_merge_shapes([tl[i].shape.as_list() for tl in tensor_list_list], enqueue_many) for i in xrange(len0)]
    return shapes
...
def _batch(tensors, batch_size, keep_input, num_threads=1, capacity=32, enqueue_many=False, shapes=None, dynamic_pad=False, allow_smaller_final_batch=False, shared_name=None, name=None):
...
def batch(tensors, batch_size, num_threads=1, capacity=32, enqueue_many=False, shapes=None, dynamic_pad=False, allow_smaller_final_batch=False, shared_name=None, name=None):
...
def shuffle_batch(tensors, batch_size, capacity, min_after_dequeue, num_threads=1, seed=None, enqueue_many=False, shapes=None, allow_smaller_final_batch=False, shared_name=None, name=None):
  ...
  *N.B.:* You must ensure that either (i) the `shapes` argument is
  ...
  @end_compatibility
  """
    return _shuffle_batch(tensors, batch_size, capacity, min_after_dequeue, keep_input=True, num_threads=num_threads, seed=seed, enqueue_many=enqueue_many, shapes=shapes, allow_smaller_final_batch=allow_smaller_final_batch, shared_name=shared_name, name=name)
