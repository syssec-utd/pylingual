...
from ethpm.exceptions import EthPMValidationError, InsufficientAssetsError
...
def validate_package_version(version: Any) -> None:
    ...
    if not is_text(version):
        raise EthPMValidationError(f'Expected a version of text type, instead received {type(version)}.')
...
def validate_manifest_version(version: str) -> None:
    ...
    if not version == 'ethpm/3':
        raise EthPMValidationError(f'Py-EthPM does not support the provided specification version: {version}')
