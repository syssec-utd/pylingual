...
def test_basefee():
    ...
    environment = Environment(account, None, None, None, None, None, basefee=symbol_factory.BitVecSym('gasfee', 256))
    og_state = GlobalState(world_state, environment, None, MachineState(gas_limit=8000000))
    og_state.transaction_stack.append((MessageCallTransaction(world_state=WorldState(), gas_limit=8000000), None))
