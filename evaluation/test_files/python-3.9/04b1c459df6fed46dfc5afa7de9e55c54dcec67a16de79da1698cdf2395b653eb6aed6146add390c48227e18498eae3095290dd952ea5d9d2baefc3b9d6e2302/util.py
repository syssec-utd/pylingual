...
from homeassistant.components.light import ATTR_BRIGHTNESS, ATTR_BRIGHTNESS_PCT, ATTR_COLOR_NAME, ATTR_COLOR_TEMP, ATTR_COLOR_TEMP_KELVIN, ATTR_HS_COLOR, ATTR_KELVIN, ATTR_RGB_COLOR, ATTR_XY_COLOR
...
def lifx_features(bulb: Light) -> dict[str, Any]:
    ...
    features: dict[str, Any] = products.features_map.get(bulb.product) or products.features_map[1]
    return features
...
def find_hsbk(hass: HomeAssistant, **kwargs: Any) -> list[float | int | None] | None:
    ...
    if (color_name := kwargs.get(ATTR_COLOR_NAME)) is not None:
        try:
            (hue, saturation) = color_util.color_RGB_to_hs(*color_util.color_name_to_rgb(color_name))
        except ValueError:
            _LOGGER.warning('Got unknown color %s, falling back to neutral white', color_name)
            (hue, saturation) = (0, 0)
    ...
    if ATTR_KELVIN in kwargs:
        _LOGGER.warning("The 'kelvin' parameter is deprecated. Please use 'color_temp_kelvin' for all service calls")
        kelvin = kwargs.pop(ATTR_KELVIN)
        saturation = 0
    if ATTR_COLOR_TEMP in kwargs:
        kelvin = color_util.color_temperature_mired_to_kelvin(kwargs.pop(ATTR_COLOR_TEMP))
        saturation = 0
...
def mac_matches_serial_number(mac_addr: str, serial_number: str) -> bool:
    ...
    return bool(formatted_serial(serial_number) == formatted_mac or _get_mac_offset(serial_number, 1) == formatted_mac)
