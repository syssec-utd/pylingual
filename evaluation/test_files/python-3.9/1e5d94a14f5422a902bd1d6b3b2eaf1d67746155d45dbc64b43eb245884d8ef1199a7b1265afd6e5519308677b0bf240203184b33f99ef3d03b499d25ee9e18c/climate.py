...
from homeassistant.components.climate.const import FAN_AUTO, FAN_HIGH, FAN_LOW, FAN_MEDIUM, ClimateEntityFeature, HVACMode
...
OP_MODES = [HVACMode.HEAT, HVACMode.COOL, HVACMode.DRY, HVACMode.FAN_ONLY, HVACMode.OFF]
...
class MelissaClimate(ClimateEntity):
    ...
    _attr_supported_features = ClimateEntityFeature.FAN_MODE | ClimateEntityFeature.TARGET_TEMPERATURE
    ...
    def hvac_mode(self) -> HVACMode | None:
        ...
        if self._cur_settings is None:
            return None
        is_on = self._cur_settings[self._api.STATE] in (self._api.STATE_ON, self._api.STATE_IDLE)
        if not is_on:
            return HVACMode.OFF
    ...
    async def async_set_hvac_mode(self, hvac_mode: HVACMode) -> None:
        ...
        await self.async_send({self._api.MODE: mode, self._api.STATE: self._api.STATE_ON})
    ...
    async def async_update(self):
        ...
        try:
            self._data = (await self._api.async_status(cached=True))[self._serial_number]
            self._cur_settings = (await self._api.async_cur_settings(self._serial_number))['controller']['_relation']['command_log']
        except KeyError:
