...
class Mosaic:
    ...
    def __init__(self, output_size, mosaic_frequency=1.0, mosaic9_frequency=0.0, mixup_frequency=0.0, letter_box=True, jitter=0.0, mosaic_crop_mode='scale', mosaic_center=0.25, aug_scale_min=1.0, aug_scale_max=1.0, aug_rand_angle=0.0, aug_rand_perspective=0.0, aug_rand_translate=0.0, random_pad=False, random_flip=False, area_thresh=0.1, pad_value=preprocessing_ops.PAD_VALUE, seed=None):
    ...
    def _generate_cut(self, num_tiles):
        ...
        if self._mosaic_crop_mode == 'crop':
            ...
            cut_x = preprocessing_ops.random_uniform_strong(self._output_size[1] * min_offset, self._output_size[1] * (1 - min_offset), seed=self._seed)
            cut_y = preprocessing_ops.random_uniform_strong(self._output_size[0] * min_offset, self._output_size[0] * (1 - min_offset), seed=self._seed)
            cut = [cut_y, cut_x]
            ishape = tf.convert_to_tensor([self._output_size[0], self._output_size[1], 3])
        else:
            cut = None
            ishape = tf.convert_to_tensor([self._output_size[0] * num_tiles, self._output_size[1] * num_tiles, 3])
        return (cut, ishape)
    ...
    def scale_boxes(self, patch, ishape, boxes, x_offset, y_offset):
        ...
        boxes = boxes + tf.cast([y_offset, x_offset, y_offset, x_offset], boxes.dtype)
    ...
    def _augment_image(self, image, boxes, classes, is_crowd, area, xs=0.0, ys=0.0, cut=None, letter_box=False):
        ...
        (image, infos, crop_points) = preprocessing_ops.resize_and_jitter_image(image, [self._output_size[0], self._output_size[1]], random_pad=False, letter_box=letter_box, jitter=self._random_crop, shiftx=xs, shifty=ys, cut=cut, seed=self._seed)
        (boxes, inds) = preprocessing_ops.transform_and_clip_boxes(boxes, infos, area_thresh=self._area_thresh, shuffle_boxes=False, filter_and_clip_boxes=True, seed=self._seed)
    ...
    def _mosaic_crop_image(self, image, boxes, classes, is_crowd, area):
        ...
        if self._mosaic_crop_mode != 'crop':
            ...
            ch = tf.math.round(preprocessing_ops.random_uniform_strong(-center[0], center[0], seed=self._seed))
            cw = tf.math.round(preprocessing_ops.random_uniform_strong(-center[1], center[1], seed=self._seed))
            image = augment.translate(image, [cw, ch], fill_value=self._pad_value, fill_mode='constant')
        ...
        (image, _, affine) = preprocessing_ops.affine_warp_image(image, [self._output_size[0], self._output_size[1]], scale_min=self._aug_scale_min, scale_max=self._aug_scale_max, translate=self._aug_rand_translate, degrees=self._aug_rand_angle, perspective=self._aug_rand_perspective, random_pad=self._random_pad, seed=self._seed)
        (height, width) = (self._output_size[0], self._output_size[1])
        image = tf.image.resize(image, (height, width))
        (boxes, inds) = preprocessing_ops.transform_and_clip_boxes(boxes, None, affine=affine, area_thresh=self._area_thresh, seed=self._seed)
        (classes, is_crowd, area) = self._select_ind(inds, classes, is_crowd, area)
        return (image, boxes, classes, is_crowd, area, area)

    def _process_image(self, sample, shiftx, shifty, cut, letter_box):
        """Process and augment an image."""
        (image, boxes, classes, is_crowd, area, crop_points) = self._augment_image(sample['image'], sample['groundtruth_boxes'], sample['groundtruth_classes'], sample['groundtruth_is_crowd'], sample['groundtruth_area'], shiftx, shifty, cut, letter_box)
        sample = sample.copy()
        sample['image'] = image
    ...
    def _update_patched_sample(self, sample, image, boxes, classes, is_crowds, areas):
        ...
        if self._mosaic_crop_mode is not None:
            (image, boxes, classes, is_crowds, areas, _) = self._mosaic_crop_image(image, boxes, classes, is_crowds, areas)
    ...
    def _patch(self, patches, ishape, num_rows, num_cols):
        ...
        for row_idx in range(num_rows):
            row_patches = [patches[row_idx * num_cols + col_idx]['image'] for col_idx in range(num_cols)]
        ...
        for row_idx in range(num_rows):
            for col_idx in range(num_cols):
                ...
                transformed_boxes = self.scale_boxes(patch['image'], ishape, patch['groundtruth_boxes'], col_idx / num_cols, row_idx / num_rows)
        ...
        return self._update_patched_sample(patches[0], image, boxes, classes, is_crowds, areas)
    ...
    def _mosaic(self, *patch_samples):
        ...
        if self._mosaic_frequency >= 1.0:
            ...
            mosaic_prob = preprocessing_ops.random_uniform_strong(0.0, 1.0, dtype=tf.float32, seed=self._seed)
            sample = patch_samples[0].copy()
        if mosaic_prob >= 1 - self._mosaic_frequency:
            mosaic9_prob = preprocessing_ops.random_uniform_strong(0.0, 1.0, dtype=tf.float32, seed=self._seed + 1)
            if self._mosaic9_frequency > 0 and mosaic9_prob >= 1 - self._mosaic9_frequency:
    ...
    def _mosaic4(self, *samples):
        ...
        samples = [self._process_image(samples[0], 1.0, 1.0, cut, letter_box=self._letter_box), self._process_image(samples[1], 0.0, 1.0, cut, letter_box=self._letter_box), self._process_image(samples[2], 1.0, 0.0, cut, letter_box=self._letter_box), self._process_image(samples[3], 0.0, 0.0, cut, letter_box=self._letter_box)]
    ...
    def _mosaic9(self, *samples):
        ...
        samples = [self._process_image(samples[0], 1.0, 1.0, cut, letter_box=self._letter_box), self._process_image(samples[1], 0.0, 0.0, cut, letter_box=False), self._process_image(samples[2], 0.0, 1.0, cut, letter_box=self._letter_box), self._process_image(samples[3], 0.0, 0.0, cut, letter_box=False), self._process_image(samples[4], 0.0, 0.0, cut, letter_box=False), self._process_image(samples[5], 0.0, 0.0, cut, letter_box=False), self._process_image(samples[6], 1.0, 0.0, cut, letter_box=self._letter_box), self._process_image(samples[7], 0.0, 0.0, cut, letter_box=False), self._process_image(samples[8], 0.0, 0.0, cut, letter_box=self._letter_box)]
    ...
    def _mixup(self, one, two):
        ...
        if self._mixup_frequency >= 1.0:
            ...
            domo = preprocessing_ops.random_uniform_strong(0.0, 1.0, dtype=tf.float32, seed=self._seed)
            noop = one.copy()
        if domo >= 1 - self._mixup_frequency:
            sample = one
            otype = one['image'].dtype
            r = self._beta(8.0, 8.0)
            sample['image'] = r * tf.cast(one['image'], tf.float32) + (1 - r) * tf.cast(two['image'], tf.float32)
            sample['image'] = tf.cast(sample['image'], otype)
            sample['groundtruth_boxes'] = tf.concat([one['groundtruth_boxes'], two['groundtruth_boxes']], axis=0)
            sample['groundtruth_classes'] = tf.concat([one['groundtruth_classes'], two['groundtruth_classes']], axis=0)
            sample['groundtruth_is_crowd'] = tf.concat([one['groundtruth_is_crowd'], two['groundtruth_is_crowd']], axis=0)
            sample['groundtruth_area'] = tf.concat([one['groundtruth_area'], two['groundtruth_area']], axis=0)
            return sample
        else:
    ...
    def _apply(self, dataset):
        ...
        for i in range(num_patches):
            patch_datasets.append(dataset.shuffle(100, seed=self._seed + i, reshuffle_each_iteration=True))
        dataset = tf.data.Dataset.zip(tuple(patch_datasets))
        dataset = dataset.map(self._mosaic, num_parallel_calls=tf.data.AUTOTUNE, deterministic=determ)
        if self._mixup_frequency > 0:
            one = dataset.shuffle(100, seed=self._seed + num_patches, reshuffle_each_iteration=True)
            two = dataset.shuffle(100, seed=self._seed + num_patches + 1, reshuffle_each_iteration=True)
            dataset = tf.data.Dataset.zip((one, two))
            dataset = dataset.map(self._mixup, num_parallel_calls=tf.data.AUTOTUNE, deterministic=determ)
        return dataset
