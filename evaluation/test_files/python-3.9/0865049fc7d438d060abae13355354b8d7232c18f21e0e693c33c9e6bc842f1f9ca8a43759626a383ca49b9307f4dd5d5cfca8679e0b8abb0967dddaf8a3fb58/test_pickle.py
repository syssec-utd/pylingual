...
pytestmark = pytest.mark.filterwarnings('ignore:Timestamp.freq is deprecated:FutureWarning')

@pytest.fixture(scope='module')
def current_pickle_data():
    from pandas.tests.io.generate_legacy_storage_files import create_pickle_data
    with catch_warnings():
        filterwarnings('ignore', "The 'freq' argument in Timestamp", category=FutureWarning)
...
class TestCompression:
    _extension_to_compression = {ext: compression for (compression, ext) in icom._compression_to_extension.items()}
...
@pytest.mark.parametrize(['pickle_file', 'excols'], [('test_py27.pkl', Index(['a', 'b', 'c'])), ('test_mi_py27.pkl', pd.MultiIndex.from_arrays([['a', 'b', 'c'], ['A', 'B', 'C']]))])
