...
class PkgConfigGeneratorWithComponentsTest(unittest.TestCase):
    ...
    def test_component_not_found_same_name_as_pkg_require(self):
        zlib = GenConanfile('zlib', '0.1').with_setting('build_type').with_generator('pkg_config')
        mypkg = GenConanfile('mypkg', '0.1').with_setting('build_type').with_generator('pkg_config')
        final = GenConanfile('final', '0.1').with_setting('build_type').with_generator('pkg_config').with_require(ConanFileReference('zlib', '0.1', None, None)).with_require(ConanFileReference('mypkg', '0.1', None, None)).with_package_info(cpp_info={'components': {'cmp': {'requires': ['mypkg::zlib', 'zlib::zlib']}}}, env_info={})
        consumer = GenConanfile('consumer', '0.1').with_setting('build_type').with_generator('pkg_config').with_requirement(ConanFileReference('final', '0.1', None, None))
        client = TestClient()
