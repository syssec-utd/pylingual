...
class DatasetCreator:
    ...
    def __init__(self, dataset_fn, input_options=None):
        if not callable(dataset_fn):
            raise TypeError(f'`dataset_fn` for `DatasetCreator` must be a `callable`. Received: {dataset_fn}')
        if input_options and (not isinstance(input_options, tf.distribute.InputOptions)):
            raise TypeError(f'`input_options` for `DatasetCreator` must be a `tf.distribute.InputOptions`. Received: {input_options}')
    ...
    def __call__(self, *args, **kwargs):
        ...
        if not isinstance(dataset, tf.data.Dataset):
            raise TypeError(f'The `callable` provided to `DatasetCreator` must return a Dataset. It returns "{dataset}"')
        return dataset