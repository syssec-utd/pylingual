...
def build_measurement_error_mitigation_circuits(qubit_list, fitter_cls, backend, backend_config=None, compile_config=None):
    ...
    """
    try:
        from spinqkit.qiskit.ignis.mitigation.measurement import complete_meas_cal, CompleteMeasFitter, TensoredMeasFitter
    except ImportError as ex:
        raise MissingOptionalLibraryError(libname='qiskit-ignis', name='build_measurement_error_mitigation_qobj', pip_install='pip install qiskit-ignis') from ex
    circlabel = 'mcal'
    if not qubit_list:
...
def build_measurement_error_mitigation_qobj(qubit_list, fitter_cls, backend, backend_config=None, compile_config=None, run_config=None):
    """
    Args:
        qubit_list (list[int]): list of ordered qubits used in the algorithm
        fitter_cls (callable): CompleteMeasFitter or TensoredMeasFitter
