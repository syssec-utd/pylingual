...
from ydb.resolver import DiscoveryResult, DiscoveryEndpointsResolver as _DiscoveryEndpointsResolver, _list_endpoints_request_factory
...
class DiscoveryEndpointsResolver(_DiscoveryEndpointsResolver):
    ...
    async def resolve(self):
        ...
        try:
            resolved = await connection(_list_endpoints_request_factory(self._driver_config), _apis.DiscoveryService.Stub, _apis.DiscoveryService.ListEndpoints, DiscoveryResult.from_response, settings=settings_impl.BaseRequestSettings().with_timeout(self._ready_timeout))
            self._add_debug_details('Resolved endpoints for database %s: %s', self._driver_config.database, resolved)
            return resolved
        except Exception as e:
            self._add_debug_details('Failed to resolve endpoints for database %s. Endpoint: "%s". Error details:\n %s', self._driver_config.database, endpoint, e)
        finally:
