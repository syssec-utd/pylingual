...
__all__ = ['FunnelModel', 'FunnelForSequenceClassification', 'FunnelForTokenClassification', 'FunnelForQuestionAnswering']
...
class PretrainedConfig(dict):
    ...
    def to_diff_dict(self):
        ...
        for (key, value) in config_dict.items():
            if key not in default_config_dict or key == 'hf_paddle_version' or value != default_config_dict[key] or (key in class_config_dict and value != class_config_dict[key]):
...
class FunnelAttentionStructure(nn.Layer):
    ...
    def init_attention_inputs(self, inputs_embeds, attention_mask=None, token_type_ids=None):
        ...
        cls_mask = pad(paddle.ones([seq_len - 1, seq_len - 1], dtype=inputs_embeds.dtype), (1, 0, 1, 0)) if self.config2.separate_cls else None
    ...
    def pool_tensor(self, tensor, mode='mean', stride=2):
        ...
        ndim = tensor.ndim
        if ndim == 2:
            tensor = tensor.unsqueeze(1).unsqueeze(3)
        elif ndim == 3:
...
def Parameter(shape_or_tensor, fill_value=None, requires_grad=True):
    if isinstance(shape_or_tensor, paddle.Tensor):
        ...
        X = paddle.create_parameter(shape=shape_or_tensor, dtype='float32', attr=paddle.ParamAttr(name=None, initializer=paddle.nn.initializer.Constant(value=fill_value)), is_bias=False)
...
class FunnelRelMultiheadAttention(nn.Layer):
    ...
    def relative_positional_attention(self, position_embeds, q_head, context_len, cls_mask=None):
        ...
        if self.config2.attention_type == 'factorized':
            ...
            q_r_attention_1 = q_r_attention * phi.unsqueeze(1)
            q_r_attention_2 = q_r_attention * pi.unsqueeze(1)
            positional_attn = paddle.einsum('bind,jd->bnij', q_r_attention_1, psi) + paddle.einsum('bind,jd->bnij', q_r_attention_2, omega)
        else:
    ...
    def relative_token_type_attention(self, token_type_mat, q_head, cls_mask=None):
        ...
        token_type_attn = paddle.where(token_type_mat, expand(same_token_type, token_type_mat.shape), expand(diff_token_type, token_type_mat.shape))
        if cls_mask is not None:
    ...
    def forward(self, query, key, value, attention_inputs, output_attentions=False):
        ...
        q_head = paddle.reshape(self.q_head(query), (batch_size, seq_len, n_head, d_head))
        k_head = paddle.reshape(self.k_head(key), (batch_size, context_len, n_head, d_head))
...
class FunnelEncoder(nn.Layer):
    ...
    def __init__(self, config):
        ...
        self.blocks = nn.LayerList([nn.LayerList([FunnelLayer(config, block_index) for _ in range(block_size)]) for (block_index, block_size) in enumerate(config.block_sizes)])

    def forward(self, inputs_embeds, attention_mask=None, token_type_ids=None, output_attentions=False, output_hidden_states=False, return_dict=True):
...
class FunnelDecoder(nn.Layer):
    ...
    def forward(self, final_hidden, first_block_hidden, attention_mask=None, token_type_ids=None, output_attentions=False, output_hidden_states=False, return_dict=True):
        upsampled_hidden = upsample(final_hidden, stride=2 ** (len(self.config2.block_sizes) - 1), target_len=first_block_hidden.shape[1], separate_cls=self.config2.separate_cls, truncate_seq=self.config2.truncate_seq)
...
class FunnelPreTrainedModel(PreTrainedModel):
    ...
    models.
    """
    pretrained_init_configuration = {'funnel-transformer/small': {}, 'funnel-transformer/small-base': {}, 'funnel-transformer/medium': {}, 'funnel-transformer/medium-base': {}, 'funnel-transformer/intermediate': {}, 'funnel-transformer/intermediate-base': {}, 'funnel-transformer/large': {}, 'funnel-transformer/large-base': {}, 'funnel-transformer/xlarge-base': {}, 'funnel-transformer/xlarge': {}}
    resource_files_names = {'model_state': 'model_state.pdparams', 'model_config': 'model_config.json'}
    pretrained_resource_files_map = {'model_state': {'funnel-transformer/small': 'https://bj.bcebos.com/paddlenlp/models/transformers/funnel-transformer/small/model_state.pdparams', 'funnel-transformer/small-base': 'https://bj.bcebos.com/paddlenlp/models/transformers/funnel-transformer/small-base/model_state.pdparams', 'funnel-transformer/medium': 'https://bj.bcebos.com/paddlenlp/models/transformers/funnel-transformer/medium/model_state.pdparams', 'funnel-transformer/medium-base': 'https://bj.bcebos.com/paddlenlp/models/transformers/funnel-transformer/medium-base/model_state.pdparams', 'funnel-transformer/intermediate': 'https://bj.bcebos.com/paddlenlp/models/transformers/funnel-transformer/intermediate/model_state.pdparams', 'funnel-transformer/intermediate-base': 'https://bj.bcebos.com/paddlenlp/models/transformers/funnel-transformer/intermediate-base/model_state.pdparams', 'funnel-transformer/large': 'https://bj.bcebos.com/paddlenlp/models/transformers/funnel-transformer/large/model_state.pdparams', 'funnel-transformer/large-base': 'https://bj.bcebos.com/paddlenlp/models/transformers/funnel-transformer/large-base/model_state.pdparams', 'funnel-transformer/xlarge-base': 'https://bj.bcebos.com/paddlenlp/models/transformers/funnel-transformer/xlarge-base/model_state.pdparams', 'funnel-transformer/xlarge': 'https://bj.bcebos.com/paddlenlp/models/transformers/funnel-transformer/xlarge/model_state.pdparams'}, 'model_config': {'funnel-transformer/small': 'https://bj.bcebos.com/paddlenlp/models/transformers/funnel-transformer/small/model_config.json', 'funnel-transformer/small-base': 'https://bj.bcebos.com/paddlenlp/models/transformers/funnel-transformer/small-base/model_config.json', 'funnel-transformer/medium': 'https://bj.bcebos.com/paddlenlp/models/transformers/funnel-transformer/medium/model_config.json', 'funnel-transformer/medium-base': 'https://bj.bcebos.com/paddlenlp/models/transformers/funnel-transformer/medium-base/model_config.json', 'funnel-transformer/intermediate': 'https://bj.bcebos.com/paddlenlp/models/transformers/funnel-transformer/intermediate/model_config.json', 'funnel-transformer/intermediate-base': 'https://bj.bcebos.com/paddlenlp/models/transformers/funnel-transformer/intermediate-base/model_config.json', 'funnel-transformer/large': 'https://bj.bcebos.com/paddlenlp/models/transformers/funnel-transformer/large/model_config.json', 'funnel-transformer/large-base': 'https://bj.bcebos.com/paddlenlp/models/transformers/funnel-transformer/large-base/model_config.json', 'funnel-transformer/xlarge-base': 'https://bj.bcebos.com/paddlenlp/models/transformers/funnel-transformer/xlarge-base/model_config.json', 'funnel-transformer/xlarge': 'https://bj.bcebos.com/paddlenlp/models/transformers/funnel-transformer/xlarge/model_config.json'}}
    ...
    def prune_heads(self, heads_to_prune):
        ...
        for (layer, heads) in heads_to_prune.items():
            ...
            self.config2.pruned_heads[layer] = list(union_heads)
...
class FunnelBaseModel(FunnelPreTrainedModel):
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None, inputs_embeds=None, output_attentions=None, output_hidden_states=None, return_dict=None):
        ...
        output_hidden_states = output_hidden_states if output_hidden_states is not None else self.config2.output_hidden_states
...
class FunnelModel(FunnelPreTrainedModel):
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, inputs_embeds=None, output_attentions=None, output_hidden_states=None, return_dict=None):
        ...
        decoder_outputs = self.decoder(final_hidden=encoder_outputs.last_hidden_state, first_block_hidden=encoder_outputs.hidden_states[self.config2.block_sizes[0]], attention_mask=attention_mask, token_type_ids=token_type_ids, output_attentions=output_attentions, output_hidden_states=output_hidden_states, return_dict=return_dict)
        if not return_dict:
            idx = 0
            ...
            return outputs
        return BaseModelOutput(last_hidden_state=decoder_outputs.last_hidden_state, hidden_states=encoder_outputs.hidden_states + decoder_outputs.hidden_states if output_hidden_states else None, attentions=encoder_outputs.attentions + decoder_outputs.attentions if output_attentions else None)
...
class FunnelForMultipleChoice(FunnelPreTrainedModel):
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, inputs_embeds=None, labels=None, output_attentions=None, output_hidden_states=None, return_dict=None):
        ...
        inputs_embeds = inputs_embeds.reshape(-1, inputs_embeds.shape[-2], inputs_embeds.shape[-1]) if inputs_embeds is not None else None
        outputs = self.funnel(input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids, inputs_embeds=inputs_embeds, output_attentions=output_attentions, output_hidden_states=output_hidden_states, return_dict=return_dict)
        last_hidden_state = outputs[0]
        pooled_output = last_hidden_state[:, 0]
        logits = self.classifier(pooled_output)
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, inputs_embeds=None, labels=None, output_attentions=None, output_hidden_states=None, return_dict=None):
        ...
        if labels is not None:
            ...
            if attention_mask is not None:
                ...
                active_labels = paddle.where(active_loss, labels.reshape(-1), paddle.tensor(loss_fct.ignore_index).astype(labels.dtype))
