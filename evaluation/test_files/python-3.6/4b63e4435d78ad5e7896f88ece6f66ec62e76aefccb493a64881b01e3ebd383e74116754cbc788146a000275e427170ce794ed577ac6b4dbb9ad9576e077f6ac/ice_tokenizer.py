...
class GLM130BTokenizer:
    ...
    def __init__(self, path='~/.icetk_models', max_blank_length=80, byte_fallback=True):
        if path is not None:
    ...
    @staticmethod
    def _configure_tokenizer(text_tokenizer: TextTokenizer, special_tokens: List[str], max_blank_length: int, byte_fallback: bool, encode_special_tokens=False):
        special_token_type = 4 if encode_special_tokens else 3
        for token in special_tokens:
            text_tokenizer.proto.pieces.append(sp_model.ModelProto.SentencePiece(piece=token, score=0.0, type=special_token_type))
        for token in [GLM130BTokenizer.get_tab_token()] + [GLM130BTokenizer.get_blank_token(i) for i in range(2, max_blank_length + 1)]:
        ...
        if byte_fallback:
            ...
            for i in range(256):
                text_tokenizer.proto.pieces.append(sp_model.ModelProto.SentencePiece(piece='<0x{:02X}>'.format(i), score=0.0, type=6))
    ...
    def _get_text_tokenizer(self, encode_special_tokens=False):
        ...
        if not hasattr(self, name):
            ...
            self._configure_tokenizer(tokenizer, self.special_tokens, self.max_blank_length, self.byte_fallback, encode_special_tokens)
    ...
    def num_text_tokens(self):
        return self.text_tokenizer.num_tokens + len(self.special_tokens) + (self.max_blank_length - 2) + (256 if self.byte_fallback else 0)
    ...
    def encode(self, text: str, linebreak=True, whitespaces=True, special_tokens=False, add_dummy_prefix=True) -> List[int]:
        """
    ...
    def tokenize(self, text: str, linebreak=True, whitespaces=True, special_tokens=False, add_dummy_prefix=True) -> List[str]:
        """
        @param text: Text to encode.
        @param linebreak: Whether to encode newline (
) in text.
