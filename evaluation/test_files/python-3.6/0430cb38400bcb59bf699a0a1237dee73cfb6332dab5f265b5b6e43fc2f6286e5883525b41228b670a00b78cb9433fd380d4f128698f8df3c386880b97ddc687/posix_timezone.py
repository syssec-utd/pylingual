...
_spec = re.compile('^(?P<std_abbr><.*?>|[^-+,\\d]{3,})(?P<std_offset>([+-])?(\\d{1,2})(:\\d{2}(:\\d{2})?)?)(?P<dst_info>    (?P<dst_abbr><.*?>|[^-+,\\d]{3,})    (?P<dst_offset>([+-])?(\\d{1,2})(:\\d{2}(:\\d{2})?)?)?)?(?:,(?P<rules>    (?P<dst_start>        (?:J\\d+|\\d+|M\\d{1,2}.\\d.[0-6])        (?:/(?P<dst_start_offset>([+-])?(\\d+)(:\\d{2}(:\\d{2})?)?))?    )    ,    (?P<dst_end>        (?:J\\d+|\\d+|M\\d{1,2}.\\d.[0-6])        (?:/(?P<dst_end_offset>([+-])?(\\d+)(:\\d{2}(:\\d{2})?)?))?    )))?$', re.VERBOSE)
...
class PosixTimezone:
    ...
    def __init__(self, std_abbr, std_offset, dst_abbr, dst_offset, dst_start=None, dst_end=None):
