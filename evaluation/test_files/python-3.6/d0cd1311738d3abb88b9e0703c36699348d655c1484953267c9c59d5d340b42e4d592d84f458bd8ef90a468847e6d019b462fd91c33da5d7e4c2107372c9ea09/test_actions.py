...
class AdminActionsTests(TestCase):
    ...
    def test_get_actions_respects_permissions(self):
        ...
        mock_request.GET = {}
        cases = [(None, self.viewuser, ['custom_action']), ('view', self.superuser, ['delete_selected', 'custom_action']), ('view', self.viewuser, ['custom_action']), ('add', self.adduser, ['custom_action']), ('change', self.changeuser, ['custom_action']), ('delete', self.deleteuser, ['delete_selected', 'custom_action']), ('custom', self.customuser, ['custom_action'])]
    ...
    def test_global_actions_description(self):
        ...
        self.assertEqual([description for (_, _, description) in ma._get_base_actions()], ['Delete selected %(verbose_name_plural)s', 'Site-wide admin action 1.', 'Global action 2'])
    ...
    def test_actions_replace_global_action(self):
        ...
        self.assertEqual([desc for (_, name, desc) in ma._get_base_actions() if name.startswith('custom_action')], ['Local admin action 1.', 'Local admin action 2.'])