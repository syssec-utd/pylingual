...
class GPTEmptyIndex(BaseGPTIndex[EmptyIndex]):
    ...
    def __init__(self, index_struct: Optional[EmptyIndex]=None, llm_predictor: Optional[LLMPredictor]=None, text_splitter: Optional[TextSplitter]=None, **kwargs: Any) -> None:
        """Initialize params."""
        super().__init__(documents=[], index_struct=index_struct, llm_predictor=llm_predictor, text_splitter=text_splitter, **kwargs)

    @classmethod
    def get_query_map(self) -> Dict[str, Type[BaseGPTIndexQuery]]:
        """Get query map."""
        return {QueryMode.DEFAULT: GPTEmptyIndexQuery}

    def _build_index_from_documents(self, documents: Sequence[BaseDocument]) -> EmptyIndex:
