    ...
    * ``identity_key(class, row=row, identity_token=token)``
    ...
    if args:
        ...
        if largs == 1:
            ...
            raise sa_exc.ArgumentError('expected up to three positional arguments, got %s' % largs)
        identity_token = kwargs.pop('identity_token', None)
        if kwargs:
            raise sa_exc.ArgumentError('unknown keyword arguments: %s' % ', '.join(kwargs))
        mapper = class_mapper(class_)
        if row is None:
            return mapper.identity_key_from_primary_key(util.to_list(ident), identity_token=identity_token)
        else:
            return mapper.identity_key_from_row(row, identity_token=identity_token)
    else:
        instance = kwargs.pop('instance')
        if kwargs:
            raise sa_exc.ArgumentError('unknown keyword arguments: %s' % ', '.join(kwargs.keys))
...
class ORMAdapter(sql_util.ColumnAdapter):
    ...
    def __init__(self, entity, equivalents=None, adapt_required=False, allow_label_resolve=True, anonymize_labels=False):
        ...
        sql_util.ColumnAdapter.__init__(self, selectable, equivalents, adapt_required=adapt_required, allow_label_resolve=allow_label_resolve, anonymize_labels=anonymize_labels, include_fn=self._include_fn)
...
class AliasedInsp(ORMEntityColumnsClauseRole, ORMFromClauseRole, sql_base.MemoizedHasCacheKey, InspectionAttr):
    ...
    * ``polymorphic_on`` - an alternate column or SQL expression which
    ...
    def __init__(self, entity, inspected, selectable, name, with_polymorphic_mappers, polymorphic_on, _base_alias, _use_mapper_path, adapt_on_names, represents_outer_join):
        ...
        self._adapter = sql_util.ColumnAdapter(selectable, equivalents=mapper._equivalent_columns, adapt_on_names=adapt_on_names, anonymize_labels=True, adapt_from_selectables=[m.selectable for m in self.with_polymorphic_mappers])
    ...
    def entity_namespace(self):
        return self.entity
    _cache_key_traversal = [('name', visitors.ExtendedInternalTraversal.dp_string), ('_adapt_on_names', visitors.ExtendedInternalTraversal.dp_boolean), ('selectable', visitors.ExtendedInternalTraversal.dp_clauseelement)]
    ...
    def __setstate__(self, state):
        self.__init__(state['entity'], state['mapper'], state['alias'], state['name'], state['with_polymorphic_mappers'], state['with_polymorphic_discriminator'], state['base_alias'], state['use_mapper_path'], state['adapt_on_names'], state['represents_outer_join'])

    def _adapt_element(self, elem, key=None):
        d = {'parententity': self, 'parentmapper': self.mapper}
        if key:
            d['proxy_key'] = key
        return self._adapter.traverse(elem)._annotate(d)._set_propagate_attrs({'compile_state_plugin': 'orm', 'plugin_subject': self})
...
class LoaderCriteriaOption(CriteriaOption):
    ...
    def __init__(self, entity_or_base, where_criteria, loader_only=False, include_aliases=False, propagate_to_loaders=True, track_closure_variables=True):
        ...
        **globally**, meaning it will apply to the entity as it appears
        ...
        if callable(where_criteria):
            ...
            self.where_criteria = lambdas.DeferredLambdaElement(where_criteria, roles.WhereHavingRole, lambda_args=(_WrapUserEntity(self.root_entity if self.root_entity is not None else self.entity.entity),), opts=lambdas.LambdaOptions(track_closure_variables=track_closure_variables))
        else:
    ...
    def process_compile_state(self, compile_state):
        ...
        if compile_state.compile_options._with_polymorphic_adapt_map:
            util.warn('The with_loader_criteria() function may not work correctly with the legacy Query.with_polymorphic() feature.  Please migrate code to use the with_polymorphic() standalone function before using with_loader_criteria().')
...
def with_polymorphic(base, classes, selectable=False, flat=False, polymorphic_on=None, aliased=False, innerjoin=False, _use_mapper_path=False, _existing_alias=None):
    ...
    return AliasedClass(base, selectable, with_polymorphic_mappers=mappers, with_polymorphic_discriminator=polymorphic_on, use_mapper_path=_use_mapper_path, represents_outer_join=not innerjoin)
...
class Bundle(ORMColumnsClauseRole, SupportsCloneAnnotations, sql_base.MemoizedHasCacheKey, InspectionAttr):
    ...
    def __init__(self, name, *exprs, **kw):
        ...
        self.exprs = exprs = [coercions.expect(roles.ColumnsClauseRole, expr, apply_propagate_attrs=self) for expr in exprs]
        self.c = self.columns = ColumnCollection(((getattr(col, 'key', col._label), col) for col in [e._annotations.get('bundle', e) for e in exprs]))
        self.single_entity = kw.pop('single_entity', self.single_entity)

    def _gen_cache_key(self, anon_map, bindparams):
        return (self.__class__, self.name, self.single_entity) + tuple([expr._gen_cache_key(anon_map, bindparams) for expr in self.exprs])
    ...
    def __clause_element__(self):
        ...
        return expression.ClauseList(*[e._annotations.get('bundle', e) for e in self.exprs], _literal_as_text_role=roles.ColumnsClauseRole, group=False)._annotate(annotations)._set_propagate_attrs({'compile_state_plugin': 'orm', 'plugin_subject': plugin_subject})
...
class _ORMJoin(expression.Join):
    ...
    def __init__(self, left, right, onclause=None, isouter=False, full=False, _left_memo=None, _right_memo=None, _extra_criteria=()):
...
def with_parent(instance, prop, from_entity=None):
    ...
    if isinstance(prop, util.string_types):
        util.warn_deprecated_20('Using strings to indicate relationship names in the ORM with_parent() function is deprecated and will be removed SQLAlchemy 2.0.  Please use the class-bound attribute directly.')
        mapper = object_mapper(instance)
...
def _entity_corresponds_to_use_path_impl(given, entity):
    ...
    """
    if given.is_aliased_class:
        return entity.is_aliased_class and (not entity._use_mapper_path) and (given is entity or given in entity._with_polymorphic_entities)
...
def _getitem(iterable_query, item, allow_negative):
    ...
    def _no_negative_indexes():
        if not allow_negative:
            ...
            util.warn_deprecated_20('Support for negative indexes for SQL index / slice operators will be removed in 2.0; these operators fetch the complete result and do not work efficiently.')
