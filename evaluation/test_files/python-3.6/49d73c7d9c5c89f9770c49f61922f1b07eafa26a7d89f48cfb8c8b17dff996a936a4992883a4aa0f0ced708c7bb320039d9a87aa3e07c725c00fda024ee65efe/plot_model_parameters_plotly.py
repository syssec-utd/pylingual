...
def plot_scalar_weights(weights, plot_name, focus=None, multiplicative=False):
    ...
    traces.append(go.Bar(name=plot_name, x=names, y=values, marker_color=color, width=0.8))
...
def plot_weekly(m, quantile, comp_name='weekly', weekly_start=0, quick=True, multiplicative=False, df_name='__df__'):
    ...
    days = days.day_name()
    traces.append(go.Scatter(name=comp_name + ' Mean' if mean_std else comp_name, x=list(range(len(days_i))), y=predicted[comp_name], mode='lines', line=dict(color=color, width=line_width), fill='none'))
    if mean_std:
        filling = 'tonexty'
        traces.append(go.Scatter(name='Quant 10%', x=list(range(len(days_i))), y=predicted_q10[comp_name], mode='lines', line=dict(color='rgba(45, 146, 255, 0.2)', width=1), fillcolor='rgba(45, 146, 255, 0.2)', showlegend=True))
        traces.append(go.Scatter(name='Quant 90%', x=list(range(len(days_i))), y=predicted_q90[comp_name], fill=filling, mode='lines', line=dict(color='rgba(45, 146, 255, 0.2)', width=1), fillcolor='rgba(45, 146, 255, 0.2)', showlegend=False))
...
def plot_parameters(m, plot_configuration, quantile=0.5, weekly_start=0, yearly_start=0, figsize=(700, 210), df_name=None, forecast_in_focus=None):
