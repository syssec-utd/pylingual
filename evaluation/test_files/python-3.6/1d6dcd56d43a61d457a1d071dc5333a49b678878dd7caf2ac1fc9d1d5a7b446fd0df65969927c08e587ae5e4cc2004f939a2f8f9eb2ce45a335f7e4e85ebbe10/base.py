...
class DeterminedDeployment(metaclass=abc.ABCMeta):
    ...
    template = None
    master_info = 'Configure the Determined CLI: ' + colored('export DET_MASTER={master_url}', 'yellow')
    ui_info = 'View the Determined UI: ' + colored('{master_url}', 'blue')
    logs_info = 'View Logs at: ' + colored('https://{region}.console.aws.amazon.com/cloudwatch/home?region={region}#logStream:group={log_group}', 'blue')
    ssh_info = 'SSH to master Instance: ' + colored('ssh -i <pem-file> ubuntu@{master_ip}', 'yellow')

    def __init__(self, parameters: Dict[str, Any]) -> None:
        assert self.template is not None
        self.template_path = pkg_resources.resource_filename(constants.misc.TEMPLATE_PATH, self.template)
    ...
    def consolidate_parameters(self) -> List[Dict[str, Any]]:
        return [{'ParameterKey': k, 'ParameterValue': str(self.parameters[k])} for k in self.parameters.keys() if self.parameters[k] and k in self.template_parameter_keys]
    ...
    def before_deploy_print(self) -> None:
        ...
        version = self.parameters[constants.cloudformation.VERSION] if self.parameters[constants.cloudformation.VERSION] else determined.__version__
    ...
    def info_partials(self) -> Iterable[str]:
        return (self.master_info, self.ui_info, self.logs_info, self.ssh_info)
    ...
    def print_results(self) -> None:
        ...
        master_url = self._get_master_url()
        self.print_output_info(master_ip=master_ip, master_url=master_url, region=region, log_group=log_group)
