...
class Dataset(torch.utils.data.Dataset, ABC):
    ...
    def num_node_features(self) -> int:
        ...
        raise AttributeError(f"'{data.__class__.__name__}' object has no attribute 'num_node_features'")
    ...
    def num_edge_features(self) -> int:
        ...
        raise AttributeError(f"'{data.__class__.__name__}' object has no attribute 'num_edge_features'")
    ...
    def _process(self):
        ...
        if osp.exists(f) and torch.load(f) != _repr(self.pre_transform):
            warnings.warn(f"The `pre_transform` argument differs from the one used in the pre-processed version of this dataset. If you want to make use of another pre-processing technique, make sure to delete '{self.processed_dir}' first")
        f = osp.join(self.processed_dir, 'pre_filter.pt')
        if osp.exists(f) and torch.load(f) != _repr(self.pre_filter):
            warnings.warn("The `pre_filter` argument differs from the one used in the pre-processed version of this dataset. If you want to make use of another pre-fitering technique, make sure to delete '{self.processed_dir}' first")
