...
class ScaledNumerics(TransformerMixin, BaseEstimator):
    ...
    def fit(self, X: pd.DataFrame):
        ...
        self.rare_category_encoder = RareCategoryEncoder(max_num_categories=self.max_num_categories, cols=self.cat_columns)
...
def preprocess_2_cat_cols_to_same_bins(dist1: Union[np.ndarray, pd.Series], dist2: Union[np.ndarray, pd.Series], max_num_categories: int=None, sort_by: str='difference') -> Tuple[np.ndarray, np.ndarray, List]:
    """
    ...
    if max_num_categories is not None and len(all_categories) > max_num_categories:
        ...
        if sort_by == 'dist1':
            ...
            sort_by_counter = Counter({key: abs(dist1_counter[key] - dist2_counter[key]) for key in set(dist1_counter.keys()).union(dist2_counter.keys())})
        else:
            raise DeepchecksValueError(f'sort_by got unexpected value: {sort_by}')
        categories_list = [x[0] for x in sorted(sort_by_counter.items(), key=lambda x: (-x[1], x[0]))][:max_num_categories]
