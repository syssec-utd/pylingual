...
def _override_url(http_msg, secret_field):
    ...
    if secret_field in url_parsed.path:
        new_url = url_parsed._replace(path=url_parsed.path.replace(secret_field, SENSITIVE_INFO_SUBSTITUTION_FLAG))
...
def _override_params(http_msg, secret_field):
    ...
    for value in param_values_list:
        url_parsed = url_parsed._replace(query=url_parsed.query.replace(f'{secret_field}={value}', f'{secret_field}={SENSITIVE_INFO_SUBSTITUTION_FLAG}'))
