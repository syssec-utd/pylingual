...
from jet_bridge_base.utils.graphql import ModelFiltersType, ModelFiltersFieldType, ModelFiltersRelationshipType, ModelLookupsType, ModelLookupsFieldType, ModelLookupsRelationshipType
...
class StatusView(BaseAPIView):
    ...
    def map_connection_graphql_schema(self, schema):
        ...
        if instance:
            ...
            return {'status': 'ok', 'types': types_count, 'filters': filters_count, 'filters_fields': filters_fields_count, 'filters_relationships': filters_relationships_count, 'lookups': lookups_count, 'lookups_fields': lookups_fields_count, 'lookups_relationships': lookups_relationships_count, 'get_schema_time': get_schema_time}
        else:
    ...
    def map_tunnel(self, tunnel):
        if not tunnel:
            return
        return {'is_active': tunnel.is_active, 'local_address': '{}:{}'.format(tunnel.local_bind_host, tunnel.local_bind_port), 'remote_address': '{}:{}'.format(tunnel.ssh_host, tunnel.ssh_port)}
    ...
    def map_connection(self, connection):
        ...
        return {'name': connection['name'], 'project': connection.get('project'), 'token': connection.get('token'), 'tables': len(MappedBase.classes), 'columns': column_count, 'relationships': relationships_count, 'graphql_schema': graphql_schema, 'graphql_schema_draft': graphql_schema_draft, 'init_start': connection.get('init_start'), 'connect_time': connection.get('connect_time'), 'reflect_time': connection.get('reflect_time'), 'tunnel': tunnel}

    def map_pending_connection(self, pending_connection):
        tunnel = self.map_tunnel(pending_connection.get('tunnel'))
        return {'name': pending_connection['name'], 'project': pending_connection.get('project'), 'token': pending_connection.get('token'), 'init_start': pending_connection.get('init_start'), 'tables_processed': pending_connection.get('tables_processed'), 'tables_total': pending_connection.get('tables_total'), 'tunnel': tunnel}

    def get(self, request, *args, **kwargs):
        now = time.time()
        uptime = round(now - configuration.init_time)
        return JSONResponse({'total_pending_connections': len(pending_connections.keys()), 'total_connections': len(connections.keys()), 'pending_connections': map(lambda x: self.map_pending_connection(x), pending_connections.values()), 'connections': map(lambda x: self.map_connection(x), connections.values()), 'uptime': uptime})