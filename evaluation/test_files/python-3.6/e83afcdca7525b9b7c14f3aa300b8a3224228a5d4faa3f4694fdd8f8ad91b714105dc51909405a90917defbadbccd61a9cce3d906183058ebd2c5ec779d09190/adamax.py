...
class Adamax(optimizer_v2.OptimizerV2):
  ...
  """
    ...
    def _prepare_local(self, var_device, var_dtype, apply_state):
        ...
        apply_state[var_device, var_dtype].update(dict(neg_scaled_lr=-lr_t / (1 - beta_1_power), epsilon=ops.convert_to_tensor(self.epsilon, var_dtype), beta_1_t=beta_1_t, beta_1_power=beta_1_power, one_minus_beta_1_t=1 - beta_1_t, beta_2_t=beta_2_t, zero=array_ops.zeros((), dtype=dtypes.int64)))
    ...
    def _resource_apply_dense(self, grad, var, apply_state=None):
        ...
        return training_ops.resource_apply_ada_max(var.handle, m.handle, v.handle, coefficients['beta_1_power'], coefficients['lr_t'], coefficients['beta_1_t'], coefficients['beta_2_t'], coefficients['epsilon'], grad, use_locking=self._use_locking)
    ...
    def get_config(self):
        ...
        config.update({'learning_rate': self._serialize_hyperparameter('learning_rate'), 'decay': self._serialize_hyperparameter('decay'), 'beta_1': self._serialize_hyperparameter('beta_1'), 'beta_2': self._serialize_hyperparameter('beta_2'), 'epsilon': self.epsilon})
        return config