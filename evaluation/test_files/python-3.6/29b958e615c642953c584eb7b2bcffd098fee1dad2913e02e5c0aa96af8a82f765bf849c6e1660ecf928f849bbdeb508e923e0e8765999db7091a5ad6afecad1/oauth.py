...
class DeclarativeOauth2Authenticator(AbstractOauth2Authenticator, DeclarativeAuthenticator, JsonSchemaMixin):
    ...
    def __post_init__(self, options: Mapping[str, Any]):
        ...
        self._token_expiry_date = pendulum.parse(InterpolatedString.create(self.token_expiry_date, options=options).eval(self.config)) if self.token_expiry_date else pendulum.now().subtract(days=1)
