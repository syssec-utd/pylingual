...
def _transform_cirq_circuit_to_pyquil_program(*, circuit: cirq.Circuit, qubits: Optional[Tuple[cirq.Qid, ...]]=None, decompose_operation: Optional[Callable[[cirq.Operation], List[cirq.Operation]]]=None, qubit_id_map: Optional[Dict[cirq.Qid, str]]=None, post_transformation_hooks: Optional[List[_PostTransformationHook]]=None) -> Tuple[Program, Dict[str, str]]:
    if qubits is None:
        qubits = cirq.QubitOrder.as_qubit_order(cirq.ops.QubitOrder.DEFAULT).order_for(circuit.all_qubits())
    quil_output = RigettiQCSQuilOutput(operations=circuit.all_operations(), qubits=qubits, qubit_id_map=qubit_id_map, decompose_operation=decompose_operation)
    ...
    if post_transformation_hooks is not None:
        for hook in post_transformation_hooks:
            (program, measurement_id_map) = hook(program=program, measurement_id_map=measurement_id_map)
...
def build(*, qubits: Optional[Tuple[cirq.Qid, ...]]=None, decompose_operation: Optional[Callable[[cirq.Operation], List[cirq.Operation]]]=None, qubit_id_map: Optional[Dict[cirq.Qid, str]]=None, post_transformation_hooks: Optional[List[_PostTransformationHook]]=None) -> CircuitTransformer:
    ...
    def transformer(*, circuit: cirq.Circuit) -> Tuple[Program, Dict[str, str]]:
        return _transform_cirq_circuit_to_pyquil_program(circuit=circuit, qubits=qubits, decompose_operation=decompose_operation, qubit_id_map=qubit_id_map, post_transformation_hooks=post_transformation_hooks)
