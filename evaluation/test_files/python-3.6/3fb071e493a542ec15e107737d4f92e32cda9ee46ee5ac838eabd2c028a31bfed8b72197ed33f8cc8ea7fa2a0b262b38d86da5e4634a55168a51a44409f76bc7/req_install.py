...
from pip._internal.utils.misc import ConfiguredPep517HookCaller, ask_path_exists, backup_dir, display_path, hide_url, redact_auth_from_url
...
class InstallRequirement:
    ...
    def load_pyproject_toml(self) -> None:
        ...
        self.pep517_backend = ConfiguredPep517HookCaller(self, self.unpacked_source_directory, backend, backend_path=backend_path)
    ...
    def archive(self, build_dir: Optional[str]) -> None:
        ...
        if os.path.exists(archive_path):
            response = ask_path_exists('The file {} exists. (i)gnore, (w)ipe, (b)ackup, (a)bort '.format(display_path(archive_path)), ('i', 'w', 'b', 'a'))
            if response == 'i':
    ...
    def install(self, install_options: List[str], global_options: Optional[Sequence[str]]=None, root: Optional[str]=None, home: Optional[str]=None, prefix: Optional[str]=None, warn_script_location: bool=True, use_user_site: bool=False, pycompile: bool=True) -> None:
        scheme = get_scheme(self.name, user=use_user_site, home=home, root=root, isolated=self.isolated, prefix=prefix)
        global_options = global_options if global_options is not None else []
        if self.editable and (not self.is_wheel):
            install_editable_legacy(install_options, global_options, prefix=prefix, home=home, use_user_site=use_user_site, name=self.name, setup_py_path=self.setup_py_path, isolated=self.isolated, build_env=self.build_env, unpacked_source_directory=self.unpacked_source_directory)
        ...
        if self.is_wheel:
            ...
            install_wheel(self.name, self.local_file_path, scheme=scheme, req_description=str(self.req), pycompile=pycompile, warn_script_location=warn_script_location, direct_url=direct_url, requested=self.user_supplied)
            self.install_succeeded = True
            return
        global_options = list(global_options) + self.global_options
        install_options = list(install_options) + self.install_options
        try:
            success = install_legacy(install_options=install_options, global_options=global_options, root=root, home=home, prefix=prefix, use_user_site=use_user_site, pycompile=pycompile, scheme=scheme, setup_py_path=self.setup_py_path, isolated=self.isolated, req_name=self.name, build_env=self.build_env, unpacked_source_directory=self.unpacked_source_directory, req_description=str(self.req))
        ...
        if success and self.legacy_install_reason == 8368:
            deprecated(reason="{} was installed using the legacy 'setup.py install' method, because a wheel could not be built for it.".format(self.name), replacement='to fix the wheel build issue reported above', gone_in=None, issue=8368)
